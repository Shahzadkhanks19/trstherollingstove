...
const PrintJobSchema = new Schema(
  {
    documentType: {
      type: String,
      enum: ["invoice", "kot", "revision_kot", "report"],
      required: true,
      index: true,
    },
    entityType: {
    ...
    status: {
      type: String,
      enum: ["requested", "opened", "printed", "failed"],
      default: "requested",
      index: true,
    },
    requestedBy: {
...
export const PrintJob: Model<PrintJobDocument> =
  (models.PrintJob as Model<PrintJobDocument>) ||
  model<PrintJobDocument>("PrintJob", PrintJobSchema);
