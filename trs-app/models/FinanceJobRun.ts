...
FinanceJobRunSchema.index({ jobType: 1, createdAt: -1 });
FinanceJobRunSchema.index({ status: 1, createdAt: -1 });
export type FinanceJobRunDocument = InferSchemaType<typeof FinanceJobRunSchema>;
export const FinanceJobRun: Model<FinanceJobRunDocument> =
  (models.FinanceJobRun as Model<FinanceJobRunDocument>) ||
  model<FinanceJobRunDocument>("FinanceJobRun", FinanceJobRunSchema);
