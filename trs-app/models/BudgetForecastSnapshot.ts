...
const BudgetForecastSnapshotSchema = new Schema(
  {
    ...
    scenario: {
      type: String,
      enum: ["base", "optimistic", "conservative"],
      required: true,
      index: true,
    },
    department: {
...
BudgetForecastSnapshotSchema.index({
  fiscalYear: -1,
  scenario: 1,
  department: 1,
});
export type BudgetForecastSnapshotRecord = InferSchemaType<
  typeof BudgetForecastSnapshotSchema
>;
export const BudgetForecastSnapshot: Model<BudgetForecastSnapshotRecord> =
  (models.BudgetForecastSnapshot as Model<BudgetForecastSnapshotRecord>) ||
  model<BudgetForecastSnapshotRecord>(
    "BudgetForecastSnapshot",
    BudgetForecastSnapshotSchema,
  );
