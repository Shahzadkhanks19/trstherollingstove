...
const ReportFilterSchema = new Schema(
  {
    ...
    operator: {
      type: String,
      enum: ["eq", "neq", "contains", "gte", "lte", "in"],
      required: true,
    },
...
const ReportDefinitionSchema = new Schema(
  {
    ...
    dataset: {
      type: String,
      enum: ["orders", "internal_consumption"],
      required: true,
      index: true,
    },
    columns: {
      ...
      required: true,
      validate: [
        (value: unknown[]) => value.length > 0 && value.length <= 30,
...
export type ReportDefinitionDocument = InferSchemaType<
  typeof ReportDefinitionSchema
>;
export const ReportDefinition: Model<ReportDefinitionDocument> =
  (models.ReportDefinition as Model<ReportDefinitionDocument>) ||
  model<ReportDefinitionDocument>("ReportDefinition", ReportDefinitionSchema);
