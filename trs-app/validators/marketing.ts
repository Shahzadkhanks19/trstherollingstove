...
export const campaignCreateSchema = z.object({
  ...
  audience: z.object({
    type: z.enum(["all", "segment", "risk", "manual"]).default("all"),
    segmentKeys: z.array(z.string().trim().min(1).max(40)).max(20).default([]),
    riskLevels: z
...
export const automationCreateSchema = z.object({
  ...
  subject: z.string().trim().max(180).default(""),
  message: z.string().trim().min(2).max(5000),
  segmentKey: z.string().trim().max(40).default(""),
...
export const audienceCreateSchema = z.object({
  ...
  riskLevels: z
    ...
    .max(3)
    .default([]),
  minimumLifetimeValue: z.number().min(0).default(0),
  minimumOrders: z.number().int().min(0).default(0),
  maximumDaysSinceLastOrder: z.number().int().min(0).nullable().default(null),
  isActive: z.boolean().default(true),
});
