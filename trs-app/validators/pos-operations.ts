...
const cartLine = z.object({
  ...
  source: z.enum(["menu", "pos"]),
  name: z.string().min(1),
  slug: z.string().default(""),
  imageUrl: z.string().default(""),
  categoryName: z.string().default("Other"),
  ...
  modifierSignature: z.string().default(""),
  modifiers: z
    .array(
      z.object({
        groupId: z.string(),
...
const cart = z.object({
  ...
  lines: z.array(cartLine).min(1).max(100),
  ...
  internalConsumption: z
    .object({
      ...
      reason: z.string(),
      notes: z.string(),
      managerApprovalEmail: z.string(),
      managerApprovalPassword: z.string(),
      managerApprovalReason: z.string(),
    })
    .default({
      saleType: "customer",
      referenceId: null,
  ...
  adjustments: z.object({
    ...
    additionalCharge: z.number().min(0),
    additionalChargeLabel: z.string(),
...
export const createTableSchema = z.object({
  ...
  section: z.string().trim().max(60).default("Main"),
  capacity: z.number().int().min(1).max(50).default(4),
  sortOrder: z.number().int().default(0),
});
export const updateTableSchema = createTableSchema.partial().extend({
  status: z.enum(["available", "reserved", "out_of_service"]).optional(),
  reservationName: z.string().trim().max(100).optional(),
  reservationPhone: z.string().trim().max(20).optional(),
  reservationTime: z.string().datetime().nullable().optional(),
  isActive: z.boolean().optional(),
});
export const createRunningOrderSchema = z.object({
  shiftId: objectId,
  tableId: objectId.nullable().default(null),
  tableName: z.string().trim().max(40).default(""),
...
export const transferRunningOrderSchema = z.object({
  tableId: objectId.nullable(),
  guestCount: z.number().int().min(1).max(100).optional(),
});
export const mergeRunningOrderSchema = z.object({ sourceOrderId: objectId });
export const splitRunningOrderSchema = z.object({
  lineQuantities: z.record(z.string(), z.number().int().min(1)),
  targetTableId: objectId.nullable().default(null),
});
export const voidRunningItemSchema = z.object({
  lineId: z.string().min(1),
  quantity: z.number().int().min(1),
  reason: z.string().trim().min(3).max(500),
});
export const cancelRunningOrderSchema = z.object({
  reason: z.string().trim().min(3).max(500),
});
  ...
  .superRefine((value, context) => {
    ...
    if (value.tipAmount > 0 && value.tipCollection === "none") {
      context.addIssue({
        code: "custom",
        path: ["tipCollection"],
        message: "Select who currently holds the tip.",
      });
    }
    if (value.tipAmount > 0 && !value.orderTakerName) {
      context.addIssue({
        code: "custom",
        path: ["orderTakerName"],
        message: "Enter the waiter or order taker name for the tip.",
      });
    }
    if (value.tipMethod === "upi" && value.tipCollection !== "restaurant") {
      context.addIssue({
        code: "custom",
        path: ["tipCollection"],
        message: "UPI tips are received by the restaurant.",
      });
    }
    if (
      value.paymentMethod === "split" &&
      value.paymentBreakdown.filter((part) => part.amount > 0).length < 2
    ) {
      context.addIssue({
        code: "custom",
        path: ["paymentBreakdown"],
        message: "Add both cash and UPI amounts for split payment.",
      });
    }
    if (value.tipMethod === "upi" && value.paymentMethod === "cash") {
      context.addIssue({
        code: "custom",
        path: ["tipMethod"],
        message: "An online tip must include a UPI payment.",
      });
    }
    if (
      value.tipMethod === "upi" &&
      value.paymentMethod === "split" &&
      !value.paymentBreakdown.some(
        (part) => part.method === "upi" && part.amount > 0,
      )
    ) {
      context.addIssue({
        code: "custom",
        path: ["paymentBreakdown"],
        message: "Add a positive UPI part for the online tip.",
      });
    }
  });
