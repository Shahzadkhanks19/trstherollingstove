...
export const reportDefinitionSchema = z
  ...
  .superRefine((input, context) => {
    ...
    if (
      (input.groups.length > 0 || hasAggregation) &&
      input.columns.some(
        (column) =>
          column.aggregation === "none" && !groupedFields.has(column.key),
      )
    ) {
      context.addIssue({
        code: "custom",
        path: ["columns"],
        message:
          "Every non-aggregated column must also be selected as a grouping field.",
      });
    }
    if (
      input.groups.some(
        (group) =>
          group.interval !== "none" &&
          !input.columns.some(
            (column) => column.key === group.field && column.type === "date",
          ),
      )
    ) {
      context.addIssue({
        code: "custom",
        path: ["groups"],
        message: "Date intervals can only be used with selected date columns.",
      });
    }
    if (input.visualization !== "table") {
      if (!input.chart.categoryField)
        context.addIssue({
          code: "custom",
          path: ["chart", "categoryField"],
          message: "Choose a category field for this visualization.",
        });
      if (!input.chart.series.length && input.visualization !== "kpi")
        context.addIssue({
          code: "custom",
          path: ["chart", "series"],
          message: "Choose at least one numeric series.",
        });
    }
  });
