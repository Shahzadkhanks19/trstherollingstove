...
export const recommendationRebuildSchema = z.object({
  customerLimit: z.number().int().min(1).max(100000).default(10000),
...
export const recommendationRuleSchema = z.object({
  ...
  menuItemIds: z.array(z.string().regex(/^[a-f\d]{24}$/i)).default([]),
  categoryIds: z.array(z.string().regex(/^[a-f\d]{24}$/i)).default([]),
  tags: z.array(z.string().trim().min(1).max(50)).default([]),
  weight: z.number().min(-100).max(100).default(10),
  startHour: z.number().int().min(0).max(23).nullable().default(null),
  endHour: z.number().int().min(0).max(23).nullable().default(null),
