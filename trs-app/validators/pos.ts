...
export const createPosOrderSchema = z
  .object({
    ...
    internalConsumption: internalConsumptionSchema.default({
      saleType: "customer",
      referenceId: null,
      personName: "",
      reason: "",
  ...
  .superRefine((value, context) => {
    ...
    if (isInternal && !value.internalConsumption.personName.trim()) {
      context.addIssue({
        code: "custom",
        path: ["internalConsumption", "personName"],
        message: "Select or enter the person/name for this internal order.",
      });
    }
    if (isInternal && !value.internalConsumption.reason.trim()) {
      context.addIssue({
        code: "custom",
        path: ["internalConsumption", "reason"],
        message: "Reason is required for internal consumption.",
      });
    }
    if (
      value.internalConsumption.saleType === "staff_meal" &&
      !value.internalConsumption.referenceId
    ) {
      context.addIssue({
        code: "custom",
        path: ["internalConsumption", "referenceId"],
    ...
    if (
      ...
      context.addIssue({
        ...
        message: "Add at least two positive payment parts.",
      });
    }
    if (value.waivedAmount > 0 && value.waivedReason.length < 3) {
      context.addIssue({
        code: "custom",
        path: ["waivedReason"],
        message: "Enter why the balance was waived.",
      });
    }
    if (value.tipAmount > 0 && value.tipMethod === "none") {
      context.addIssue({
        code: "custom",
        path: ["tipMethod"],
        message: "Select how the tip was received.",
      });
    }
    ...
    if (value.paymentMethod === "cash" && value.amountTendered < 0) {
      context.addIssue({
        code: "custom",
        path: ["amountTendered"],
        message: "Amount tendered cannot be negative.",
      });
    }
    if (
      value.adjustments.discountType !== "none" &&
      !value.adjustments.discountReason
    ) {
      context.addIssue({
        code: "custom",
        path: ["adjustments", "discountReason"],
