...
export const createKitchenStationSchema = z.object({
  ...
  targetPreparationMinutes: z.number().int().min(1).max(240).default(15),
});
...
export const createRoutingRuleSchema = routingRuleBaseSchema.superRefine(
  (value, ctx) => {
    if (!value.menuItemId && !value.categoryId) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["menuItemId"],
        message: "Either menuItemId or categoryId must be provided.",
      });
    }
  },
);

export const updateRoutingRuleSchema = routingRuleBaseSchema
  .partial()
  .superRefine((value, ctx) => {
    const menuItemWasProvided = Object.prototype.hasOwnProperty.call(
      value,
      "menuItemId",
    );
    ...
    if (
      ...
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["menuItemId"],
        message: "menuItemId and categoryId cannot both be null.",
      });
    }
  });
