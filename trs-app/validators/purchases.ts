  ...
  .superRefine((value, ctx) => {
    ...
    if (value.fulfilmentType === "self_pickup" && !value.pickupPersonId) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["pickupPersonId"],
        message: "Select a pickup person for self pickup.",
      });
    }
    const uniqueItems = new Set(
      value.items.map((item) => item.inventoryItemId),
    );
    if (uniqueItems.size !== value.items.length) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["items"],
        message: "Each inventory item can only be added once.",
      });
    }
    if (value.fulfilmentType === "vendor_delivery" && value.pickupPersonId) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["pickupPersonId"],
        message: "Pickup person is only allowed for self pickup.",
      });
    }
  });
  ...
  .superRefine((value, ctx) => {
    ...
    if (uniqueItems.size !== value.items.length) {
      ctx.addIssue({
        ...
        path: ["items"],
        message:
          "Each purchase-order item can only be received once per receipt.",
      });
    }
