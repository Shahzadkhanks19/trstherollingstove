...
const common = { isActive: z.boolean().default(true) };
...
export const inventoryUnitSchema = z.object({
  ...
  symbol: z
    ...
    .max(20)
    .regex(/^[A-Za-z0-9_-]+$/),
  unitType: z.enum(["weight", "volume", "count", "packaging"]),
  baseUnitSymbol: z.string().trim().max(20).default(""),
...
export const storageLocationSchema = z
  .object({
    ...
    code: z
      ...
      .regex(/^[A-Za-z0-9_-]+$/),
    locationType: z
      .enum(["ambient", "chilled", "frozen", "dry", "packaging", "other"])
      .default("ambient"),
    temperatureMin: z.number().nullable().default(null),
    temperatureMax: z.number().nullable().default(null),
    ...common,
  })
  .refine(
    (value) =>
