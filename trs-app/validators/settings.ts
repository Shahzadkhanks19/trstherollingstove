...
const optionalEmailSchema = z.union([z.email(), z.literal("")]);

export const settingSectionSchema = z.enum(SETTING_SECTIONS);
...
export const orderingSettingsSchema = z
  .object({
    ...
    storeStatus: z.enum(["open", "busy", "closed", "not_accepting_orders"]),
    acceptingOrders: z.boolean(),
    statusMessage: z.string().trim().min(3).max(300),
    delayMessage: z.string().trim().min(3).max(300),
    orderCancellationWindowMinutes: z.number().int().min(0).max(240),
  ...
  .superRefine((value, ctx) => {
    if (value.maximumOrderAmount < value.minimumOrderAmount) {
      ctx.addIssue({
    ...
    if (value.orderingEnabled && !value.dineInEnabled && !value.pickupEnabled) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["orderingEnabled"],
        message: "At least one supported fulfilment type must be enabled.",
      });
    }
  });
...
export const loyaltySettingsSchema = z.object({
  ...
  maximumRedemptionPercent: z.number().min(0).max(100),
  ...
  dailySpinCooldownHours: z.number().int().min(1).max(720),
});
...
export const taxSettingsSchema = z.object({
  ...
  roundOffMode: z.enum(["nearest", "up", "down"]),
});
...
export const paymentSettingsSchema = z
  .object({
    ...
    paymentTimeoutMinutes: z.number().int().min(1).max(1440),
    allowPartialRefunds: z.boolean(),
    automaticRefundsEnabled: z.boolean(),
    refundProcessingDays: z.number().int().min(0).max(90),
  })
  .superRefine((value, ctx) => {
    if (
      !value.cashEnabled &&
    ...
    if (value.razorpayEnabled && !value.onlinePaymentEnabled) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["razorpayEnabled"],
        message: "Online payment must be enabled when Razorpay is enabled.",
      });
    }
  });
...
export const operationsSettingsSchema = z.object({
  ...
  maintenanceMessage: z.string().trim().min(3).max(500),
  ...
  lowStockCheckIntervalMinutes: z.number().int().min(5).max(1440),
  posEnabled: z.boolean(),
  requireOpenPosShift: z.boolean(),
  businessDayClosingHour: z.number().int().min(0).max(23),
});
...
export const seoSettingsSchema = z.object({
  ...
  defaultKeywords: z.array(z.string().trim().min(1).max(80)).max(100),
  canonicalBaseUrl: z.union([z.url(), z.literal("")]),
  googleSiteVerification: z.string().trim().max(250),
  robotsIndexingEnabled: z.boolean(),
  localBusinessSchemaEnabled: z.boolean(),
  googleAnalyticsMeasurementId: z.string().trim().max(50),
