  ...
  .superRefine((value, ctx) => {
    if (value.type === "coupon" && value.couponCode.length < 3) {
      ctx.addIssue({
        ...
        path: ["couponCode"],
        message: "Coupon prizes require a valid coupon code.",
      });
    }
    if (value.type === "try_again" && value.value !== 0) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["value"],
        message: "Try Again prizes must have a zero value.",
      });
    }
  });

export const spinWheelCampaignSchema = z
  .object({
  ...
  .superRefine((value, ctx) => {
    ...
    if (!value.prizes.some((item) => item.isActive))
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["prizes"],
        message: "At least one prize segment must be active.",
      });
  });
...
const spinWheelCampaignIdSchema = z
  .string()
  .regex(/^[a-f\d]{24}$/i, "Invalid campaign identifier.");
