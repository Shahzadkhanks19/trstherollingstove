...
export const scheduledReportScheduleSchema = z
  .object({
    frequency: z.enum([
      "one_time",
      "daily",
  ...
  .superRefine((value, context) => {
    if (value.frequency === "one_time" && !value.runAt) {
      context.addIssue({
        code: "custom",
        path: ["runAt"],
        message: "Run date and time are required for a one-time schedule.",
      });
    }
  });
...
export const scheduledReportUpdateSchema =
  scheduledReportCreateSchema.partial();
export const scheduledReportActionSchema = z.object({
  action: z.enum(["pause", "resume", "run_now", "archive", "restore"]),
});
