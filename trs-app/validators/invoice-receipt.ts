...
const documentBaseSchema = z.object({
  documentType: z.enum(["sales_invoice", "credit_note", "debit_note"]),
  sourceType: z
    .enum(["order", "receivable", "payable", "manual"])
    .default("manual"),
  sourceId: objectId.nullable().optional(),
  customerId: objectId.nullable().optional(),
  partyName: z.string().trim().min(2).max(180),
  partyEmail: z.string().trim().email().or(z.literal("")).default(""),
  partyPhone: z.string().trim().max(30).default(""),
  ...
  roundingAmount: z.coerce.number().min(-10).max(10).default(0),
...
export const financeDocumentCreateSchema = documentBaseSchema.superRefine(
  (value, ctx) => {
    ...
    for (const [index, item] of value.lineItems.entries()) {
      if (item.discountAmount > item.quantity * item.unitPrice)
        ctx.addIssue({
          ...
          message: "Line discount exceeds line subtotal.",
        });
    }
  },
);
export const financeDocumentUpdateSchema = documentBaseSchema
  .partial()
...
export const invoiceReceiptRangeSchema = z.object({
  days: z.coerce.number().int().min(1).max(3650).default(30),
});
export const invoiceReceiptRebuildSchema = invoiceReceiptRangeSchema.extend({
