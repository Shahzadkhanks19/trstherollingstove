...
import { DATA_EXPORT_FORMATS, RESTORE_MODES } from "@/types/dataTransfer";

export const exportQuerySchema = z.object({
  collection: z.string().trim().min(1).max(120),
  format: z.enum(DATA_EXPORT_FORMATS).default("json"),
  limit: z.coerce.number().int().min(1).max(100_000).default(10_000),
});

export const backupQuerySchema = z.object({
  collections: z.string().trim().optional(),
...
export const restorePayloadSchema = z.object({
  mode: z.enum(RESTORE_MODES).default("insert"),
