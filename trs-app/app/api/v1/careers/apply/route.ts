...
export async function POST(request: Request) {
  try {
    ...
    if (!opening)
      throw new AppError("This job opening is unavailable or has closed.", 400);

    const recent = await JobApplication.exists({
      opening: opening._id,
      email: input.email,
      createdAt: { $gte: new Date(Date.now() - 24 * 60 * 60 * 1000) },
    });
    if (recent)
      throw new AppError("You already applied for this role recently.", 409);
    ...
    if (resume instanceof File && resume.size > 0) {
      ...
      if (!extension)
        throw new AppError("Upload a PDF, DOC or DOCX resume.", 400);
      if (resume.size > 5 * 1024 * 1024)
        throw new AppError("Resume size must be 5 MB or less.", 400);
      ...
      resumeOriginalName = resume.name.slice(0, 255);
    }
