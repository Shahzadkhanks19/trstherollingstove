...
export async function PATCH(request: Request) {
  try {
    ...
    device.set({
      ...
      androidVersion:
        body.androidVersion?.trim().slice(0, 40) ?? device.androidVersion,
      manufacturer:
        body.manufacturer?.trim().slice(0, 80) ?? device.manufacturer,
      ...
      printerAddress:
        body.printerAddress?.trim().slice(0, 32) ?? device.printerAddress,
      lastSeenAt: new Date(),
      lastIpAddress:
        request.headers
          .get("x-forwarded-for")
    ...
    return successResponse(
      { id: String(device._id), lastSeenAt: device.lastSeenAt },
      "Print bridge heartbeat recorded.",
    );
  } catch (error) {
    return handleApiError(error);
  }
