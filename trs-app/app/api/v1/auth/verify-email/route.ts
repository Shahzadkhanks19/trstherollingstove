...
export async function POST(r: Request) {
  try {
    const i = await validateRequestBody(r, verifyEmailSchema);
    ...
    if (!t) throw new AppError("Verification token invalid or expired.", 400);
    const u = await User.findById(t.userId);
    if (!u) throw new AppError("User not found.", 404);
    u.emailVerifiedAt = u.emailVerifiedAt ?? new Date();
    t.consumedAt = new Date();
    await Promise.all([u.save(), t.save()]);
    return successResponse(null, "Email verified.");
  } catch (e) {
