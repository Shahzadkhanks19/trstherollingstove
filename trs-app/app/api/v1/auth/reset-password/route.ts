...
export async function POST(r: Request) {
  try {
    const i = await validateRequestBody(r, resetPasswordSchema);
    await connectToDatabase();
    const t = await SecurityToken.findOne({
      type: "password_reset",
      tokenHash: hashOpaqueToken(i.token),
      consumedAt: null,
      expiresAt: { $gt: new Date() },
    }).select("+tokenHash");
    if (!t) throw new AppError("Reset token invalid or expired.", 400);
    const u = await User.findById(t.userId);
    if (!u || !u.isActive) throw new AppError("Account unavailable.", 400);
    u.passwordHash = await hashPassword(i.newPassword);
