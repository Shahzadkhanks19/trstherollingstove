...
export async function PATCH(request: Request, context: Context) {
  try {
    ...
    const record = await POSCartRecord.findOneAndUpdate(
      ...
      { $set: { title, lastSavedAt: new Date() } },
      { new: true },
    ).lean();
    if (!record) throw new AppError("Held order not found.", 404);
    await writeAuditLog({
      actor,
      action: "pos.held_order_renamed",
      module: "pos",
      entityType: "pos_cart_record",
      entityId: String(record._id),
      description: `Held POS order renamed to ${title}.`,
    });
    ...
    return successResponse(
      { id: String(record._id), title: record.title },
      "Held order renamed.",
    );
  } catch (error) {
...
export async function DELETE(_request: Request, context: Context) {
  try {
    ...
    if (!isValidObjectId(id)) throw new AppError("Invalid held order.", 400);
    await connectToDatabase();
    const deleted = await POSCartRecord.findOneAndDelete({
      _id: id,
      ownerId: actor.id,
      status: "held",
    });
    ...
    await writeAuditLog({
      actor,
      action: "pos.held_order_deleted",
      module: "pos",
      entityType: "pos_cart_record",
      entityId: id,
      description: "Held POS order deleted.",
    });
    publishPosCartRecordChanged({
      recordId: id,
      status: "held",
