...
function parseCart(value: unknown) {
  ...
  if (!cart.lines.length)
    throw new AppError("Add at least one item before holding the order.", 422);
  return cart;
}
...
export async function GET() {
  try {
    ...
    const records = await POSCartRecord.find({
      ownerId: actor.id,
      status: "held",
    })
      .sort({ updatedAt: -1 })
...
export async function POST(request: Request) {
  try {
    ...
    const record = await POSCartRecord.create({
      ...
      title:
        title ||
        `Held order · ${totals.itemCount} item${totals.itemCount === 1 ? "" : "s"}`,
      cartSnapshot: cart,
    ...
    await writeAuditLog({
      actor,
      action: "pos.cart_held",
      module: "pos",
      entityType: "pos_cart_record",
      entityId: record.id,
      description: `POS order held by ${actor.name}.`,
      metadata: { itemCount: totals.itemCount, grandTotal: totals.grandTotal },
    });
