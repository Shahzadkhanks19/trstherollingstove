...
export async function PATCH(request: Request, context: Context) {
  try {
    ...
    if (order.saleType !== "customer")
      throw new AppError(
        "Customer details cannot be attached to an internal-consumption order.",
        409,
      );
    ...
    if (selectedCustomerId) {
      if (!Types.ObjectId.isValid(selectedCustomerId))
        throw new AppError("Selected customer is invalid.", 422);
      const selected = await User.findOne({
        ...
        .lean();
      if (!selected)
        throw new AppError("Selected customer is unavailable.", 404);
      identity = {
        id: String(selected._id),
        objectId: new Types.ObjectId(selected._id),
        name: selected.name,
      ...
      if (duplicate) {
        identity = {
          ...
          objectId: new Types.ObjectId(duplicate._id),
          name: duplicate.name,
        ...
        const created = await User.create({
          name,
          phone,
          email: suppliedEmail || `pos-${phone}@customer.trs.local`,
          roleId,
    ...
    await Promise.all([
      ...
      Invoice.updateOne(
        { orderId: order._id },
        { $set: { customerSnapshot: snapshot } },
      ),
      writeAuditLog({
        actor,
        action: "pos.order_customer_attached",
        module: "pos",
        entityType: "order",
        entityId: order.id,
        description: `Customer ${snapshot.name} attached to ${order.orderNumber}.`,
        metadata: { customerId: identity.id },
      }),
    ]);
    return successResponse(
      { customer: { id: identity.id, ...snapshot, isWalkIn: false } },
      "Customer attached to order and invoice.",
    );
  } catch (error) {
