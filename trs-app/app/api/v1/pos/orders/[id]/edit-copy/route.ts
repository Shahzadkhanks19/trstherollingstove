  ...
  try {
    ...
    const cart: PosCartState = {
      ...
      internalConsumption: {
        ...
        referenceId: order.internalConsumption?.referenceId
          ? String(order.internalConsumption.referenceId)
          : null,
      ...
      lines: order.items.map((item) => ({
        ...
        modifierSignature: (item.modifiers ?? [])
          .map((modifier) => `${String(modifier.optionId)}:1`)
          .sort()
          .join("|"),
      })),
          ...
          order.subtotal > 0
            ? Math.round(
                (Number(order.taxTotal ?? 0) /
                  Math.max(
                    1,
                    Number(order.subtotal) - Number(order.discountTotal ?? 0),
                  )) *
                  10000,
              ) / 100
            : 0,
    ...
    return successResponse(
      { cart, orderNumber: order.orderNumber },
      "Order copied to POS for correction.",
    );
  } catch (error) {
