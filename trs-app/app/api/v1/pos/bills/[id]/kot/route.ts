...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
            ...
            const nextParams = new URLSearchParams({
              paper: url.searchParams.get("invoicePaper") === "58mm"
                ...
                : url.searchParams.get("invoicePaper") === "80mm"
                  ? "80mm"
                  : "a4",
              copies: String(Math.min(3, Math.max(1, Number(url.searchParams.get("invoiceCopies") ?? 1)))),
    ...
    const mainItems = invoiceObject.items.filter((item) => !isNaanCounterItem(item));
    ...
    if (!stationItems.length) {
      ...
      fallbackUrl.searchParams.set("station", station === "naan" ? "main" : "naan");
    ...
    const html = renderKotHtml({ ...invoiceObject, items: stationItems }, {
      paper,
      copies,
