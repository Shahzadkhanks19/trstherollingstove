...
export async function GET(request: Request) {
  try {
    ...
    const from =
      indiaDateRange(url.searchParams.get("from") ?? "") ?? defaultFrom;
    const to =
      indiaDateRange(url.searchParams.get("to") ?? "", true) ??
      indiaDateRange(today, true)!;
    ...
    else if (cashierId && isValidObjectId(cashierId))
      filter.openedBy = cashierId;
    ...
    const rows = shifts
      .map((shift) => {
        ...
        const movementsWithBalance = shiftMovements.map((movement) => {
          ...
          const positive =
            movement.type === "cash_in" || movement.type === "cash_sale";
