...
export async function PATCH(request: Request) {
  try {
    ...
    if (input.userIds.includes(actor.id))
      throw new AppError("Bulk actions cannot include your own account.", 400);
    ...
    if (!active && superAdminRole) {
      const [selectedActiveSuperAdmins, totalActiveSuperAdmins] =
        await Promise.all([
          User.countDocuments({
      ...
      if (selectedActiveSuperAdmins >= totalActiveSuperAdmins)
        throw new AppError(
    ...
    if (!active)
      await AuthSession.updateMany(
        { userId: { $in: input.userIds }, revokedAt: null },
        { $set: { revokedAt: now, revokeReason: "Bulk deactivation." } },
      );
    await writeAuditLog({
      actorUserId: actor.id,
      action: active ? "staff.bulk_activated" : "staff.bulk_deactivated",
      entityType: "user",
      description: `${result.modifiedCount} staff accounts ${active ? "activated" : "deactivated"}.`,
      metadata: { userIds: input.userIds, reason: input.reason ?? "" },
    });
