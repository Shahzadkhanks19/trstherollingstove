...
export async function GET(_request: Request, context: Context) {
  try {
    ...
    const order = await PurchaseOrder.findById(id)
      .populate(
        "supplierId",
        "name code contactPerson phone addressLine1 addressLine2 city state postalCode",
      )
      .lean();
    if (!order) throw new AppError("Purchase order not found.", 404);
    const supplier = order.supplierId as unknown as {
      name?: string;
      code?: string;
    ...
    const lines = [
      ...
      ...(order.fulfilmentType === "self_pickup"
        ? [
            `Pickup person: ${order.pickupPersonName || "Not specified"}`,
            `Pickup WhatsApp: ${order.pickupPersonWhatsapp || "Not specified"}`,
          ]
        : []),
      "",
      "ITEMS",
      ...order.items.map(
        (
          ...
          `${index + 1}. ${item.itemName} | SKU: ${item.sku} | Quantity: ${item.orderedQuantity} ${item.unit}`,
      ),
      "",
      `Notes: ${order.notes || "None"}`,
      "",
      "Generated from TRS Purchasing Management",
    ];
    const pdf = createSimpleTextPdf(
      `TRS - ${order.purchaseOrderNumber}`,
      lines,
    );
