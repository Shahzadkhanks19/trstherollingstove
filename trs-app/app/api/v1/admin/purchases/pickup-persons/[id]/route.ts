...
export async function PATCH(request: Request, context: Context) {
  try {
    ...
    const person = await PickupPerson.findByIdAndUpdate(
      id,
...
export async function DELETE(_request: Request, context: Context) {
  try {
    ...
    const usageCount = await PurchaseOrder.countDocuments({
      pickupPersonId: id,
    });
    if (usageCount > 0)
      throw new AppError(
        "This pickup person is used in purchase records and cannot be deleted. Deactivate it instead.",
        409,
      );
