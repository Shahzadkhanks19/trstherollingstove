...
export async function GET() {
  try {
    ...
    const users = await User.find({ roleId })
      .select("name email phone isActive emailVerifiedAt lastLoginAt createdAt")
      .sort({ createdAt: -1 })
      .limit(10000)
      .lean();
    ...
    return new Response(csv, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="trs-customers-${new Date().toISOString().slice(0, 10)}.csv"`,
      },
    });
  } catch (e) {
