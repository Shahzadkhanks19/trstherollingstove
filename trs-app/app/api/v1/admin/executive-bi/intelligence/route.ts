...
export async function GET(request: Request) {
  try {
    ...
    const parsed = executiveBIIntelligenceQuerySchema.safeParse(
      Object.fromEntries(new URL(request.url).searchParams),
    );
    if (!parsed.success)
      throw new AppError(
        parsed.error.issues[0]?.message ??
          "Invalid executive intelligence query.",
        400,
      );
    ...
    if (parsed.data.format === "json")
      return successResponse(report, "Executive intelligence generated.");
