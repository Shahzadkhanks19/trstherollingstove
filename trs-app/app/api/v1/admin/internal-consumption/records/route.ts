...
export async function GET(request: Request) {
  try {
    ...
    const limit = Math.min(
      100,
      Math.max(10, Number(params.get("limit") || 25)),
    );
    ...
    const [records, total, audit] = await Promise.all([
      Order.find(filter)
        .select(
          "orderNumber saleType status createdAt items internalConsumption cashierId",
        )
        .populate("cashierId", "name")
        .populate("internalConsumption.approvedBy", "name email")
        .sort({ createdAt: -1 })
        .skip((page - 1) * limit)
        .limit(limit)
        .lean(),
      ...
      InternalConsumptionAudit.find({})
        .select(
          "action saleType subjectName actorName reason metadata createdAt",
        )
        .sort({ createdAt: -1 })
        .limit(50)
        .lean(),
    ...
    return successResponse(
      ...
      "Internal consumption records loaded.",
    );
  } catch (error) {
