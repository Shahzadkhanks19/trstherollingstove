...
import {
  internalDirectorySchema,
  internalDirectoryTypeSchema,
...
export async function GET(request: Request) {
  try {
    ...
    const records = await InternalConsumptionDirectory.find(filter)
...
export async function POST(request: Request) {
  try {
    ...
    await connectToDatabase();
    if (
      await InternalConsumptionDirectory.exists({
        type: input.type,
        name: input.name,
        deletedAt: null,
      })
    )
      throw new AppError("A matching record already exists.", 409);
    const record = await InternalConsumptionDirectory.create({
      ...input,
      createdBy: actor.id,
      updatedBy: actor.id,
    });
...
export async function PATCH(request: Request) {
  try {
    ...
    await connectToDatabase();
    const before = await InternalConsumptionDirectory.findById(
      payload.id,
    ).lean();
    const record = await InternalConsumptionDirectory.findOneAndUpdate(
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id || !Types.ObjectId.isValid(id))
      throw new AppError("Invalid record.", 422);
    await connectToDatabase();
    const record = await InternalConsumptionDirectory.findByIdAndUpdate(
      id,
      {
        $set: restore
    ...
    if (!record) throw new AppError("Directory record not found.", 404);
    await InternalConsumptionAudit.create({
      action: restore ? "directory_restored" : "directory_archived",
      actorId: actor.id,
      actorName: "",
      subjectId: record._id,
      subjectName: record.name,
