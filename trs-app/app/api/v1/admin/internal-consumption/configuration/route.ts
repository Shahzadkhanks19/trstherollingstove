...
import {
  internalMasterSchema,
  internalPolicySchema,
  internalSettingsSchema,
...
export async function GET(request: Request) {
  try {
    ...
    const [masters, policies, settings, audits] = await Promise.all([
      InternalConsumptionMaster.find(includeDeleted ? {} : { deletedAt: null })
        .sort({ type: 1, sortOrder: 1, name: 1 })
        .lean(),
      InternalConsumptionPolicy.find(includeDeleted ? {} : { deletedAt: null })
      ...
      InternalConsumptionAudit.find({
        action: { $regex: /^(master|policy|settings)_/ },
      })
        .sort({ createdAt: -1 })
        .limit(100)
        .lean(),
    ...
    return successResponse(
      { masters, policies, settings, audits },
      "Internal consumption configuration loaded.",
    );
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    if (payload.action === "master.create") {
      ...
      if (
        await InternalConsumptionMaster.exists({
          type: input.type,
          name: input.name,
          deletedAt: null,
        })
      ...
      const record = await InternalConsumptionMaster.create({
        ...input,
        createdBy: actor.id,
        updatedBy: actor.id,
      });
      await audit(actor, "master_created", record._id, record.name, {
        type: record.type,
      });
    ...
    if (payload.action === "policy.create") {
      ...
      if (
        ...
        throw new AppError("A valid scope is required.", 422);
      const record = await InternalConsumptionPolicy.create({
        ...input,
        scopeId: input.scopeType === "global" ? null : input.scopeId,
        createdBy: actor.id,
        updatedBy: actor.id,
      });
      await audit(actor, "policy_created", record._id, record.name, {
...
export async function PATCH(request: Request) {
  try {
    ...
    if (payload.entity === "settings") {
      ...
      const before = await InternalConsumptionSettings.findOne({
        key: "default",
      }).lean();
      const record = await InternalConsumptionSettings.findOneAndUpdate(
        { key: "default" },
    ...
    if (payload.entity === "master") {
      ...
      const before = await InternalConsumptionMaster.findById(
        payload.id,
      ).lean();
      const record = await InternalConsumptionMaster.findOneAndUpdate(
        { _id: payload.id, deletedAt: null },
        { $set: { ...input, updatedBy: actor.id } },
        { new: true },
      ).lean();
      if (!record) throw new AppError("Master record not found.", 404);
      await audit(actor, "master_updated", record._id, record.name, {
        before,
        after: record,
      });
    ...
    if (payload.entity === "policy") {
      ...
      const before = await InternalConsumptionPolicy.findById(
        payload.id,
      ).lean();
      const record = await InternalConsumptionPolicy.findOneAndUpdate(
        { _id: payload.id, deletedAt: null },
        { $set: { ...input, updatedBy: actor.id } },
        { new: true },
      ).lean();
      if (!record) throw new AppError("Meal policy not found.", 404);
      await audit(actor, "policy_updated", record._id, record.name, {
        before,
        after: record,
      });
...
export async function DELETE(request: Request) {
  try {
    ...
    const update = {
      $set: restore
        ? { deletedAt: null, isActive: true, updatedBy: actor.id }
        : { deletedAt: new Date(), isActive: false, updatedBy: actor.id },
    };
    if (entity === "master") {
      const record = await InternalConsumptionMaster.findByIdAndUpdate(
        id,
        update,
        { new: true },
      ).lean();
      ...
      await audit(
        actor,
        `master_${restore ? "restored" : "deleted"}`,
        record._id,
        record.name,
      );
      return successResponse(
        record,
        restore ? "Record restored." : "Record archived.",
      );
    }
    if (entity === "policy") {
      const record = await InternalConsumptionPolicy.findByIdAndUpdate(
        id,
        update,
        { new: true },
      ).lean();
      ...
      await audit(
        actor,
        `policy_${restore ? "restored" : "deleted"}`,
        record._id,
        record.name,
      );
      return successResponse(
