...
import {
  internalConsumptionAnalyticsToPdf,
  internalConsumptionAnalyticsToXlsx,
} from "@/services/internal-consumption-export.service";
...
export async function GET(request: Request) {
  try {
    ...
    const parsed = internalConsumptionAnalyticsQuerySchema.safeParse(
    ...
    if (parsed.data.format !== "json") {
      await InternalConsumptionAudit.create({
        ...
        reason: `${parsed.data.format} export`,
        metadata: {
      ...
      if (parsed.data.format === "csv")
        return new Response(internalConsumptionAnalyticsToCsv(report), {
          status: 200,
          headers: {
      ...
      if (parsed.data.format === "xlsx")
        return new Response(
          new Uint8Array(await internalConsumptionAnalyticsToXlsx(report)),
          {
            status: 200,
            headers: {
              "Content-Type":
      ...
      return new Response(
        new Uint8Array(await internalConsumptionAnalyticsToPdf(report)),
        {
          status: 200,
          headers: {
