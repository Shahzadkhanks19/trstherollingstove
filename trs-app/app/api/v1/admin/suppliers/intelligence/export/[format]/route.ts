...
export async function GET(request: Request, context: Context) {
  try {
    ...
    if (format !== "xlsx" && format !== "pdf")
      throw new AppError("Supplier export format must be xlsx or pdf.", 400);
    ...
    if (!run)
      throw new AppError(
        "Generate supplier intelligence before exporting.",
        404,
      );
    const snapshots = await SupplierIntelligenceSnapshot.find({
      runId: run._id,
    })
    ...
    const rows = snapshots.map((row) => ({
      ...
      purchaseOrders: row.purchaseOrderCount,
      onTimeDeliveryPercent: row.onTimeDeliveryPercent,
      averageLeadTimeDays: row.averageLeadTimeDays,
      averageDelayDays: row.averageDelayDays,
      fillRatePercent: row.fillRatePercent,
      rejectionRatePercent: row.rejectionRatePercent,
      qualityScore: row.qualityScore,
      purchasePriceVariancePercent: row.purchasePriceVariancePercent,
      returnValue: row.returnValue,
      recommendation: row.recommendation,
    }));
    const title = "Supplier Performance Scorecards & Recommendations";
    const bytes =
      format === "xlsx"
    ...
    await recordInventoryAudit({
      ...
      action: "supplier.intelligence_exported",
      entityType: "SupplierIntelligenceRun",
      entityId: String(run._id),
      description: `Supplier intelligence exported as ${format.toUpperCase()}.`,
      ...
      headers: {
        "content-type":
          format === "xlsx"
            ...
            : "application/pdf",
        "content-disposition": `attachment; filename="supplier-intelligence.${format}"`,
