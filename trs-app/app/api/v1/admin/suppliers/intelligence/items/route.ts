...
export async function GET(request: Request) {
  try {
    ...
    const input = supplierIntelligenceQuerySchema.parse(
    ...
    const filter: Record<string, unknown> = {
      ...
      ...(input.preferredSupplier
        ? { preferredSupplier: input.preferredSupplier === "true" }
        : {}),
      ...(input.isActive ? { isActive: input.isActive === "true" } : {}),
    };
    if (input.search)
      filter.$or = [
        { supplierName: { $regex: input.search, $options: "i" } },
        { supplierCode: { $regex: input.search, $options: "i" } },
      ];
    const skip = (input.page - 1) * input.limit;
    const [rows, total] = await Promise.all([
      SupplierIntelligenceSnapshot.find(filter)
        .sort({ overallScore: -1, totalSpend: -1 })
        .skip(skip)
        .limit(input.limit)
        .lean(),
      SupplierIntelligenceSnapshot.countDocuments(filter),
    ]);
    return successResponse({
      rows,
      pagination: {
