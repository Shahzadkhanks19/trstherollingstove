...
export async function GET(request: Request) {
  try {
    ...
    const sections = [
      ...
      ...snapshot.profitAndLoss.map((item) => [
        item.label,
        item.amount,
        item.debit,
        item.credit,
      ...
      ["BALANCE SHEET"],
      ["Account", "Amount", "Debit", "Credit"],
      ...snapshot.balanceSheet.map((item) => [
        item.label,
        item.amount,
        item.debit,
        item.credit,
      ...
      ["TRIAL BALANCE"],
      ["Account", "Amount", "Debit", "Credit"],
      ...snapshot.trialBalance.map((item) => [
        item.label,
        item.amount,
        item.debit,
        item.credit,
    ...
    return new Response(csv, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="financial-reports-${days}-days.csv"`,
      },
    });
  } catch (error) {
