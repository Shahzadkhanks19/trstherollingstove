...
export async function GET(request: Request) {
  try {
    ...
    const input = budgetForecastQuerySchema.parse({
    ...
    if (!snapshot?.metrics)
      throw new Error("Budget forecast metrics are unavailable.");
    const lines = [
      [
      ...
      ...(snapshot.monthly ?? []).map((item) => [
        item.month,
        item.budgetRevenue,
        item.actualRevenue,
        item.forecastRevenue,
        item.revenueVariance,
        item.budgetExpenses,
        item.actualExpenses,
        item.forecastExpenses,
        item.expenseVariance,
        item.forecastProfit,
        item.forecastCashBalance,
      ]),
    ];
    return new Response(
      lines.map((line) => line.map(csv).join(",")).join("\n"),
      {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="budget-forecast-${input.fiscalYear}-${input.scenario}.csv"`,
        },
      },
    );
  } catch (error) {
