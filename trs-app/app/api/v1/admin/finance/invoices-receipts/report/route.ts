...
export async function GET(request: Request) {
  try {
    ...
    const summary = await getInvoiceReceiptSummary(days);
    const rows = [
      [
        "Document",
        "Type",
      ...
      ...summary.recentDocuments.map((doc) => [
        ...
        doc.documentType,
        doc.partyName,
        new Date(doc.issueDate).toISOString(),
    ...
    return new Response(rows.map((row) => row.map(csv).join(",")).join("\n"), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="invoice-register-${days}-days.csv"`,
      },
    });
  } catch (error) {
