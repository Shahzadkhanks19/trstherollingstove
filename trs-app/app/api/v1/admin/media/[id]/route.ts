...
export async function DELETE(
  ...
  try {
    ...
    const [galleryReference, bannerReference, testimonialReference] =
      await Promise.all([
        GalleryItem.exists({
          $or: [{ mediaUrl: asset.url }, { thumbnailUrl: asset.url }],
        }),
        SiteBanner.exists({
          $or: [{ imageUrl: asset.url }, { mobileImageUrl: asset.url }],
        }),
        Testimonial.exists({ avatarUrl: asset.url }),
      ]);
    if (galleryReference || bannerReference || testimonialReference) {
      throw new AppError(
        "This media file is currently used by published content. Remove or replace those references first.",
        409,
      );
    }
