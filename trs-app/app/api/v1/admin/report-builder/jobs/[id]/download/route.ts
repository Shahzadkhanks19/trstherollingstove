...
export async function GET(
  ...
  try {
    ...
    if (!Types.ObjectId.isValid(id))
      throw new AppError("Invalid report job.", 422);
    await connectToDatabase();
    const job = await ReportJob.findById(id)
      .select("status outputKey outputFilename outputContentType")
      .lean();
    if (!job || job.status !== "completed" || !job.outputKey)
      throw new AppError("Generated report output is unavailable.", 404);
    const artifact = await readReportJobArtifact(job.outputKey);
    const body = Readable.toWeb(
      artifact.stream as Readable,
