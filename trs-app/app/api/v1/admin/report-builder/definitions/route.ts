...
export async function GET(request: Request) {
  try {
    ...
    return successResponse(
      { reports, datasets: getRegistryPayload() },
      "Report builder loaded.",
    );
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    const actor = await requirePermission("reports.read");
    const input = reportDefinitionSchema.parse(await request.json());
    await connectToDatabase();
    ...
    await Promise.all([
      ReportVersion.create({
        reportId: report._id,
        version: 1,
        snapshot: reportSnapshot(report.toObject() as Record<string, unknown>),
        changeSummary: "Initial report version",
        createdBy: actor.id,
      }),
      ReportAudit.create({
        reportId: report._id,
        action: "created",
        actorId: actor.id,
        metadata: { dataset: report.dataset },
      }),
    ]);
