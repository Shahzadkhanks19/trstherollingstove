...
export async function GET(
  ...
  try {
    ...
    if (!(["csv", "xlsx", "pdf"] as string[]).includes(format))
      throw new AppError("Export format must be CSV, XLSX or PDF.", 422);
    ...
    if (!report)
      throw new AppError("Report not found or unavailable for export.", 404);
    ...
    await ReportAudit.create({
      ...
      actorId: actor.id,
      format: typedFormat,
      metadata: { rowCount: result.rowCount, durationMs: result.durationMs },
    });
    const contentType =
      typedFormat === "csv"
    ...
    return new Response(new Uint8Array(bytes), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${reportExportFilename(report.name, typedFormat)}"`,
