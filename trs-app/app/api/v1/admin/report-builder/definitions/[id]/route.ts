...
import {
  createReportVersion,
  reportSnapshot,
} from "@/services/report-version.service";
...
export async function PATCH(
  ...
  try {
    ...
    const { id } = await context.params;
    assertId(id);
    const input = reportDefinitionSchema.partial().parse(await request.json());
    await connectToDatabase();
    const current = await ReportDefinition.findOne({
      _id: id,
      createdBy: actor.id,
    }).lean();
    if (!current)
      throw new AppError("Only the report owner can edit this report.", 403);
    ...
    const report = await ReportDefinition.findByIdAndUpdate(
      id,
      { $set: { ...input, updatedBy: actor.id }, $inc: { version: 1 } },
      { new: true },
    ).lean();
  ...
  try {
    ...
    if (payload.action === "favorite" || payload.action === "pin") {
      ...
      const report = await ReportDefinition.findOneAndUpdate(
        { _id: id, createdBy: actor.id },
        { $set: { [field]: payload.value, updatedBy: actor.id } },
        { new: true },
      ).lean();
      if (!report)
        throw new AppError(
          "Only the report owner can update this setting.",
          403,
        );
      await ReportAudit.create({
        reportId: id,
    ...
    await Promise.all([
      createReportVersion(
        String(report._id),
        actor.id,
        "Initial duplicated version",
      ),
      ReportAudit.create({
        reportId: report._id,
        action: "duplicated",
        actorId: actor.id,
        metadata: { sourceReportId: id },
      }),
    ]);
...
export async function DELETE(
  ...
  try {
    ...
    const report = await ReportDefinition.findOneAndUpdate(
      { _id: id, createdBy: actor.id },
      { $set: { isArchived: !restore, updatedBy: actor.id } },
      { new: true },
    ).lean();
    if (!report)
      throw new AppError(
        "Only the report owner can archive or restore this report.",
        403,
      );
    await ReportAudit.create({
      reportId: id,
    ...
    return successResponse(
      report,
      restore ? "Report restored." : "Report archived.",
    );
  } catch (error) {
    return handleApiError(error);
  }
