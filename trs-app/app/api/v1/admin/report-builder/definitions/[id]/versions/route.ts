...
export async function GET(
  ...
  try {
    ...
    const report = await ReportDefinition.findOne({
      ...
      .select("_id version createdBy")
      .lean();
    if (!report) throw new AppError("Report not found.", 404);
    if (String(report.createdBy) === actor.id)
      await createReportVersion(id, actor.id, "Current report version");
    const versions = await ReportVersion.find({ reportId: id })
      .sort({ version: -1 })
      .limit(100)
      .lean();
    return successResponse(
      { versions, currentVersion: report.version },
      "Report versions loaded.",
    );
  } catch (error) {
...
export async function POST(
  ...
  try {
    ...
    await createReportVersion(
      id,
      actor.id,
      `Before restoring version ${input.version}`,
    );
