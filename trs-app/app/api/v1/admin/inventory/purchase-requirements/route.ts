  ...
  try {
    ...
    const requirements = items.map((item) => {
      ...
      const suggestedQuantity = Math.max(
        0,
        Number((target - item.currentStock).toFixed(3)),
      );
    ...
    return successResponse({
      ...
      summary: {
        ...
        requiringPurchase: requirements.filter(
          (row) => row.suggestedQuantity > 0,
        ).length,
        critical: requirements.filter((row) => row.priority === "critical")
          .length,
        estimatedValue: Number(
          requirements
