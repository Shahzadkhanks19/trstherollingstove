  ...
  try {
    ...
    return successResponse(
      await StockTransfer.find()
        .populate("fromWarehouseId toWarehouseId", "name code")
        .sort({ createdAt: -1 })
        .limit(500)
        .lean(),
    );
...
export async function POST(request: Request) {
  try {
    ...
    const input = await validateRequestBody(request, createStockTransferSchema);
    await connectToDatabase();
    return successResponse(
      await createStockTransfer({ ...input, actorId: actor.id }),
