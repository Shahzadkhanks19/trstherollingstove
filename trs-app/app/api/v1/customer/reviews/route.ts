...
export async function GET(request: Request) {
  try {
    ...
    const orderReference = new URL(request.url).searchParams
      .get("order")
      ?.trim();
    if (orderReference) {
      ...
      if (order.paymentStatus !== "paid")
        throw new AppError(
          "This order does not have a confirmed payment.",
          409,
        );
...
export async function POST(request: Request) {
  try {
    ...
    publishRealtimeEventSafely({
      ...
      target: { roleKeys: ["super_admin", "admin", "manager"] },
    });
    return successResponse(
      { id: String(review._id), status: review.status },
