...
import { customerSelfUpdateSchema } from "@/validators/userManagement";
async function auth() {
...
export async function GET() {
  try {
    ...
    return successResponse(await serializeCustomer(u.id), "Profile loaded.");
  } catch (e) {
...
export async function PATCH(request: Request) {
  try {
    const a = await auth();
    const input = await validateRequestBody(request, customerSelfUpdateSchema);
    await connectToDatabase();
