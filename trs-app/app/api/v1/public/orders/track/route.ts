...
const trackOrderSchema = z.object({
  ...
  phone: z
    ...
    .regex(
      /^[6-9]\d{9}$/,
      "Enter the registered 10-digit Indian mobile number.",
    ),
});
...
export async function POST(request: Request) {
  try {
    ...
    if (!order)
      throw new AppError("Order not found for the supplied details.", 404);

    const estimated = order.estimatedReadyAt
      ? new Date(order.estimatedReadyAt)
      : null;
    const estimatedReadyWindow =
      estimated && !Number.isNaN(estimated.getTime())
