...
export const metadata = createAdminMetadata(
  ...
  "Production readiness and enterprise operations control center.",
);
export const dynamic = "force-dynamic";
export default async function EnterpriseOperationsPage() {
  const user = await getAuthenticatedUser();
  if (!user) redirect("/admin/login?redirect=/admin/enterprise-operations");
  if (!user.permissions.includes("settings.manage"))
    redirect("/admin/login?error=unauthorized");
  return <EnterpriseOperationsDashboardClient />;
}
