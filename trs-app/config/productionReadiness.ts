...
const productionReadinessSchema = z.object({
  ...
  NEXT_PUBLIC_REALTIME_SERVER_URL: z.string().trim().url(),

  ACCESS_TOKEN_SECRET: z.string().trim().min(32),
  REFRESH_TOKEN_SECRET: z.string().trim().min(32),
  ...
  EMAIL_PROVIDER_TOKEN: z.string().trim().optional(),

  WHATSAPP_PROVIDER_ENDPOINT: optionalUrl,
  WHATSAPP_PROVIDER_TOKEN: z.string().trim().optional(),

  CRON_SECRET: z.string().trim().min(24).optional(),
});

export type ProductionReadinessEnvironment = z.infer<
  typeof productionReadinessSchema
>;
...
export function inspectProductionEnvironment(
  ...
  const parsed = productionReadinessSchema.safeParse(source);
  ...
  if (hasAny(paymentFields) && !hasAll(paymentFields)) {
    issues.push({
      field: "razorpay",
      message:
  ...
  if (hasAny(emailFields) && !hasAll(emailFields)) {
    issues.push({
      field: "email",
  ...
  if (hasAny(whatsappFields) && !hasAll(whatsappFields)) {
    issues.push({
      field: "whatsapp",
  ...
  return {
    ...
    issues,
    environment: parsed.success ? parsed.data : null,
  };
}
