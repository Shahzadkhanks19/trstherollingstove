...
export function SecurityPanel() {
  ...
  useEffect(() => {
    void fetch("/api/v1/auth/sessions", { cache: "no-store" })
      .then((r) => r.json())
      .then((b: SessionsResponse) => setSessions(b.data ?? []));
  }, []);
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      >
        ...
        <button
          disabled={saving}
          className="mt-6 rounded-xl bg-[#C8102E] px-6 py-3 font-extrabold text-white disabled:opacity-60"
        >
          {saving ? "Updating…" : "Update password"}
        </button>
        {message ? (
          <p className="mt-4 text-sm font-bold text-black/60">{message}</p>
        ) : null}
      </form>
      <section className="rounded-3xl border border-black/5 bg-white p-6 shadow-sm">
        <h2 className="text-2xl font-black">Active sessions</h2>
        <p className="mt-1 text-sm text-black/50">
          Devices currently signed into your TRS account.
        </p>
            ...
            <div key={s._id} className="py-4">
              <div className="flex items-center justify-between gap-3">
                <p className="font-black">
                  {s.isCurrent ? "This device" : "Signed-in device"}
                </p>
                {s.isCurrent ? (
              ...
              <p className="mt-1 text-xs text-black/40">
                Last used {new Date(s.lastUsedAt).toLocaleString("en-IN")}
              </p>
            </div>
          ))}
        </div>
