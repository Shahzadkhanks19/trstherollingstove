...
export function CustomerOrders() {
  ...
  async function reorder(orderId: string) {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(body.message || "Could not reorder these items.");
      setNotice(
    ...
    <div className="space-y-6">
      <section className="rounded-3xl bg-[#111] p-6 text-white md:p-8">
        ...
        <p className="mt-2 max-w-2xl text-white/65">
          Track preparation, review payment details, download invoices and
          quickly order your favourites again.
        </p>
      </section>
      ...
      {loading ? (
        ...
        <div className="rounded-3xl border border-black/5 bg-white p-10 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-black/50">
            Your TRS orders will appear here after checkout.
          </p>
          <Link
            href="/menu"
            className="mt-5 inline-flex rounded-xl bg-[#C8102E] px-5 py-3 text-sm font-extrabold text-white"
          >
            Browse menu
          </Link>
        </div>
        ...
        <div className="space-y-4">
          {orders.map((order) => (
            <article
              key={order._id}
              className="rounded-3xl border border-black/5 bg-white p-5 shadow-sm md:p-6"
            >
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-extrabold capitalize ${statusClasses(order.status)}`}
                    >
                      {order.status}
                    </span>
                  </div>
                  <p className="mt-2 text-sm text-black/50">
                    ...
                    {order.orderMode === "dine_in" ? "Dine-in" : "Takeaway"}
                  </p>
                </div>
              ...
              <div className="mt-5 grid gap-3 rounded-2xl bg-[#F7F2EC] p-4 sm:grid-cols-3">
                <div>
                  ...
                  <p className="mt-1 font-black">{order.itemCount}</p>
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-black/40">
                    Payment
                ...
                <div>
                  ...
                  <p className="mt-1 flex items-center gap-2 font-black capitalize">
                    <FontAwesomeIcon
                      icon={
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <Link
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-[#111] px-4 py-3 text-sm font-extrabold text-white"
                >
                  View details <FontAwesomeIcon icon={faArrowRight} />
                </Link>
                {order.status === "completed" ? (
                  <button
                    onClick={() => void reorder(order._id)}
                    disabled={reorderingId === order._id}
                    className="inline-flex items-center gap-2 rounded-xl border border-black/10 px-4 py-3 text-sm font-extrabold disabled:opacity-60"
                  >
      ...
      {pages > 1 ? (
        <div className="flex items-center justify-center gap-3">
          <button
            ...
            className="rounded-xl border border-black/10 bg-white px-4 py-2 text-sm font-bold disabled:opacity-40"
          >
            Previous
          </button>
