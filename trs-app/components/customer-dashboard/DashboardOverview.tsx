...
export function DashboardOverview() {
  ...
  useEffect(() => {
    void fetch("/api/v1/customer/dashboard-summary", { cache: "no-store" })
      .then(async (response) => {
        ...
        if (!response.ok)
          throw new Error(body.message || "Could not load dashboard.");
  ...
  const cards = [
    ...
    },
    {
      ...
      value: summary.stats.upcomingReservations,
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-black md:text-4xl">
              Good to see you, {summary.customer.user.name.split(" ")[0]}.
            </h1>
            <p className="mt-3 max-w-2xl text-white/65">
              Track your orders, manage your profile, view rewards and keep
              every TRS experience in one place.
            </p>
          </div>
          <div className="rounded-2xl border border-[#D9A441]/30 bg-[#D9A441]/10 px-6 py-5">
            <p className="text-xs font-bold uppercase tracking-wider text-[#D9A441]">
              Loyalty tier
            </p>
            <p className="mt-1 text-2xl font-black capitalize">
              {summary.stats.loyaltyTier}
            </p>
            <p className="mt-1 text-sm text-white/60">
              Annual spend {money.format(summary.stats.annualSpend)}
            </p>
          </div>
        </div>
      </section>
        ...
        {cards.map((card) => (
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm font-bold text-black/50">{card.label}</p>
        ...
        <div className="mb-5 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-black/50">
              Your latest TRS activity
            </p>
          </div>
          <Link
            href="/customer-dashboard/orders"
            className="text-sm font-extrabold text-[#A50E27]"
          >
            View all
          </Link>
        </div>
          ...
          <div className="divide-y divide-black/5">
            {summary.recentOrders.map((order) => (
              ...
              >
                ...
                <span className="w-fit rounded-full bg-black/5 px-3 py-1 text-xs font-extrabold capitalize">
                  {order.status}
                </span>
                <p className="font-black">{money.format(order.grandTotal)}</p>
              </div>
            ))}
          </div>
        ) : (
          <div className="rounded-2xl bg-[#F7F2EC] p-8 text-center">
            <p className="font-bold">No orders yet.</p>
            <Link
              href="/menu"
              className="mt-3 inline-block font-extrabold text-[#A50E27]"
            >
              Explore the menu
            </Link>
          </div>
