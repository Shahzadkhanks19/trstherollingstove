...
export function SpinWheel() {
    ...
    async function load() {
      ...
      try {
        ...
        if (signedIn) {
          ...
          const body =
            (await response.json()) as ApiResponse<EligibilityPayload>;
  ...
  async function spin() {
    ...
    try {
      ...
      if (!response.ok || !body.success || !body.data)
        throw new Error(body.message || "Unable to spin right now.");
      const selectedIndex = Math.max(
        0,
        prizes.findIndex((prize) => prize.id === body.data?.prize.id),
      );
      const slice = 360 / prizes.length;
      ...
      window.setTimeout(() => {
        ...
        const suffix =
          prize?.type === "coupon" && prize.couponCode
      ...
      setError(
        reason instanceof Error ? reason.message : "Unable to spin right now.",
      );
      setSpinning(false);
    }
  }
  ...
  const disabled =
    loading ||
    spinning ||
    (authenticated && (remainingSpins <= 0 || prizes.length === 0));
  ...
  return (
    <div className="grid h-full gap-5 lg:grid-cols-[.78fr_1.22fr] lg:items-center">
      <div>
        <div className="inline-flex items-center gap-2 rounded-full bg-[#fff1dd] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-[#a45c06]">
          <FontAwesomeIcon icon={faWandMagicSparkles} className="h-3" /> Daily
          reward
        </div>
        <h3 className="mt-3 text-2xl font-black uppercase tracking-[-0.04em] text-[#25211d]">
          {campaign?.name ?? "Daily Spin & Win"}
        </h3>
        <p className="mt-1 text-sm font-extrabold text-[#d71920]">
          {loading
            ? "Loading today’s campaign..."
            : !authenticated
              ? "Sign in to unlock your daily spin."
              : campaign
                ? `${remainingSpins} spin${remainingSpins === 1 ? "" : "s"} remaining today.`
                : "No active campaign right now."}
        </p>
        {campaign?.description ? (
          <p className="mt-3 text-xs font-semibold text-[#62574e]">
            {campaign.description}
          </p>
        ) : null}
        <button
          type="button"
          onClick={authenticated ? spin : signInToSpin}
          disabled={disabled}
          className="mt-5 inline-flex h-11 items-center gap-2 rounded-xl bg-[#d71920] px-5 text-[11px] font-black uppercase text-white shadow-[0_12px_24px_rgba(215,25,32,.22)] transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60"
        >
          <FontAwesomeIcon
            icon={
              !authenticated ? faRightToBracket : spinning ? faRotate : faGift
            }
            className={`h-3.5 ${spinning ? "animate-spin" : ""}`}
          />
          {buttonLabel}
        </button>
        <div className="mt-3 min-h-6 text-xs font-black" aria-live="polite">
          {result ? (
            <span className="text-[#26733b]">You won {result}!</span>
          ) : null}
      ...
      <div className="relative mx-auto aspect-square w-full max-w-[255px]">
        ...
        <div className="absolute inset-0 rounded-full bg-[#efc06e] p-[10px] shadow-[0_18px_42px_rgba(72,42,18,.2)]">
          <div className="h-full rounded-full bg-[#7e4d15] p-[7px]">
            <div
              className="relative h-full rounded-full border-[5px] border-[#f8df9e] transition-transform duration-[4200ms] ease-[cubic-bezier(.12,.7,.08,1)]"
              style={{
            ...
            >
              {prizes.map((prize, index) => {
                ...
                return (
                  <span
                    key={prize.id}
                    className="pointer-events-none absolute left-1/2 top-1/2 flex w-[62px] items-center justify-center text-center text-[8px] font-black uppercase leading-[1.05] text-white drop-shadow-[0_1px_1px_rgba(0,0,0,.45)]"
                    style={{
        ...
        {!authenticated ? (
          <div className="absolute inset-[17px] z-10 grid place-items-center rounded-full bg-black/35 backdrop-blur-[1px]">
            <div className="grid h-20 w-20 place-items-center rounded-full border-4 border-[#f2bd54] bg-[#fff7dd] text-[#34200a] shadow-xl">
        ...
        <button
          ...
          onClick={authenticated ? spin : signInToSpin}
          disabled={disabled}
          className="absolute left-1/2 top-1/2 z-20 grid h-[68px] w-[68px] -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border-[5px] border-[#f2bd54] bg-[#ffd74f] text-[10px] font-black uppercase text-[#34200a] shadow-[0_5px_14px_rgba(42,24,7,.35)] disabled:cursor-not-allowed disabled:opacity-70"
          aria-label={
            authenticated
              ? "Spin the reward wheel"
              : "Sign in to unlock the reward wheel"
          }
        >
          {authenticated ? (
            "Spin"
          ) : (
            <FontAwesomeIcon icon={faLock} className="h-5" />
          )}
        </button>
      </div>
          ...
          >
            ...
            >
              ...
              >
                <FontAwesomeIcon
                  icon={
                    rewardResult.prize.type === "try_again" ? faRotate : faGift
                  }
                  className="h-9"
                />
              </motion.div>
              ...
              <p className="mt-5 text-[11px] font-black uppercase tracking-[0.18em] text-[#d71920]">
                {rewardResult.rewardGranted
                  ? "Reward granted"
                  : "Spin completed"}
              </p>
              <h3
                id="spin-reward-title"
                className="mt-2 text-3xl font-black uppercase tracking-[-0.045em] text-[#25211d]"
              >
                {rewardResult.prize.label}
              </h3>
              <p
                id="spin-reward-description"
                className="mx-auto mt-3 max-w-sm text-sm font-semibold leading-6 text-[#62574e]"
              >
                {rewardResult.rewardMessage}
              </p>

              {rewardResult.prize.type === "coupon" &&
              rewardResult.prize.couponCode ? (
                <div className="mt-5 rounded-2xl border border-dashed border-[#d9b15a] bg-white p-4">
                  <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#8b6a22]">
                    Your coupon code
                  </p>
                  <div className="mt-2 flex items-center justify-center gap-3">
                    <span className="text-xl font-black tracking-[0.12em] text-[#25211d]">
                      {rewardResult.prize.couponCode}
                    </span>
                    <button
                      type="button"
                  ...
                  {copied ? (
                    <p
                      className="mt-2 text-xs font-black text-[#26733b]"
                      aria-live="polite"
                    >
                      Coupon code copied.
                    </p>
