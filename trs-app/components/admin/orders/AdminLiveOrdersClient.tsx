...
const nextAction: Record<
  LiveStatus,
  { status: AdminOrderStatus; label: string }
> = {
...
function elapsedLabel(dateValue: string, now: number) {
  const elapsedMinutes = Math.max(
    0,
    Math.floor((now - new Date(dateValue).getTime()) / 60_000),
  );
...
function formatOrderModifiers(
  ...
  const grouped = new Map<
    string,
    { groupName: string; optionName: string; quantity: number }
  >();
...
function OrderCard({
  ...
  const isLate =
    now - new Date(order.createdAt).getTime() > 20 * 60_000 &&
    order.status !== "ready";
      ...
      <div className="p-4">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              {order.isRunningOrder && (
                <span className="rounded-full bg-violet-50 px-2 py-1 text-[8px] font-black uppercase tracking-wider text-violet-700">
                  Pay Later
                </span>
              )}
            </div>
            <p className="mt-1 text-xs font-semibold text-[#766a61]">
              {order.customerSnapshot.name}
            </p>
          </div>
          <span
            className={`rounded-full px-2.5 py-1 text-[9px] font-black uppercase tracking-wider ${isLate ? "bg-red-50 text-red-700" : "bg-[#f4efe9] text-[#74685f]"}`}
          >
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 text-[11px] font-bold text-[#685e56]">
          <span className="flex items-center gap-2 rounded-xl bg-[#faf7f3] px-3 py-2">
            <FontAwesomeIcon
              icon={order.orderMode === "dine_in" ? faChair : faMotorcycle}
        ...
        {canManage && order.status === "preparing" && (
          <div className="mt-3 flex gap-2">
            <button
              type="button"
              onClick={() => onAddTime(order, 5)}
              className="rounded-xl bg-amber-50 px-3 py-2 text-[10px] font-black text-amber-800"
            >
              +5 min
            </button>
            <button
              type="button"
              onClick={() => onAddTime(order, 10)}
              className="rounded-xl bg-amber-50 px-3 py-2 text-[10px] font-black text-amber-800"
            >
              +10 min
            </button>
            {order.estimatedReadyAt && (
              <span className="rounded-xl bg-slate-100 px-3 py-2 text-[10px] font-black">
                ETA {timeFormatter.format(new Date(order.estimatedReadyAt))}
              </span>
            )}
          </div>
        )}
          ...
          {expanded && (
            ...
            >
              <div className="mt-3 space-y-2 border-t border-[#eee4dc] pt-3">
                <div className="rounded-xl bg-[#f8f4ef] p-3 text-[10px] font-bold text-[#5f554d]">
                  ...
                  {order.orderTakerName && (
                    <p>Order taker: {order.orderTakerName}</p>
                  )}
                  <p>
                    Payment: {order.paymentMethod} · {order.paymentStatus}
                  </p>
                  {(order.waivedAmount ?? 0) > 0 && (
                    <p>
                      Waived: {money.format(order.waivedAmount ?? 0)} ·{" "}
                      {order.waivedReason}
                    </p>
                  )}
                  {(order.tipAmount ?? 0) > 0 && (
                    <p>
                      Tip: {money.format(order.tipAmount ?? 0)} via{" "}
                      {order.tipMethod}
                    </p>
                  )}
                  {order.paymentBreakdown?.map((part, index) => (
                    <p key={index}>
                      {part.method.toUpperCase()}: {money.format(part.amount)}
                  ...
                  >
                    <div>
                      ...
                      {item.specialInstructions && (
                        <p className="mt-1 text-[10px] font-bold text-[#C8102E]">
                          Note: {item.specialInstructions}
                        </p>
                      )}
                    </div>
                    <span className="shrink-0 font-black text-[#173044]">
          ...
          {order.status === "placed" && (
            <div className="mt-2 grid grid-cols-2 gap-2">
              <button
                type="button"
                disabled={acting}
                onClick={() => onCancel(order, "rejected")}
                className="min-h-10 rounded-xl border border-[#e5d9cf] bg-white text-[11px] font-black text-[#173044] disabled:opacity-50"
              >
                Reject
              </button>
              <button
                type="button"
                disabled={acting}
                onClick={() => onCancel(order, "cancelled")}
                className="min-h-10 rounded-xl border border-red-200 bg-red-50 text-[11px] font-black text-red-700 disabled:opacity-50"
              >
                Cancel
              </button>
            </div>
...
export function AdminLiveOrdersClient({ canManage }: { canManage: boolean }) {
  ...
  const loadOrders = useCallback(async (background = false) => {
    ...
    try {
      ...
      if (!response.ok || !payload.success)
        throw new Error(payload.message || "Unable to load live orders.");
  ...
  async function updateStatus(
    ...
    try {
      ...
      setOrders((current) =>
        current
          .map((item) => (item._id === order._id ? payload.data : item))
          .filter((item) => LIVE_STATUSES.includes(item.status as LiveStatus)),
      );
  ...
  async function addPreparationTime(order: AdminOrder, minutes: number) {
    ...
    await updateStatus(
      order,
      order.status,
      `Preparation time extended by ${minutes} minutes.`,
  ...
  const takeawayCount = orders.filter(
    (order) => order.orderMode === "takeaway",
  ).length;
    ...
    <div>
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          <div
            key={item.label}
            className="rounded-[20px] border border-[#e7dcd2] bg-[#fffdf9] p-4 shadow-[0_8px_24px_rgba(30,35,40,.045)]"
          >
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[9px] font-black uppercase tracking-[.16em] text-[#83776e]">
                  {item.label}
                </p>
                <p className="mt-2 text-2xl font-black text-[#173044]">
                  {item.value}
                </p>
              </div>
              <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#fff0e8] text-[#C8102E]">
      ...
      {(error || actionError) && (
        <div
          role="alert"
          className="mb-5 flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700"
        >
      ...
      <div className="mb-3 flex items-center justify-between text-[10px] font-bold text-[#84786f]">
        <span>
          {canManage
            ...
            : "You have view-only access to live orders."}
        </span>
        <span>
          {lastUpdatedAt
            ? `Updated ${timeFormatter.format(lastUpdatedAt)}`
            : "Not updated yet"}
        </span>
      </div>
      ...
      {loading ? (
        <div className="grid min-h-72 place-items-center rounded-[24px] border border-[#e7dcd2] bg-[#fffdf9]">
          <div className="text-center">
            <FontAwesomeIcon
              icon={faRotate}
              className="animate-spin text-2xl text-[#C8102E]"
            />
            <p className="mt-3 text-sm font-black text-[#173044]">
              Loading live orders…
            </p>
          </div>
        </div>
        ...
        <div className="grid items-start gap-4 md:grid-cols-2 min-[1800px]:grid-cols-4">
          {columns.map((column) => (
            <section
              key={column.status}
              className="overflow-hidden rounded-[24px] border border-[#e7dcd2] bg-[#f8f4ef]"
            >
              <header className="border-b border-[#e7dcd2] bg-[#fffdf9] p-4">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <span
                      className={`grid h-10 w-10 place-items-center rounded-xl ring-1 ${column.tone}`}
                    >
                      <FontAwesomeIcon icon={column.icon} />
                    </span>
                    <div>
                      <h2 className="text-sm font-black text-[#173044]">
                        {column.title}
                      </h2>
                      <p className="mt-0.5 text-[10px] font-semibold text-[#81756c]">
                        {column.description}
                  ...
                  <span className="grid h-8 min-w-8 place-items-center rounded-full bg-[#173044] px-2 text-xs font-black text-white">
                  ...
                  {groupedOrders[column.status].map((order) => (
                    <OrderCard
                      ...
                      onAdvance={(selected) =>
                        void updateStatus(
                          selected,
                          nextAction[selected.status as LiveStatus].status,
                        )
                      }
                      onCancel={(selected, status) =>
                        setCancelAction({ order: selected, status })
                      }
                      onAddTime={(selected, minutes) =>
                        void addPreparationTime(selected, minutes)
                      }
                    />
                  ))}
                </AnimatePresence>
                {groupedOrders[column.status].length === 0 && (
                  <div className="grid min-h-40 place-items-center rounded-2xl border border-dashed border-[#d9ccc1] bg-white/60 px-4 text-center">
                    <div>
                      <FontAwesomeIcon
                        icon={column.icon}
                        className="text-xl text-[#b4a79d]"
                      />
                      <p className="mt-2 text-xs font-black text-[#766a61]">
      ...
      <CustomActionModal
        ...
        confirmLabel={
          cancelAction?.status === "rejected" ? "Reject order" : "Cancel order"
        }
        tone="danger"
        loading={Boolean(
          cancelAction && actingOrderId === cancelAction.order._id,
        )}
        inputLabel="Reason"
        inputPlaceholder="Enter a clear reason"
        inputRequired
        onClose={() => {
          if (!actingOrderId) setCancelAction(null);
        }}
