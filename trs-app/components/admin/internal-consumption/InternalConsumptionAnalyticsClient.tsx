...
function MetricCard({
  ...
  return (
    <article
      className={`rounded-[24px] border p-5 ${strong ? "border-[#173044] bg-[#173044] text-white" : "bg-white text-[#173044]"}`}
    >
      <p
        className={`text-xs font-black uppercase tracking-widest ${strong ? "text-white/60" : "text-slate-400"}`}
      >
        {label}
      </p>
      <p className="mt-3 text-2xl font-black">{value}</p>
      <p
        className={`mt-1 text-xs font-bold ${strong ? "text-[#E8A53A]" : "text-slate-500"}`}
      >
        {detail}
      </p>
    </article>
...
export function InternalConsumptionAnalyticsClient() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/v1/admin/internal-consumption/analytics?${query}`,
        { cache: "no-store" },
      );
  ...
  const maxDailyValue = Math.max(
    0,
    ...(report?.dailyTrend.map((row) => row.menuValue) ?? []),
  );
  const maxTypeValue = Math.max(
    0,
    ...(report?.byType.map((row) => row.menuValue) ?? []),
  );
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            {(["csv", "xlsx", "pdf"] as const).map((format) => (
              <a
                key={format}
                href={`/api/v1/admin/internal-consumption/analytics?${query}&format=${format}`}
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
            <MetricCard
              ...
              value={currency(report.totals.menuValue)}
              detail="Excluded from sales revenue"
            />
            <MetricCard
              label="Internal orders"
              value={String(report.totals.orders)}
              detail={`${report.totals.items} total items`}
            />
            <MetricCard
              label="Average value"
              value={currency(report.totals.averageOrderValue)}
              detail="Per internal order"
            />
            <MetricCard
              label="People served"
              value={String(report.totals.uniquePeople)}
              detail="Unique recorded names"
            />
            <MetricCard
              label="Inventory cost"
              value={currency(report.totals.inventoryCost)}
              detail={`${report.totals.costCoveragePercent}% of menu value`}
            />
          </div>
          ...
          <div className="grid gap-5 xl:grid-cols-2">
            <SectionCard>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Menu value assigned to each internal order type.
              </p>
              <div className="mt-5 space-y-4">
                {report.byType.length ? (
                  report.byType.map((row) => (
                    <div key={row.saleType}>
                      <div className="mb-2 flex items-center justify-between gap-3">
                        ...
                        <p className="text-sm font-black text-[#C8102E]">
                          {currency(row.menuValue)}
                        </p>
                      </div>
                      <Bar value={row.menuValue} max={maxTypeValue} />
                    </div>
                  ))
                ) : (
                  <p className="py-8 text-center text-sm font-bold text-slate-400">
                    No internal orders in this period.
                  </p>
                )}
              </div>
            </SectionCard>
            <SectionCard>
              <h2 className="text-lg font-black text-[#173044]">Daily trend</h2>
              <p className="mt-1 text-sm text-slate-500">
                Operational consumption over the selected range.
              </p>
                ...
                {report.dailyTrend.length ? (
                  report.dailyTrend.map((row) => (
                    ...
                    >
                      ...
                      <Bar value={row.menuValue} max={maxDailyValue} />
          ...
          <div className="grid gap-5 xl:grid-cols-3">
            <SectionCard>
              ...
              <div className="mt-4 divide-y">
                {report.topPeople.map((row, index) => (
                  <div
                    key={`${row.saleType}-${row.name}`}
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500">
                        {labels[row.saleType]} · {row.orders} orders
                      </p>
                    </div>
                    <p className="shrink-0 text-xs font-black text-[#C8102E]">
                      {currency(row.menuValue)}
                    </p>
                  </div>
                ))}
                {!report.topPeople.length ? (
                  <p className="py-8 text-center text-sm font-bold text-slate-400">
                    No people data.
                  </p>
                ) : null}
                ...
                {report.topItems.map((row, index) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500">
                        {row.variantName || "Standard"} · Qty {row.quantity}
                      </p>
                    </div>
                    <p className="shrink-0 text-xs font-black text-[#C8102E]">
                      {currency(row.menuValue)}
            ...
            <SectionCard>
              ...
              <div className="mt-4 divide-y">
                {report.topReasons.map((row, index) => (
                  <div
                    key={`${row.saleType}-${row.reason}`}
                    className="flex items-center justify-between gap-3 py-3"
                  >
                    <div className="min-w-0">
                      <p className="truncate text-sm font-black text-[#173044]">
