...
function CreateOrderDrawer({
  ...
  const [fulfilmentType, setFulfilmentType] = useState<
    "vendor_delivery" | "self_pickup"
  >("vendor_delivery");
  const [pickupPersonId, setPickupPersonId] = useState("");
  const [notes, setNotes] = useState("");
  const [lines, setLines] = useState<DraftLine[]>([
    { inventoryItemId: inventory[0]?._id ?? "", orderedQuantity: "1" },
  ]);
  ...
  function focusFirst(next: Record<string, string>, form: HTMLFormElement) {
    const key = Object.keys(next)[0];
    if (key) form.querySelector<HTMLElement>(`[name="${key}"]`)?.focus();
  }
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (fulfilmentType === "self_pickup" && !pickupPersonId)
      next.pickupPersonId = "Select a pickup person.";
    lines.forEach((line, index) => {
      if (!line.inventoryItemId) next[`item-${index}`] = "Select an item.";
      if (
        !Number.isFinite(Number(line.orderedQuantity)) ||
        Number(line.orderedQuantity) <= 0
      )
        next[`quantity-${index}`] = "Enter a quantity greater than zero.";
    });
    if (notes.trim().length > 1500)
      next.notes = "Notes cannot exceed 1500 characters.";
    setErrors(next);
    if (Object.keys(next).length) return focusFirst(next, event.currentTarget);
    ...
    setSaving(true);
    try {
      const response = await fetch("/api/v1/admin/purchases/orders", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          supplierId,
          ...
          fulfilmentType,
          pickupPersonId:
            fulfilmentType === "self_pickup" ? pickupPersonId : null,
          items,
          notes: notes.trim(),
        }),
      });
      ...
      if (!response.ok)
        throw new Error(payload.message || "Unable to create order request.");
      await onCreated();
    } catch (caught) {
      setErrors({
        form:
  ...
  return (
    <Drawer title="New order request" eyebrow="Purchasing" onClose={onClose}>
      <form
        onSubmit={submit}
        noValidate
      ...
      >
        <div className="flex-1 space-y-5 overflow-y-auto p-4 sm:p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Expected date" error={errors.expectedDeliveryDate}>
              <input
            ...
            <Field label="Fulfilment type *">
              <select
                ...
                onChange={(event) => {
                  ...
                  if (value === "vendor_delivery") setPickupPersonId("");
                }}
                className={inputClass}
              >
            ...
            {fulfilmentType === "self_pickup" && (
              <Field label="Pickup person *" error={errors.pickupPersonId}>
                <select
                  name="pickupPersonId"
                  value={pickupPersonId}
                  onChange={(event) => setPickupPersonId(event.target.value)}
                  className={inputClass}
                >
                  ...
                  {pickupPeople
                    ...
                    .map((person) => (
                      <option key={person._id} value={person._id}>
                        {person.name} · {person.whatsappNumber}
                      </option>
                    ))}
                </select>
              </Field>
          ...
          <section>
            <div className="flex items-center justify-between gap-3">
              <div>
                <h3 className="text-xs font-black uppercase tracking-wider text-[#173044]">
                  Items to order
                </h3>
                <p className="mt-1 text-xs text-[#82766d]">
                  Select the item and required quantity only.
                </p>
              </div>
              <button
                type="button"
                ...
                className="shrink-0 rounded-xl border border-[#173044] px-3 py-2 text-[10px] font-black text-[#173044]"
              >
            ...
            <div className="mt-3 space-y-3">
              {lines.map((line, index) => {
                const selected = inventory.find(
                  (item) => item._id === line.inventoryItemId,
                );
                return (
                  <div
                  ...
                  >
                    <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_160px_auto]">
                      ...
                      <button
                        ...
                        className="self-end rounded-xl border border-red-200 px-3 py-3 text-[10px] font-black text-red-700 disabled:opacity-40"
                      >
                        Remove
                      </button>
                    </div>
                    {selected && (
                      <p className="mt-2 text-[10px] font-semibold text-[#867970]">
...
function OrderDrawer({
  ...
  canManage: boolean;
  onClose: () => void;
  onApprove: (order: PurchaseOrder) => void | Promise<void>;
  ...
  return (
    <Drawer
      ...
      eyebrow="Order request details"
      onClose={onClose}
    >
      <div className="flex-1 space-y-5 overflow-y-auto p-4 sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#e8ddd3] bg-[#fffaf6] p-4">
          <div>
            <p className="text-xs font-black text-[#173044]">
              {order.supplierId?.name}
            </p>
            <p className="mt-1 text-[10px] text-[#82766d]">
              {order.supplierId?.code} ·{" "}
              {order.supplierId?.phone || "No WhatsApp number"}
            </p>
          </div>
          <PurchaseStatus value={order.status} />
        </div>
          ...
          <div className="mt-3 space-y-3">
            {order.items.map((item) => (
              ...
              >
                ...
                <p className="mt-1 text-[10px] text-[#81756c]">{item.sku}</p>
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <Info
                    label="Ordered"
                    value={`${item.orderedQuantity} ${item.unit}`}
                  />
                  <Info
                    label="Received"
        ...
        {order.pickupPersonName && (
          <section>
            ...
            <p className="mt-2 rounded-2xl border border-[#e8ddd3] bg-white p-4 text-sm text-[#655b54]">
              {order.pickupPersonName}
            </p>
          </section>
        )}
        {order.whatsappDeliveries?.length ? (
          <section>
            <h3 className="text-xs font-black uppercase tracking-wider text-[#173044]">
              WhatsApp delivery
            </h3>
            <div className="mt-2 space-y-2">
              {order.whatsappDeliveries.map((delivery, index) => (
                <div
                  key={`${delivery.recipientType}-${index}`}
                  className="rounded-xl border border-[#e8ddd3] bg-white p-3 text-xs"
                >
                  <b className="capitalize text-[#173044]">
                    {delivery.recipientType.replaceAll("_", " ")}:{" "}
                    {delivery.status}
                  </b>
                  <p className="mt-1 text-[#81756c]">
                    {delivery.destination || "No number configured"}
                  </p>
        ...
        {order.notes && (
          <section>
            ...
            <p className="mt-2 rounded-2xl border border-[#e8ddd3] bg-white p-4 text-sm leading-6 text-[#655b54]">
              {order.notes}
            </p>
          </section>
        )}
        {order.cancellationReason && (
          <section>
            ...
            <p className="mt-2 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
              {order.cancellationReason}
            </p>
          </section>
        )}
      </div>
      <div className="flex flex-col-reverse gap-2 border-t border-[#eee4dc] bg-white p-4 sm:flex-row sm:flex-wrap sm:justify-end">
        <a
          href={`/api/v1/admin/purchases/orders/${order._id}/pdf`}
          className="inline-flex min-h-11 items-center justify-center rounded-xl border border-[#173044] px-5 text-xs font-black text-[#173044]"
        >
        ...
        {canManage && (
          <button
            ...
            className="min-h-11 rounded-xl border border-red-200 px-5 text-xs font-black text-red-700"
          >
          ...
          <>
            ...
            {order.status === "draft" && (
              <button
                ...
                className="min-h-11 rounded-xl bg-[#173044] px-5 text-xs font-black text-white"
              >
