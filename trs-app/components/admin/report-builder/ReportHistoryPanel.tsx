...
export function ReportHistoryPanel({
  ...
  const load = useCallback(async () => {
    const [versionsResponse, executionsResponse] = await Promise.all([
      fetch(`/api/v1/admin/report-builder/definitions/${reportId}/versions`, {
        cache: "no-store",
      }),
      fetch(
        `/api/v1/admin/report-builder/definitions/${reportId}/executions?limit=20`,
        { cache: "no-store" },
      ),
    ]);
    const versionsBody = (await versionsResponse.json()) as ApiResponse<{
      versions: Version[];
    }>;
    const executionsBody = (await executionsResponse.json()) as ApiResponse<{
  ...
  async function restore(version: number) {
    ...
    try {
      const response = await fetch(
        `/api/v1/admin/report-builder/definitions/${reportId}/versions`,
        {
          method: "POST",
      ...
      setError(
        reason instanceof Error ? reason.message : "Unable to restore version.",
      );
    } finally {
      setBusyVersion(null);
    }
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <div>
        ...
        <div className="max-h-72 space-y-2 overflow-auto pr-1">
          ...
          {!versions.length ? (
            <p className="text-sm text-black/50">No versions recorded yet.</p>
          ) : null}
        </div>
      </div>
      <div>
        ...
        <div className="max-h-72 space-y-2 overflow-auto pr-1">
          {executions.map((execution) => (
            ...
            >
              <div className="flex items-center justify-between">
                <b
                  className={`text-sm ${execution.status === "failed" ? "text-red-700" : "text-emerald-700"}`}
                >
                  {execution.status}
                </b>
          ...
          {!executions.length ? (
            <p className="text-sm text-black/50">
              Run a saved report to create execution history.
            </p>
          ) : null}
        </div>
      </div>
      {error ? (
        <p
          role="alert"
          className="text-sm font-bold text-red-700 lg:col-span-2"
        >
          {error}
        </p>
      ) : null}
