      ...
      <section className="min-w-0 overflow-hidden rounded-[20px] sm:rounded-[24px] border border-[#e8ddd3] bg-[#fffdf9] shadow-[0_10px_32px_rgba(30,35,40,.05)]">
              ...
              <tr>
                <th className="px-5 py-4">
                  <input
                    ...
                    onChange={() =>
                      setSelected(
                        allSelected ? [] : items.map((item) => item._id),
                      )
                    }
                  />
                </th>
                ...
                <th className="px-3 py-4">Merchandising</th>
                    ...
                    >
                      ...
                      <td className="px-3 py-4">
                        <div className="flex items-center gap-3">
                          <ItemImage item={item} />
                          <div>
                            <p className="max-w-[220px] truncate text-sm font-black text-[#122b3c]">
                              {item.name}
                            </p>
                            <p className="mt-1 text-[10px] font-bold text-[#94877d]">
                              {item.preparationTimeMinutes} min · Sort{" "}
                              {item.sortOrder}
                            </p>
                          </div>
                        </div>
                      </td>
                      <td className="px-3 py-4 text-xs font-bold text-[#62584f]">
                        {typeof item.categoryId === "string"
                          ? "—"
                        ...
                        {item.compareAtPrice != null &&
                          item.compareAtPrice > item.basePrice && (
                            <div className="flex items-center gap-2">
                              <p className="text-xs font-bold text-[#9b8e85] line-through">
                      ...
                      <td className="px-3 py-4">
                        <div className="flex gap-1">
                          <MiniBadge
                            active={item.availableForDineIn}
                            label="Dine-in"
                          />
                          <MiniBadge
                            active={item.availableForTakeaway}
                            label="Pickup"
                          />
                        </div>
                      </td>
                      <td className="px-3 py-4">
                        <button
                          disabled={!canUpdate || acting}
                          onClick={() =>
                            void patchItem(item, {
                              isAvailable: !item.isAvailable,
                            })
                          }
                          className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[9px] font-black uppercase tracking-wider ${item.isAvailable ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}
                        >
                          <FontAwesomeIcon
                            icon={item.isAvailable ? faEye : faEyeSlash}
                          />
                      ...
                      <td className="px-3 py-4">
                        <div className="flex gap-1">
                          {item.isFeatured && (
                            <IconBadge icon={faStar} label="Featured" />
                          )}
                          {item.isBestseller && (
                            <IconBadge icon={faFire} label="Bestseller" />
                          )}
                          {item.isTodaysSpecialOffer && (
                            <IconBadge
                              icon={faFire}
                              label="Today's Special Offer"
                            />
                          )}
                          {item.trackInventory && (
                            <IconBadge icon={faBoxOpen} label="Tracked" />
                          )}
                        </div>
                      </td>
                      <td className="px-5 py-4">
        ...
        <div className="divide-y divide-[#eee4dc] xl:hidden">
          {loading
            ? Array.from({ length: 5 }).map((_, index) => (
                <div
                  key={index}
                  className="m-4 h-32 animate-pulse rounded-2xl bg-[#f1ebe5]"
                />
              ))
            : items.map((item) => (
                <article key={item._id} className="min-w-0 p-3 sm:p-4">
                  <div className="flex min-w-0 gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:justify-between">
                        <div>
                        ...
                        <div className="shrink-0 text-right">
                          {item.compareAtPrice != null &&
                            item.compareAtPrice > item.basePrice && (
                              <p className="text-[10px] font-bold text-[#9b8e85] line-through">
                                {money.format(item.compareAtPrice)}
                              </p>
                            )}
                  ...
                  <div className="mt-4 flex flex-col gap-3 border-t sm:flex-row sm:items-center sm:justify-between border-[#eee4dc] pt-3">
                    <label className="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider text-[#7d7068]">
                      <input
                        type="checkbox"
                        checked={selected.includes(item._id)}
                        onChange={() =>
        ...
        {!loading && !items.length && (
          <div className="grid place-items-center px-6 py-20 text-center">
            <span className="grid h-14 w-14 place-items-center rounded-2xl bg-[#fff0e8] text-[#C8102E]">
              <FontAwesomeIcon icon={faUtensils} />
            </span>
            <h3 className="mt-4 text-lg font-black text-[#122b3c]">
              No menu items found
            </h3>
            <p className="mt-2 max-w-sm text-sm font-medium text-[#7d7068]">
              Adjust the filters or add the first item to this catalog.
            </p>
          </div>
        )}
...
function ItemImage({ item }: { item: MenuItem }) {
  return item.imageUrl ? (
    <img
      src={item.imageUrl}
      alt=""
      className="h-12 w-12 shrink-0 rounded-2xl object-cover ring-1 ring-[#e8ddd3]"
    />
  ) : (
    <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#fff0e8] text-[#C8102E]">
      <FontAwesomeIcon icon={faUtensils} />
    </span>
  );
}
function MiniBadge({ active, label }: { active: boolean; label: string }) {
  return (
    <span
...
function IconBadge({ icon, label }: { icon: typeof faStar; label: string }) {
  return (
    <span
      title={label}
      className="grid h-7 w-7 place-items-center rounded-full bg-amber-50 text-amber-700"
    >
...
function ActionButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className={`grid h-10 flex-1 place-items-center rounded-xl border transition sm:h-9 sm:w-9 sm:flex-none ${danger ? "border-red-100 text-red-600 hover:bg-red-50" : "border-[#e5d9cf] text-[#122b3c] hover:bg-[#f8f1eb]"}`}
    >
...
function BulkButton({
  ...
  return (
    <button
      onClick={onClick}
      className="shrink-0 rounded-xl border border-[#e5d9cf] bg-white px-3 py-2 text-[10px] font-black text-[#122b3c]"
    >
      {label}
    </button>
  );
  ...
  return (
    <label>
      ...
      <select
        ...
        className="h-11 w-full rounded-2xl border border-[#e5d9cf] bg-white px-3 text-xs font-bold outline-none focus:border-[#C8102E]"
      >
        {options.map((option) => (
  ...
  return (
    <tr>
      <td colSpan={8} className="px-5 py-3">
        <div className="h-14 animate-pulse rounded-2xl bg-[#f1ebe5]" />
      </td>
    </tr>
  );
}
