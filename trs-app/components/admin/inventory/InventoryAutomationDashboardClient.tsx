...
export function InventoryAutomationDashboardClient({
  canManage,
}: {
  canManage: boolean;
}) {
  ...
  const [topItems, setTopItems] = useState<{
    mostConsumed: TopItem[];
    highestValue: TopItem[];
  }>({ mostConsumed: [], highestValue: [] });
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const [summaryData, trendData, topData, jobData, scheduleData] =
        await Promise.all([
          request<Summary>("/api/v1/admin/inventory/dashboard/summary"),
          request<{ rows: TrendRow[] }>(
            "/api/v1/admin/inventory/dashboard/trends?days=30",
          ),
          request<{ mostConsumed: TopItem[]; highestValue: TopItem[] }>(
            "/api/v1/admin/inventory/dashboard/top-items?limit=8",
          ),
          request<{ jobs: AutomationJob[] }>(
            "/api/v1/admin/inventory/automation/jobs?limit=12",
          ),
          request<{ schedules: ScheduledReport[] }>(
            "/api/v1/admin/inventory/scheduled-reports",
          ),
        ]);
    ...
    return Math.max(
      ...
      ...trends.flatMap((row) =>
        Object.entries(row)
          .filter(
            ([key, value]) =>
              key.endsWith("Quantity") && typeof value === "number",
          )
  ...
  const runAlertScan = async () => {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "Unable to run alert scan.",
      );
    } finally {
      setWorking(false);
    }
  ...
  const createSchedule = async () => {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "Unable to create schedule.",
      );
    } finally {
      setWorking(false);
    }
  ...
  const deleteSchedule = async (id: string) => {
    ...
    setError("");
    try {
      await request(`/api/v1/admin/inventory/scheduled-reports/${id}`, {
        method: "DELETE",
      });
      ...
      setError(
        caught instanceof Error ? caught.message : "Unable to delete schedule.",
      );
    } finally {
      setWorking(false);
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-3xl bg-neutral-950 p-6 text-white md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-neutral-300">
            Live KPIs, movement trends, alert scans, job history and scheduled
            report delivery.
          </p>
        </div>
        {canManage ? (
          <button
            type="button"
            disabled={working}
            onClick={() => void runAlertScan()}
            className="rounded-2xl bg-red-600 px-5 py-3 text-sm font-bold hover:bg-red-500 disabled:opacity-50"
          >
            Run alert scan
          </button>
        ) : null}
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-3xl border border-neutral-200 bg-white p-6">
          <h2 className="text-xl font-black">30-day stock movement trend</h2>
          <div className="mt-5 space-y-3">
            {trends.slice(-14).map((row) => {
              ...
              return (
                <div
                  key={row.date}
                  className="grid grid-cols-[90px_1fr_70px] items-center gap-3 text-xs"
                >
                  <span>{row.date}</span>
                  <div className="h-3 overflow-hidden rounded-full bg-neutral-100">
                    <div
                      ...
                      style={{
                        width: `${Math.max(2, (total / maxTrend) * 100)}%`,
                      }}
                    />
                  </div>
            ...
            {trends.length === 0 ? (
              <p className="text-sm text-neutral-500">No movement data yet.</p>
            ) : null}
          </div>
        </section>
        <section className="rounded-3xl border border-neutral-200 bg-white p-6">
          <h2 className="text-xl font-black">Alert status</h2>
          <div className="mt-5 grid grid-cols-3 gap-3">
            {["open", "acknowledged", "resolved"].map((status) => (
              <div key={status} className="rounded-2xl bg-neutral-50 p-4">
                <p className="text-xs uppercase tracking-wide text-neutral-500">
                  {status}
                </p>
                <p className="mt-2 text-2xl font-black">
                  {summary?.alerts?.[status] ?? 0}
                </p>
              </div>
            ))}
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-3xl border border-neutral-200 bg-white p-6">
          <h2 className="text-xl font-black">Most consumed</h2>
          <div className="mt-4 divide-y divide-neutral-100">
            {topItems.mostConsumed.map((item) => (
              <div
        ...
        <section className="rounded-3xl border border-neutral-200 bg-white p-6">
          <h2 className="text-xl font-black">Highest inventory value</h2>
          <div className="mt-4 divide-y divide-neutral-100">
            {topItems.highestValue.map((item) => (
              <div
              ...
              >
                ...
                <p className="font-black">
                  {money.format(Number(item.inventoryValue ?? 0))}
                </p>
              </div>
            ))}
          </div>
      ...
      <section className="rounded-3xl border border-neutral-200 bg-white p-6">
        <h2 className="text-xl font-black">Automation job history</h2>
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {jobs.map((job) => (
                <tr key={job._id} className="border-b border-neutral-100">
                  <td className="py-3 font-bold">{job.jobType}</td>
                  <td>{job.status}</td>
                  <td>{job.source}</td>
                  <td>{job.attempts}</td>
      ...
      <section className="rounded-3xl border border-neutral-200 bg-white p-6">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-black">Scheduled reports</h2>
        </div>
        {canManage ? (
          <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            ...
            <select
              ...
              onChange={(event) =>
                setScheduleForm((current) => ({
                  ...current,
                  reportType: event.target.value as ScheduleForm["reportType"],
                }))
              }
            >
              <option value="valuation">Valuation</option>
              <option value="stock_ledger">Stock ledger</option>
            ...
            <button
              ...
              className="rounded-xl bg-neutral-950 px-4 py-2 font-bold text-white disabled:opacity-50"
            >
              Create schedule
            </button>
            <input
              className="rounded-xl border px-3 py-2 md:col-span-2 xl:col-span-5"
            ...
            <tbody>
              {schedules.map((schedule) => (
                <tr key={schedule._id} className="border-b border-neutral-100">
                  <td className="py-3 font-bold">{schedule.name}</td>
      ...
      <CustomActionModal
        ...
        onConfirm={async () => {
          if (deleteTarget) await deleteSchedule(deleteTarget._id);
        }}
      />
    </div>
  );
