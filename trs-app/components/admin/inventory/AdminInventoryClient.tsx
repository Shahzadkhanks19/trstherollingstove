...
type InventoryMovement = {
  _id: string;
  inventoryItemId:
    { _id: string; name: string; sku: string; unit: Unit } | string;
  type: string;
  quantity: number;
  stockBefore: number;
  stockAfter: number;
...
function InventoryActionModal({
  ...
  const canConfirm =
    !saving && (!isPermanentDelete || dialog.confirmationText === "DELETE");
...
function SummaryCard({
  ...
  return (
    <div className="rounded-2xl border border-[#e8ddd4] bg-white p-4 sm:p-5">
      <p className="text-[9px] font-black uppercase tracking-[.16em] text-[#8c8178]">
        {label}
      </p>
      <p
        className={`mt-3 break-words text-xl font-black sm:text-2xl ${alert ? "text-[#C8102E]" : "text-[#173044]"}`}
      >
        {value}
      </p>
    </div>
...
function TabButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`rounded-xl px-3 py-2 text-[10px] font-black uppercase tracking-wider sm:px-4 ${active ? "bg-[#fff0e8] text-[#C8102E]" : "text-[#786e66]"}`}
    >
      {children}
    </button>
  );
...
function InventoryRow({
  ...
  return (
    <tr className={!item.isActive ? "bg-slate-50/70" : ""}>
      <td className="px-5 py-4">
        <b className="block text-sm text-[#173044]">{item.name}</b>
        <span className="text-[10px] font-bold text-[#968a81]">{item.sku}</span>
      </td>
      <td className="px-5 py-4 text-xs font-bold text-[#655d57]">
        {item.category}
      </td>
      <td className="px-5 py-4">
        <b className="text-sm text-[#173044]">{item.currentStock}</b>{" "}
        <span className="text-xs text-[#8c8178]">{item.unit}</span>
      </td>
      <td className="px-5 py-4 text-xs text-[#655d57]">
        {item.reorderLevel} / {item.idealStockLevel} {item.unit}
      </td>
      <td className="px-5 py-4 text-xs font-bold text-[#173044]">
        {money.format(item.averageUnitCost)}/{item.unit}
      </td>
      <td className="px-5 py-4">
        <span
          className={`rounded-full px-3 py-1 text-[9px] font-black uppercase ${status.cls}`}
        >
          {status.label}
        </span>
      </td>
            ...
            (item.isActive ? (
              <>
                <button
                  disabled={saving}
                  onClick={onMovement}
                  className="rounded-lg border border-[#ded3ca] px-3 py-2 text-[10px] font-black disabled:opacity-50"
                >
                  Stock
                </button>
                <button
                  disabled={saving}
                  onClick={onEdit}
                  className="rounded-lg bg-[#173044] px-3 py-2 text-[10px] font-black text-white disabled:opacity-50"
                >
                  Edit
                </button>
                <button
                  disabled={saving}
                  onClick={onArchive}
                  className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-[10px] font-black text-red-700 disabled:opacity-50"
                >
                  Delete
                </button>
              </>
              ...
              <>
                ...
                <button
                  ...
                  className="rounded-lg bg-red-700 px-3 py-2 text-[10px] font-black text-white disabled:opacity-50"
                >
                  Permanent delete
                </button>
              </>
...
function InventoryCard({
  ...
  return (
    <article
      className={`min-w-0 rounded-2xl border p-4 ${item.isActive ? "border-[#eee4dc]" : "border-slate-200 bg-slate-50"}`}
    >
      <div className="flex min-w-0 items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="truncate text-sm font-black text-[#173044]">
            {item.name}
          </h3>
          <p className="mt-1 truncate text-[10px] font-bold text-[#8c8178]">
            {item.sku} · {item.category}
          </p>
        </div>
        <span
          className={`shrink-0 rounded-full px-2 py-1 text-[8px] font-black uppercase ${status.cls}`}
        >
          {status.label}
        </span>
      </div>
      <div className="mt-4 grid grid-cols-2 gap-3 rounded-xl bg-[#fffaf6] p-3">
        <div>
          <p className="text-[8px] font-black uppercase text-[#968a81]">
            Current
          </p>
          <b className="mt-1 block text-base text-[#173044]">
        ...
        (item.isActive ? (
          <div className="mt-3 grid grid-cols-3 gap-2">
            <button
              disabled={saving}
              onClick={onMovement}
              className="rounded-xl border border-[#ded3ca] px-2 py-2 text-[9px] font-black disabled:opacity-50"
            >
              Stock
            </button>
            <button
              disabled={saving}
              onClick={onEdit}
              className="rounded-xl bg-[#173044] px-2 py-2 text-[9px] font-black text-white disabled:opacity-50"
            >
              Edit
            </button>
            <button
              disabled={saving}
              onClick={onArchive}
              className="rounded-xl border border-red-200 bg-red-50 px-2 py-2 text-[9px] font-black text-red-700 disabled:opacity-50"
            >
              Delete
            </button>
          </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <button
              disabled={saving}
              onClick={onRestore}
              className="rounded-xl bg-emerald-600 px-3 py-2 text-[10px] font-black text-white disabled:opacity-50"
            >
              Restore item
            </button>
            <button
              disabled={saving}
              onClick={onPermanentDelete}
              className="rounded-xl bg-red-700 px-3 py-2 text-[10px] font-black text-white disabled:opacity-50"
            >
              Permanent delete
            </button>
          </div>
    ...
    <div className="divide-y divide-[#eee5de]">
      {movements.map((movement) => {
        ...
        return (
          <div
            key={movement._id}
            className="grid gap-2 p-4 sm:grid-cols-[1fr_auto] sm:items-center sm:px-5"
          >
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <b className="truncate text-sm text-[#173044]">
                  {item?.name ?? "Inventory item"}
                </b>
                <span className="rounded-full bg-[#fff0e8] px-2 py-1 text-[8px] font-black uppercase text-[#C8102E]">
                  {movement.type.replaceAll("_", " ")}
                </span>
              </div>
              <p className="mt-1 text-[10px] font-bold text-[#8c8178]">
                {item?.sku ?? ""} ·{" "}
                {new Date(movement.createdAt).toLocaleString("en-IN")}
              </p>
              {movement.reason && (
                <p className="mt-2 text-xs text-[#6f655e]">{movement.reason}</p>
              )}
            </div>
  ...
  return (
    <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: 6 }, (_, index) => (
        <div
          key={index}
          className="h-36 animate-pulse rounded-2xl bg-[#f5eee8]"
        />
      ))}
    </div>
  );
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="px-5 py-20 text-center">
      <p className="text-sm font-bold text-[#8c8178]">{text}</p>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {selected && (
          <div className="rounded-xl bg-[#fff5ee] p-4 text-xs font-bold text-[#6c5e54]">
            Current stock:{" "}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {selected?.expiryTrackingEnabled && inbound && (
        ...
        <Field label="Reason / note">
          <textarea
            ...
            placeholder="Why is this stock being added or removed?"
          />
        </Field>
      </div>
    </Drawer>
...
function Drawer({
  ...
  return (
    ...
    >
      <motion.section
        onMouseDown={(event) => event.stopPropagation()}
        className="absolute inset-x-0 bottom-0 flex max-h-[92dvh] flex-col rounded-t-3xl bg-[#fffdfb] shadow-2xl sm:inset-y-0 sm:left-auto sm:w-[620px] sm:rounded-none"
        initial={{ y: 70, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        exit={{ y: 70, opacity: 0 }}
      >
        <header className="flex items-center justify-between border-b border-[#eee4dc] p-5">
          <div>
            <p className="text-[9px] font-black uppercase tracking-[.2em] text-[#C8102E]">
              Inventory editor
            </p>
            <h2 className="mt-1 text-xl font-black text-[#173044]">{title}</h2>
          </div>
          <button
            onClick={onClose}
            className="grid h-10 w-10 place-items-center rounded-xl border border-[#e5dbd3] text-lg"
          >
            ×
          </button>
        </header>
        <div className="min-h-0 flex-1 overflow-y-auto p-5">{children}</div>
        <footer className="flex flex-col-reverse gap-2 border-t border-[#eee4dc] bg-white p-4 sm:flex-row sm:justify-end">
          {footer}
        </footer>
      </motion.section>
    </motion.div>
...
function Field({
  ...
  return (
    <label className="block text-[10px] font-black uppercase tracking-wider text-[#756b63]">
      <span className="mb-2 block">{label}</span>
      {cloneElement(children, {
        className: `${children.props.className ?? ""} w-full min-w-0 rounded-xl border border-[#ded3ca] bg-white px-4 py-3 text-sm font-medium normal-case tracking-normal text-[#173044] outline-none focus:border-[#C8102E]`,
      })}
    </label>
  );
}
