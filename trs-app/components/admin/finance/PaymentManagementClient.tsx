    ...
    try {
      ...
      setData(
        await financeApi<Summary>(
          `/api/v1/admin/finance/payment-management/summary?days=${days}`,
        ),
      );
    } catch (value) {
      setError(
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap gap-2">
            ...
            <Link
              href={`/api/v1/admin/finance/payment-management/report?days=${days}`}
              prefetch={false}
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          label="Net collected"
          value={money(metrics?.netCollectedAmount ?? 0)}
          icon={faMoneyBillTransfer}
          detail={`${metrics?.paymentCount ?? 0} payment records`}
        />
        <StatCard
          label="Success rate"
          value={`${metrics?.successRate ?? 0}%`}
          icon={faCircleCheck}
          detail={`${metrics?.capturedCount ?? 0} captured payments`}
        />
        <StatCard
          label="Refunded"
          value={money(metrics?.refundedAmount ?? 0)}
          icon={faRotateLeft}
          detail={`${metrics?.refundedCount ?? 0} affected payments`}
        />
        <StatCard
          label="Unmatched"
          value={money(metrics?.unmatchedAmount ?? 0)}
          icon={faTriangleExclamation}
          detail={`${metrics?.matchedReconciliationCount ?? 0}/${metrics?.reconciliationCount ?? 0} reconciled`}
        />
      </div>
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
            ...
            {data?.recentPayments.map((payment) => (
              ...
              >
                ...
                <div className="text-right">
                  <p className="font-black">
                    {money(payment.amount - payment.amountRefunded)}
                  </p>
        ...
        <section className="rounded-2xl border bg-white p-5">
          ...
          <div className="mt-4 space-y-3">
            {data?.recentReconciliations.map((row) => (
              <div
                key={row._id}
                className="flex justify-between gap-4 rounded-xl border p-3"
              >
                <div>
                  <p className="font-black">
                    {row.settlementReference || row.reconciliationType}
                  </p>
                  <p className="text-xs text-slate-500">
                    Expected {money(row.expectedAmount)} · Settled{" "}
                    {money(row.settledAmount)}
                  </p>
                </div>
                <div className="text-right">
                  <p className="font-black">
                    {money(Math.abs(row.differenceAmount))}
                  </p>
                  <p className="text-[11px] font-bold uppercase text-slate-500">
                    {row.status}
                  </p>
            ...
            {!data?.recentReconciliations.length ? (
              <p className="rounded-xl border border-dashed p-5 text-sm text-slate-500">
                No reconciliation records in this period.
              </p>
            ) : null}
          </div>
        </section>
