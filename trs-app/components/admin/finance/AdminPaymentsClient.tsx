...
export function AdminPaymentsClient({ canRefund }: { canRefund: boolean }) {
  ...
  useRealtimeRefresh({
    events: ["payment.updated", "order.payment_updated"],
    onEvent: () => load(),
  });
  useEffect(() => {
  ...
  const totals = useMemo(
    () => ({
      ...
      refunded: rows.reduce((a, b) => a + (b.amountRefunded || 0), 0),
  ...
  async function submitRefund(reason: string) {
    ...
    const response = await fetch(
      `/api/v1/admin/payments/${refund._id}/refund`,
      {
        method: "POST",
    ...
    if (!response.ok) throw new Error(json.message || "Refund failed.");
    setRefund(null);
    await load();
  }
  return (
    <div>
      <PageHeader
        eyebrow="Finance"
        title="Payments"
        description="Monitor online transactions, failures, captured revenue and refunds from one place."
      />
      <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric label="Transactions" value={String(total)} icon={faReceipt} />
        <Metric
          label="Captured on page"
          value={money(totals.captured)}
      ...
      <div className="mb-5 grid gap-3 lg:grid-cols-[1fr_220px_auto]">
        <label className="flex items-center gap-2 rounded-xl border border-[#e8ddd3] bg-white px-3">
          <FontAwesomeIcon icon={faMagnifyingGlass} />
          <input
            value={searchInput}
            onChange={(e) => setSearchInput(e.target.value)}
            placeholder="Order, payment, email or phone"
        ...
        <select
          ...
          className="rounded-xl border border-[#e8ddd3] bg-white px-3 text-sm"
        >
        ...
        <button
          onClick={() => void load()}
          className="rounded-xl border border-[#e8ddd3] bg-white px-4 py-3 text-xs font-black"
        >
          Refresh
        </button>
      </div>
      <SectionCard
        ...
        subtitle={loading ? "Loading…" : `${total} transaction(s)`}
      >
        {error && (
          ...
          <table className="min-w-[1050px] w-full text-left text-sm">
            ...
            <tbody>
              {rows.map((row) => (
                ...
                >
                  <td className="px-3 py-4 font-black text-[#173044]">
                    #{row.orderId?.orderNumber || "—"}
                  </td>
                  <td className="px-3 py-4">
                    <p className="font-bold">
                      {row.customerId?.name || "Guest"}
                    </p>
                    <p className="text-xs text-slate-500">
                      {row.email || row.customerId?.email || row.contact || "—"}
                    </p>
                  </td>
                  <td className="px-3 py-4 font-mono text-xs">
                    <p>{row.providerPaymentId || "Pending"}</p>
                    <p className="text-slate-400">{row.providerOrderId}</p>
                  </td>
                  ...
                  <td className="px-3 py-4 font-black">
                    ...
                    {row.amountRefunded > 0 && (
                      <p className="text-xs text-red-600">
                        Refunded {money(row.amountRefunded)}
                      </p>
                    )}
                  </td>
                  <td className="px-3 py-4">
                  ...
                  <td className="px-3 py-4">
                    {canRefund &&
                    ["captured", "partially_refunded"].includes(row.status) &&
                    row.amountRefunded < row.amount ? (
                      <button
      ...
      <CustomActionModal
        ...
        description={
          refund
            ? `Refund ${money(refund.amount - refund.amountRefunded)} for order #${refund.orderId?.orderNumber || "—"}.`
            : ""
        }
