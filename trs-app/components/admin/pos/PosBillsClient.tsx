...
async function readApiResponse<T>(response: Response): Promise<ApiResponse<T>> {
  ...
  if (contentType.includes("application/json")) {
    try {
      ...
      throw new Error(
        `The server returned malformed JSON (${response.status}).`,
      );
    }
  }
...
export function PosBillsClient({
  ...
  const load = useCallback(
    async (signal?: AbortSignal) => {
      ...
      const response = await fetch(`/api/v1/pos/bills?${params}`, {
        cache: "no-store",
        signal,
      });
  ...
  async function modifyOrder(bill: Bill) {
    ...
    try {
      const response = await fetch(
        `/api/v1/pos/orders/${bill.orderId}/edit-copy`,
        { cache: "no-store" },
      );
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "Unable to load order for modification.",
      );
      setBusy(false);
    }
  }
  ...
  async function cancelPaidOrder() {
    ...
    try {
      const response = await fetch(
        `/api/v1/pos/orders/${cancelBill.orderId}/cancel-paid`,
        {
          method: "POST",
  ...
  async function attachCustomer() {
    ...
    try {
      ...
      setMessage(
        `Customer attached to ${customerBill.orderNumber}. Invoice details updated.`,
      );
      ...
      setMessage(
        error instanceof Error ? error.message : "Unable to attach customer.",
      );
    } finally {
      setBusy(false);
    }
    ...
    <section className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-950">Bill history</h1>
        </div>
        <Link
          href="/admin/pos"
      ...
      <div className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:grid-cols-[1fr_180px]">
        <input
          value={query}
                ...
                return (
                  <tr key={bill._id} className="border-t border-slate-100">
                    <td className="px-4 py-3">
                      <p className="font-black">{bill.invoiceNumber}</p>
                      <p className="text-xs text-slate-500">
                        {bill.orderNumber} ·{" "}
                        {new Date(bill.issuedAt).toLocaleString()}
                      </p>
                      {closed && (
                        <span className="mt-1 inline-flex rounded-full bg-red-100 px-2 py-1 text-[10px] font-black uppercase text-red-700">
                          Cancelled / refunded
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <p className="font-bold">{bill.customerSnapshot.name}</p>
                      <p className="text-xs text-slate-500">
                    ...
                    <td className="px-4 py-3">{bill.printCount}</td>
                    <td className="px-4 py-3">
                      <div className="flex min-w-72 flex-wrap gap-2">
                        ...
                        <button
                          ...
                          onClick={() => void modifyOrder(bill)}
                          className="rounded-lg bg-blue-700 px-3 py-2 text-xs font-black text-white disabled:opacity-40"
                        >
                          Duplicate / rebill
                        </button>
                        <button
                            ...
                            setCustomerForm({
                              name:
                                ...
                                "Walk-in Customer"
                                  ? ""
                                  : bill.customerSnapshot.name,
                              phone: bill.customerSnapshot.phone ?? "",
                              email: "",
                            });
                          }}
                          className="rounded-lg border border-amber-400 bg-amber-50 px-3 py-2 text-xs font-black text-amber-900 disabled:opacity-40"
                        >
                          Add customer later
                        </button>
                        <button
                          ...
                          className="rounded-lg border border-red-300 px-3 py-2 text-xs font-black text-red-700 disabled:opacity-40"
                        >
                          Cancel order
                        </button>
                      </div>
      ...
      {customerBill && (
        <div className="fixed inset-0 z-[190] grid place-items-end bg-black/55 p-0 backdrop-blur-sm sm:place-items-center sm:p-5">
          <button
            type="button"
            className="absolute inset-0"
            onClick={() => !busy && setCustomerBill(null)}
            aria-label="Close customer attachment"
          />
          <section
            role="dialog"
            aria-modal="true"
            className="relative z-10 w-full rounded-t-3xl bg-white p-5 shadow-2xl sm:max-w-md sm:rounded-3xl"
          >
            <h2 className="text-xl font-black">Add customer after checkout</h2>
            <p className="mt-2 text-sm text-slate-600">
              Creates or matches a customer by phone, then updates the order and
              invoice.
            </p>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2">
              ...
              <button
                ...
                className="h-11 rounded-xl bg-amber-500 font-black text-slate-950 disabled:opacity-50"
              >
                {busy ? "Saving…" : "Create/link customer"}
              </button>
            </div>
          </section>
        </div>
      ...
      {cancelBill && (
        <div className="fixed inset-0 z-[190] grid place-items-end bg-black/55 p-0 backdrop-blur-sm sm:place-items-center sm:p-5">
          <button
            type="button"
            className="absolute inset-0"
            onClick={() => !busy && setCancelBill(null)}
          ...
          <section
            ...
            className="relative z-10 w-full rounded-t-3xl bg-white p-5 shadow-2xl sm:max-w-md sm:rounded-3xl"
          >
            <h2 className="text-xl font-black">Cancel paid order</h2>
            <p className="mt-2 text-sm text-slate-600">
              This records a full refund for{" "}
              <strong>{cancelBill.orderNumber}</strong> and marks the order
              cancelled. The original invoice remains preserved for audit.
            </p>
            <label className="mt-4 block text-xs font-black">
              ...
              <select
                value={refundMethod}
                onChange={(event) =>
                  setRefundMethod(event.currentTarget.value as "cash" | "upi")
                }
                className="mt-1 h-11 w-full rounded-xl border px-3"
              >
                <option value="cash">Cash</option>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2">
              ...
              <button
                ...
                className="h-11 rounded-xl bg-red-700 font-black text-white disabled:opacity-50"
              >
                {busy
                  ? "Cancelling…"
                  : `Refund ${money.format(cancelBill.grandTotal)} & cancel`}
              </button>
            </div>
          </section>
        </div>
