...
import type { PosBillingState } from "@/components/admin/pos/use-pos-billing";
...
type Props = {
  cart: PosCartState;
  billing: PosBillingState;
};

export function PosBillingPanel({ cart, billing }: Props) {
  const {
    totals,
    shift,
    registers,
      ...
      {!shift ? (
        <section className="rounded-2xl border border-[#e5d9cf] bg-white p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-black text-[#122b3c]">Open a register shift</h3>
        ...
        <>
          <section className="rounded-2xl border border-[#e5d9cf] bg-white p-4">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="text-right text-xs font-bold text-[#7c7067]">
                ...
                <br />
                Today&apos;s cash drawer {money.format(shift.expectedCash ?? 0)}
              </div>
            </div>
          </section>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-[#7c7067]">
                  KOT and invoice open as separate print jobs using the formats
                  selected in POS Setup.
                </p>
              </div>
              <a
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            >
              <FontAwesomeIcon icon={faQrcode} className="text-violet-700" />
          ...
          {paymentMethod === "cash" ? (
            ...
            <label className="block text-xs font-black text-[#756960]">
              ...
              <input
                value={upiReference}
                onChange={(event) => setUpiReference(event.currentTarget.value)}
                maxLength={100}
                className="mt-1 h-11 w-full rounded-xl border border-[#e5d9cf] px-3 text-sm"
              />
            </label>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="text-xs font-black">
                ...
                <input
                  ...
                  min="0"
                  value={splitCash}
                  onChange={(event) => setSplitCash(event.currentTarget.value)}
              ...
              <label className="text-xs font-black">
                ...
                <input
                  ...
                  min="0"
                  value={splitUpi}
                  onChange={(event) => setSplitUpi(event.currentTarget.value)}
                  className="mt-1 h-11 w-full rounded-xl border px-3"
                />
              </label>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-xs font-black">
              ...
              <input
                ...
                min="0"
                value={waivedAmount}
                onChange={(event) => setWaivedAmount(event.currentTarget.value)}
                className="mt-1 h-11 w-full rounded-xl border px-3"
                placeholder="e.g. 10"
              />
            </label>
            <label className="text-xs font-black">
              ...
              <input
                value={waivedReason}
                onChange={(event) => setWaivedReason(event.currentTarget.value)}
                className="mt-1 h-11 w-full rounded-xl border px-3"
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-xs font-black">
              ...
              <input
                ...
                onChange={(event) => setOrderTakerName(event.currentTarget.value)}
                className="mt-1 h-11 w-full rounded-xl border px-3"
                placeholder="Name"
              />
            </label>
            <div className="grid grid-cols-2 gap-2">
              ...
              <label className="text-xs font-black">
                ...
                <select
                  value={tipMethod}
                  onChange={(event) =>
                    setTipMethod(event.currentTarget.value as PosBillingTipMethod)
                  }
                  className="mt-1 h-11 w-full rounded-xl border px-3"
                >
            ...
            >
              {tipMethod === "upi" ? (
                <>
                  ...
                  . Of this, <b>{money.format(Number(tipAmount || 0))}</b> is
                  payable to {orderTakerName.trim() || "the waiter"} and is not
                  restaurant revenue.
                </>
              ) : (
                <>
                  Cash tip is collected directly by{" "}
                  {orderTakerName.trim() || "the waiter"}. It is not added to the
                  restaurant payment or cash drawer.
                </>
              )}
            </div>
          )}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-800">
                  KOT and invoice are opened as independent print jobs so kitchen
                  and billing printers can be selected separately.
                </p>
              </div>
            </div>
          </section>
          ...
          >
            <FontAwesomeIcon icon={faPrint} /> Complete sale & open print jobs
          </button>
          ...
          {lastInvoiceId && (
            <div className="grid gap-2 sm:grid-cols-2">
              <button
                type="button"
                onClick={() => reprintPosInvoice(lastInvoiceId, printSettings)}
              ...
              <button
                type="button"
                onClick={() => reprintPosKot(lastInvoiceId, printSettings)}
                className="h-11 rounded-xl border border-[#e5d9cf] bg-white text-xs font-black"
              >
                Reprint kitchen KOT
              </button>
