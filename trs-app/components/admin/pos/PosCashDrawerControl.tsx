    ...
    <section className="min-w-0 rounded-2xl border border-[#e5d9cf] bg-white p-4">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <div className="rounded-xl bg-[#111820] p-3 text-white">
          <p className="text-[9px] font-black uppercase text-white/55">
            Expected
          </p>
      ...
      <div
        className={`mt-4 space-y-2 ${compact ? "max-h-56" : "max-h-72"} overflow-y-auto pr-1 [scrollbar-width:thin]`}
      >
        ...
        {movements.map((movement) => {
          ...
          return (
            <article
              key={movement._id}
              className="flex items-start justify-between gap-3 rounded-xl border border-[#eadfd6] bg-white p-3"
            >
              <div className="flex min-w-0 items-start gap-3">
                <span
                  className={`mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-lg ${positive ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}
                >
                  <FontAwesomeIcon icon={positive ? faArrowUp : faArrowDown} />
                </span>
                <div className="min-w-0">
                  <p className="text-xs font-black text-[#122b3c]">
              ...
              <p
                className={`shrink-0 text-sm font-black ${positive ? "text-emerald-700" : "text-amber-700"}`}
              >
                {positive ? "+" : "−"}
      ...
      {shift.status === "closed" ? (
        <div className="mt-4 grid grid-cols-3 gap-2 rounded-xl bg-slate-50 p-3 text-center">
          ...
          <div>
            ...
            <p
              className={`mt-1 text-xs font-black ${(shift.cashDifference ?? 0) === 0 ? "text-emerald-700" : "text-red-700"}`}
            >
