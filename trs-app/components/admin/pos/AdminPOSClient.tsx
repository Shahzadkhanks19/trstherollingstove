...
export function AdminPOSClient({ canManage }: { canManage: boolean }) {
  ...
  const [orderMode, setOrderMode] = useState<"dine_in" | "takeaway">(
    "takeaway",
  );
  ...
  const load = useCallback(async () => {
    const [catalogResponse, registerResponse, shiftResponse] =
      await Promise.all([
    ...
    if (catalogResponse.ok) {
      const json = (await catalogResponse.json()) as ApiResponse<{
        menuItems: CatalogItem[];
        posItems: CatalogItem[];
      }>;
  ...
  async function completeSale() {
    ...
    if (paymentMethod === "upi" && !upiConfirmOpen) {
    ...
    try {
      ...
      setMessage(
        `${json.data.order.orderNumber} created, sent to live orders/KDS, and bill saved.`,
      );
      setCart([]);
      setUpiReference("");
      window.open(
        `/api/v1/pos/bills/${json.data.invoice._id}/print`,
        "_blank",
        "noopener,noreferrer",
      );
    } catch (error) {
    ...
    return (
      <section className="mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-600">
          A shift is required before creating cash or PhonePe QR/UPI reference
          sales.
        </p>
        ...
        <button
          ...
          className="mt-5 rounded-xl bg-red-700 px-6 py-3 font-black text-white disabled:opacity-50"
        >
          Open shift
        </button>
        {registers.length === 0 && (
          <p className="mt-4 text-sm text-amber-700">
            Create a POS register through the existing POS register API before
            opening a shift.
          </p>
        )}
        {message && (
  ...
  return (
    <section className="grid min-h-[calc(100vh-8rem)] gap-4 xl:grid-cols-[minmax(0,1fr)_390px]">
      <div className="min-w-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl font-black text-slate-950">New sale</h1>
          </div>
          <div className="text-right text-xs font-bold text-slate-500">
            Shift open
            <br />
            Expected cash: {formatMoney(shift.expectedCash)}
          </div>
        </div>
        <div className="mt-4 flex gap-3">
          <input
            ...
            className="h-11 min-w-0 flex-1 rounded-xl border border-slate-300 px-4"
            placeholder="Search menu and POS-only items"
          />
          {canManage && (
            <a
              href="/admin/pos/setup"
              className="grid h-11 place-items-center rounded-xl border border-slate-300 px-4 text-xs font-black"
            >
              Manage items
            </a>
          )}
        ...
        <div className="mt-3 flex gap-2 overflow-x-auto pb-2">
          {categories.map((value) => (
            <button
              ...
              className={`whitespace-nowrap rounded-full px-4 py-2 text-xs font-black ${category === value ? "bg-red-700 text-white" : "bg-slate-100 text-slate-700"}`}
            >
              {value}
            </button>
          ))}
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-[10px] font-black uppercase text-red-700">
                    {item.sourceType === "pos" ? "POS only" : item.category}
                  </p>
              ...
              {item.variants && item.variants.length > 0 ? (
                <div className="mt-3 grid gap-2">
                  {item.variants.map((variant) => (
                    <button
                      ...
                      className="rounded-lg bg-slate-100 px-3 py-2 text-left text-xs font-bold"
                    >
                ...
                <button
                  ...
                  className="mt-3 w-full rounded-lg bg-red-700 px-3 py-2 text-xs font-black text-white"
                >
                  Add
                </button>
              )}
      ...
      <aside className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            >
              ...
              <div className="mt-2 flex items-center gap-2">
                <button
                  ...
                  className="h-8 w-8 rounded-lg bg-slate-100 font-black"
                >
                  −
                </button>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button
            ...
            className={`rounded-xl px-3 py-2 text-xs font-black ${orderMode === "takeaway" ? "bg-slate-950 text-white" : "bg-slate-100"}`}
          >
            Takeaway
          </button>
          <button
            ...
            className={`rounded-xl px-3 py-2 text-xs font-black ${orderMode === "dine_in" ? "bg-slate-950 text-white" : "bg-slate-100"}`}
          >
            Dine-in
          </button>
        </div>
        {orderMode === "dine_in" && (
          <input
            ...
            className="mt-2 h-10 w-full rounded-xl border border-slate-300 px-3"
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <button
            ...
            className={`rounded-xl px-3 py-3 text-xs font-black ${paymentMethod === "cash" ? "bg-emerald-700 text-white" : "bg-slate-100"}`}
          >
            Cash
          </button>
          <button
            ...
            className={`rounded-xl px-3 py-3 text-xs font-black ${paymentMethod === "upi" ? "bg-violet-700 text-white" : "bg-slate-100"}`}
          >
            PhonePe QR / UPI
          </button>
        </div>
        ...
        <button
          ...
          className="mt-4 w-full rounded-xl bg-red-700 px-4 py-4 font-black text-white disabled:opacity-50"
        >
          Complete, save & print
        </button>
        {lastInvoiceId && (
          <button
            ...
            onClick={() =>
              window.open(
                `/api/v1/pos/bills/${lastInvoiceId}/print`,
                "_blank",
                "noopener,noreferrer",
              )
            }
            className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm font-black"
          >
            Reprint last bill
          </button>
        )}
      ...
      <CustomActionModal
        open={upiConfirmOpen}
        title="Confirm PhonePe / UPI payment"
        description="Confirm the payment in PhonePe or your merchant app before completing this sale."
        confirmLabel="Payment confirmed"
