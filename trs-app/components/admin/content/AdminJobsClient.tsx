...
const emptyForm = {
  ...
  location: "Shastri Circle, Jodhpur",
  summary: "",
  responsibilities: "",
  requirements: "",
  vacancies: 1,
...
export function AdminJobsClient() {
  ...
  const load = useCallback(async () => {
    try {
      const response = await fetch("/api/v1/admin/careers/jobs", {
        cache: "no-store",
      });
      const json = (await response.json()) as Api<Job[]>;
  ...
  async function create() {
    if (form.closesAt && new Date(form.closesAt).getTime() <= Date.now()) {
      setMessage("Closing date must be in the future.");
      return;
    }
    const payload = {
      ...form,
      ...
      closesAt: isoOrNull(form.closesAt),
    };
    const response = await fetch("/api/v1/admin/careers/jobs", {
      method: "POST",
    ...
    if (!response.ok)
      return setMessage(json.message || "Unable to create job.");
    setForm(emptyForm);
    setShowForm(false);
  ...
  async function toggle(job: Job) {
    await fetch(`/api/v1/admin/careers/jobs/${job._id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
  ...
  async function destroy() {
    if (!remove) return;
    await fetch(`/api/v1/admin/careers/jobs/${remove._id}`, {
      method: "DELETE",
    });
    setRemove(null);
    await load();
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Content"
        title="Jobs"
        description="Publish and manage career openings without affecting the existing background-job worker APIs."
        action={
          <div className="flex gap-2">
            <a
              href="/admin/jobs/applications"
              className="rounded-xl border border-[#173044] px-4 py-3 text-xs font-black text-[#173044]"
            >
              Applications
            </a>
            <button
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        {[
          ["Openings", rows.length, faBriefcase],
          ["Published", rows.filter((j) => j.isPublished).length, faUsers],
          ["Vacancies", rows.reduce((sum, j) => sum + j.vacancies, 0), faUsers],
      ...
      {showForm && (
        <SectionCard title="Create job opening">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <button
              onClick={() => void create()}
              className="rounded-xl bg-[#C8102E] px-4 py-3 text-xs font-black text-white md:col-span-2"
            >
              Create job opening
            </button>
          </div>
            ...
            <div className="flex flex-col justify-between gap-4 sm:flex-row">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full bg-[#fff0e8] px-2.5 py-1 text-[9px] font-black text-[#C8102E]">
                    {job.employmentType}
                  </span>
                </div>
                <p className="mt-1 text-xs font-semibold text-[#756960]">
                  {job.location} · {job.vacancies}{" "}
                  {job.vacancies === 1 ? "vacancy" : "vacancies"}
                </p>
                <p className="mt-3 max-w-3xl text-sm leading-6 text-[#655e57]">
                  {job.summary}
                </p>
              </div>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  onClick={() => setRemove(job)}
                  className="h-11 rounded-xl border px-4 text-red-700"
                >
      ...
      <CustomActionModal
        ...
        description={`Delete ${remove?.title ?? "this opening"} permanently?`}
        confirmLabel="Delete"
        tone="danger"
