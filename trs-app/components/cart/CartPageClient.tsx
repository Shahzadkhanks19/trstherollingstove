...
export function CartPageClient() {
  ...
  async function updateQuantity(itemId: string, quantity: number) {
    ...
    try {
      if (authenticated)
        await mutate(`/api/v1/customer/cart/items/${itemId}`, "PATCH", {
          quantity,
        });
  ...
  async function removeItem(itemId: string) {
    ...
    try {
      if (authenticated)
        await mutate(`/api/v1/customer/cart/items/${itemId}`, "DELETE");
    ...
    <main className="min-h-screen bg-[#FFFDF9] text-[#172536]">
      <section className="overflow-hidden border-b border-[#EDE3D8] bg-[linear-gradient(135deg,#FFFDF9,#FFF6EC)]">
        <div className="mx-auto grid min-h-[390px] w-[min(100%-2rem,1240px)] items-center gap-8 py-10 lg:grid-cols-[minmax(0,.9fr)_minmax(0,1.1fr)] lg:py-12">
          <div>
            <p className="text-sm font-black italic text-[#C8102E]">
              Review Your Selection
            </p>
            <h1 className="mt-3 text-[clamp(3rem,8vw,6rem)] font-black uppercase leading-none tracking-[-.055em]">
              Your <span className="text-[#C8102E]">Cart</span>
            </h1>
            <p className="mt-4 max-w-xl text-sm leading-6 text-[#655E57]">
              Real menu items, configurations and server-verified prices. Review
              everything before continuing to checkout.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                href="/menu"
                className="inline-flex items-center gap-2 rounded-xl border border-[#C8102E] bg-white px-5 py-3 text-[10px] font-black uppercase text-[#C8102E]"
              >
                Continue Shopping <FontAwesomeIcon icon={faArrowRight} />
              </Link>
              <span className="inline-flex items-center rounded-xl bg-[#FFF1E5] px-5 py-3 text-[10px] font-black uppercase text-[#7F0007]">
                {cart?.itemCount ?? 0} items selected
              </span>
            </div>
          </div>
          <CmsHeroMedia
      ...
      <section className="mx-auto grid w-[min(100%-2rem,1240px)] gap-5 py-10 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="rounded-3xl border border-[#EDE3D8] bg-white p-5 shadow-sm sm:p-7">
          <div className="flex items-center justify-between gap-4 border-b border-[#EDE3D8] pb-5">
            ...
            {!!cart?.items.length && (
              <button
                type="button"
                disabled={workingId === "clear"}
                onClick={() => void clearCart()}
                className="rounded-xl border border-[#C8102E] px-4 py-3 text-[9px] font-black uppercase text-[#C8102E] disabled:opacity-50"
              >
          ...
          {loading ? (
            <div className="py-16 text-center text-sm font-bold text-[#655E57]">
              Loading your cart…
            </div>
          ) : !cart?.items.length ? (
            <div className="py-16 text-center">
              ...
              <p className="mt-2 text-sm text-[#655E57]">
                Choose an item from the live menu to begin.
              </p>
              <Link
                href="/menu"
                className="mt-5 inline-flex rounded-xl bg-[#C8102E] px-6 py-4 text-xs font-black uppercase text-white"
              >
                Browse Menu
              </Link>
            </div>
            ...
            <div className="divide-y divide-[#EDE3D8]">
              {cart.items.map((item) => (
                <article
                  key={item._id}
                  className="grid gap-4 py-5 sm:grid-cols-[110px_minmax(0,1fr)_150px] sm:items-center"
                >
                  {item.imageUrl ? (
                    <Image
                      src={item.imageUrl}
                  ...
                  <div>
                    ...
                    {item.variantName && (
                      <p className="mt-1 text-[10px] font-bold text-[#655E57]">
                        {item.variantName}
                      </p>
                    )}
                    {item.modifiers?.map((modifier, index) =>
                      modifier.optionName ? (
                        <p
                          key={`${modifier.optionName}-${index}`}
                          className="mt-1 text-[9px] text-[#655E57]"
                        >
                          + {modifier.optionName}
                        </p>
                      ) : null,
                    ...
                    {item.specialInstructions && (
                      <p className="mt-2 text-[9px] italic text-[#655E57]">
                        Note: {item.specialInstructions}
                      </p>
                    )}
                    <p className="mt-3 font-black text-[#C8102E]">
                      {money(item.lineUnitPrice)} each
                    </p>
                  </div>
                  <div className="flex items-center justify-between gap-3 sm:flex-col sm:items-end">
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        disabled={workingId === item._id || item.quantity <= 1}
                        onClick={() =>
                      ...
                      <strong className="w-7 text-center">
                        {item.quantity}
                      </strong>
                      <button
                        type="button"
                        disabled={workingId === item._id || item.quantity >= 50}
                        onClick={() =>
                          void updateQuantity(item._id, item.quantity + 1)
                        }
                        className="grid h-9 w-9 place-items-center rounded-lg border disabled:opacity-40"
                    ...
                    <button
                      ...
                      className="text-[9px] font-black uppercase text-[#C8102E] disabled:opacity-40"
                    >
                      <FontAwesomeIcon icon={faTrashCan} className="mr-2" />
                      Remove
                    </button>
        ...
        <aside className="h-fit rounded-3xl border border-[#EDE3D8] bg-white p-6 shadow-sm lg:sticky lg:top-28">
          ...
          <div className="mt-6 space-y-4 text-sm">
            ...
            <div className="flex justify-between">
              <span>Taxes</span>
              <strong>{money(cart?.taxTotal ?? 0)}</strong>
            </div>
            <div className="flex justify-between border-t pt-4 text-lg">
              <span className="font-black">Total</span>
              <strong className="text-[#C8102E]">
                {money(cart?.grandTotal ?? 0)}
              </strong>
            </div>
          </div>
          <Link
            ...
            className="mt-6 flex h-12 items-center justify-center gap-3 rounded-xl bg-[#C8102E] px-5 text-[10px] font-black uppercase text-white"
          >
            {cart?.items.length
              ? authenticated
                ? "Proceed to Checkout"
          ...
          <p className="mt-4 text-[9px] leading-4 text-[#655E57]">
            Final coupon, coin and availability checks are performed by the
