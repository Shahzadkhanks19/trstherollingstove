    ...
    return (
      <main className="grid min-h-[70vh] place-items-center bg-[#FFF9EF] px-4 py-16 text-[#171717]">
        <div className="w-full max-w-lg rounded-[28px] border border-[#EADFCC] bg-white p-8 text-center shadow-[0_22px_60px_rgba(88,56,19,0.12)] sm:p-12">
          ...
          />
          <h1 className="mt-6 text-3xl font-black uppercase tracking-[-0.04em] text-[#B7192C]">
            Confirming Your Order
          </h1>
          <p className="mt-4 text-sm leading-7 text-[#6A6158]">
        ...
        <div className="w-full max-w-xl rounded-[28px] border border-[#EADFCC] bg-white p-8 text-center shadow-[0_22px_60px_rgba(88,56,19,0.12)] sm:p-12">
          ...
          <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            <button
              ...
              className="min-h-12 rounded-xl bg-[#C91F32] px-6 text-xs font-black uppercase tracking-wider text-white transition hover:bg-[#AE1728] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C91F32] focus-visible:ring-offset-2"
            >
              Try Again
            </button>
            <Link
              href="/menu"
              className="grid min-h-12 place-items-center rounded-xl border border-[#C91F32] px-6 text-xs font-black uppercase tracking-wider text-[#B7192C] transition hover:bg-[#FFF2F3]"
            >
              Return to Menu
            </Link>
          </div>
      ...
      <div className="mx-auto w-full max-w-6xl">
        ...
        >
          <div className="relative overflow-hidden bg-[#FFFCF6] px-6 py-10 text-center sm:px-10 sm:py-14">
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-0 opacity-50 [background-image:radial-gradient(circle_at_12%_20%,rgba(201,154,46,0.15),transparent_25%),radial-gradient(circle_at_88%_75%,rgba(201,31,50,0.10),transparent_27%)]"
            />
            ...
            <div className="relative">
              ...
              >
                ...
                {!reduceMotion && (
                  <span
                    className="absolute inset-[-14px] animate-ping rounded-full border border-[#C91F32]/25 [animation-duration:1.8s]"
                    aria-hidden="true"
                  />
                )}
              </motion.div>

              <p className="mt-8 text-xs font-black uppercase tracking-[0.22em] text-[#A97814]">
                Payment Successful
              </p>
              <h1 className="mt-3 text-4xl font-black uppercase tracking-[-0.05em] text-[#B7192C] sm:text-6xl">
                Your Order Is Confirmed
              </h1>
              <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-[#645B52] sm:text-base">
                Thank you, {order.customerName}. Your payment was verified and
                your order has been sent to The Rolling Stove.
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-0 lg:grid-cols-[1.15fr_.85fr]">
            <section
              id="order-details"
              className="border-b border-[#EEE4D6] p-6 sm:p-9 lg:border-b-0 lg:border-r"
            >
              <div className="flex flex-wrap items-end justify-between gap-4 border-b border-[#EEE4D6] pb-6">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#A97814]">
                    Order ID
                  </p>
                  <h2 className="mt-2 text-2xl font-black uppercase text-[#171717] sm:text-3xl">
                    {order.orderNumber}
                  </h2>
                </div>
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <InfoCard
                  icon={faReceipt}
                  label="Payment ID"
                  value={maskPaymentId(payment?.paymentId)}
                />
                <InfoCard
                  icon={faShieldHalved}
                  label="Amount Paid"
                  value={money(payment?.amount ?? order.grandTotal)}
                  accent
                />
                <InfoCard
                  ...
                  value={formatTime(order.requestedPickupAt)}
                />
                <InfoCard
                  icon={faCoins}
                  label="Earned TRS Coins"
                  value={`${order.coinsEarned} Coins`}
                  gold
                />
              </div>

              <div className="mt-7 rounded-2xl border border-[#F0E2BD] bg-[#FFF7E6] p-5">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#8D6515]">
                  Estimated Preparation
                </p>
                <p className="mt-2 text-xl font-black text-[#B7192C]">
              ...
              <div className="mt-7">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-xs font-bold text-[#756C62]">
            ...
            <aside className="bg-[#FFFCF7] p-6 sm:p-9">
              ...
              <div className="mt-5 space-y-4 text-sm">
                ...
                <div className="flex items-center justify-between border-t border-[#E8DDCD] pt-5 text-xl font-black">
                  ...
                  <span className="text-[#B7192C]">
                    {money(payment?.amount ?? order.grandTotal)}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-8 grid gap-3">
                <a
                  href="#order-details"
                  className="grid min-h-12 grid-cols-[20px_1fr] items-center rounded-xl border border-[#DCCDB8] bg-white px-5 text-xs font-black uppercase tracking-wider text-[#25211E] transition hover:border-[#C91F32] hover:text-[#B7192C]"
                >
                  <FontAwesomeIcon icon={faEye} className="h-4" />
                  <span className="text-center">View Order</span>
                </a>
                <Link
                  href={trackHref}
                  className="grid min-h-12 grid-cols-[20px_1fr] items-center rounded-xl bg-[#C91F32] px-5 text-xs font-black uppercase tracking-wider text-white shadow-[0_12px_25px_rgba(201,31,50,0.2)] transition hover:bg-[#AE1728]"
                >
                ...
                <a
                  href={invoiceHref}
                  className="grid min-h-12 grid-cols-[20px_1fr] items-center rounded-xl border border-[#C91F32] bg-white px-5 text-xs font-black uppercase tracking-wider text-[#B7192C] transition hover:bg-[#FFF1F2]"
                >
                  <FontAwesomeIcon icon={faDownload} className="h-4" />
                  <span className="text-center">Download Invoice</span>
                </a>
                <Link
                  href="/menu"
                  className="grid min-h-12 grid-cols-[20px_1fr] items-center rounded-xl border border-[#C99A2E] bg-[#FFF8E9] px-5 text-xs font-black uppercase tracking-wider text-[#8A6112] transition hover:bg-[#FFF2D3]"
                >
                  <FontAwesomeIcon icon={faArrowRotateRight} className="h-4" />
                  <span className="text-center">Order Again</span>
                </Link>
              </div>
              ...
              <div className="mt-8 rounded-2xl border border-[#E9DDCC] bg-white p-5 text-xs leading-6 text-[#6A6158]">
                ...
                />
                Your payment was securely handled by Razorpay. TRS does not
...
function InfoCard({
  ...
  return (
    <div
      className={`rounded-2xl border p-5 ${gold ? "border-[#ECD9A7] bg-[#FFF8E5]" : accent ? "border-[#F0D1D5] bg-[#FFF3F4]" : "border-[#E9E0D4] bg-white"}`}
    >
      <div className="flex items-center gap-3">
        <span
          className={`grid h-10 w-10 place-items-center rounded-xl ${gold ? "bg-[#F2E2B7] text-[#93660E]" : "bg-[#FFF0F1] text-[#B7192C]"}`}
        >
          <FontAwesomeIcon icon={icon} className="h-4" />
        </span>
        <div className="min-w-0">
          <p className="text-[9px] font-black uppercase tracking-[0.14em] text-[#81776C]">
            {label}
          </p>
          <p
            className={`mt-1 truncate text-sm font-black sm:text-base ${accent ? "text-[#B7192C]" : gold ? "text-[#8A6112]" : "text-[#1C1A18]"}`}
          >
            {value}
          </p>
        </div>
...
function PriceRow({
  ...
  return (
    <div
      className={`flex items-center justify-between ${discount ? "text-[#187A38]" : ""}`}
    >
      <span>{label}</span>
