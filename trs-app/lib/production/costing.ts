...
export function calculateProductionCosts(
  ...
  const estimatedCost = roundMoney(
    inputs.reduce((sum, line) => sum + line.plannedQuantity * line.unitCost, 0),
  );

  const actualCost = roundMoney(
    inputs.reduce((sum, line) => sum + line.actualQuantity * line.unitCost, 0),
  );
  ...
  return {
    ...
    actualCost,
    costPerOutputUnit:
      totalActualOutput > 0 ? roundMoney(actualCost / totalActualOutput) : 0,
  };
}
