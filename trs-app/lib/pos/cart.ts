...
export function createModifierSignature(
  configuration: PosConfiguredItem,
): string {
  const modifierPart = [...configuration.modifiers]
    ...
    .map(
      (modifier) =>
        `${modifier.groupId}:${modifier.optionId}:${modifier.quantity}:${sanitizeMoney(modifier.unitPrice)}`,
    )
    .join("|");
  return [
    configuration.variantId ?? "base",
    modifierPart,
    configuration.specialInstructions.trim(),
  ].join("::");
}
...
export function createPosCartLine(
  ...
  const modifierTotal = normalized.modifiers.reduce(
    (total, modifier) =>
      total +
      sanitizeMoney(modifier.unitPrice) * normalizeQuantity(modifier.quantity),
    0,
  );
  return {
    lineId: createLineId(item.id, signature),
    itemId: item.id,
    source: item.source,
    name: item.name,
    slug: item.slug,
    ...
    basePrice: sanitizeMoney(normalized.basePrice),
    unitPrice: sanitizeMoney(normalized.basePrice + modifierTotal),
    quantity: 1,
    note: normalized.specialInstructions.slice(0, 240),
    variantId: normalized.variantId,
    variantName: normalized.variantName,
    modifiers: normalized.modifiers.map(normalizeModifier),
...
export function addCatalogItemToCart(
  state: PosCartState,
  item: PosCatalogItem,
  configuration?: PosConfiguredItem,
): PosCartState {
  if (!item.isAvailable || !Number.isFinite(item.price) || item.price < 0)
    return state;
...
export function updateCartLineNote(
  state: PosCartState,
  lineId: string,
  note: string,
): PosCartState {
...
export function setInternalConsumption(
  state: PosCartState,
  ...
  const internalConsumption: PosInternalConsumption = {
    ...
    managerApprovalEmail: value.managerApprovalEmail
      .trim()
      .toLowerCase()
      .slice(0, 254),
...
export function updateCartAdjustments(
  state: PosCartState,
  patch: Partial<PosCartAdjustments>,
): PosCartState {
  const adjustments = normalizeAdjustments({ ...state.adjustments, ...patch });
  return JSON.stringify(adjustments) === JSON.stringify(state.adjustments)
    ? state
    : { ...state, adjustments };
}
export function clearPosCart(state: PosCartState): PosCartState {
  if (
    !state.lines.length &&
    !state.orderNote &&
    JSON.stringify(state.adjustments) === JSON.stringify(EMPTY_POS_ADJUSTMENTS)
  )
    return state;
...
export function calculatePosCartTotals(state: PosCartState): PosCartTotals {
  ...
  const discountAmount = roundRupee(
    Math.min(base.subtotal, Math.max(0, requestedDiscount)),
  );
  ...
  const chargesTotal = roundRupee(
    packingCharge + serviceCharge + additionalCharge,
  );
  const preTaxTotal = roundRupee(netSubtotal + chargesTotal);
  const taxRate = Math.min(
    POS_MAX_TAX_PERCENT,
    Math.max(0, adjustments.taxRate),
  );
  const taxAmount =
    taxRate <= 0
      ? 0
  ...
  const taxableAmount =
    adjustments.taxMode === "inclusive"
      ? roundRupee(preTaxTotal - taxAmount)
      : preTaxTotal;
  const grandTotal = roundRupee(
    adjustments.taxMode === "inclusive" ? preTaxTotal : preTaxTotal + taxAmount,
  );
  return {
    ...base,
    subtotal: roundRupee(base.subtotal),
    discountAmount,
    netSubtotal,
    packingCharge,
    serviceCharge,
...
export function parseStoredPosCart(value: string | null): PosCartState {
  ...
  try {
    ...
    if (!isRecord(parsed) || ![1, 2, 3, 4].includes(Number(parsed.version)))
      return EMPTY_POS_CART;
    const orderType: PosOrderType =
      parsed.orderType === "dine_in" ? "dine_in" : "takeaway";
    const orderNote =
      typeof parsed.orderNote === "string"
        ? parsed.orderNote.slice(0, 500)
        : "";
    const lines = (Array.isArray(parsed.lines) ? parsed.lines : []).flatMap(
      (value): PosCartLine[] => {
        if (!isRecord(value)) return [];
        const itemId = asNonEmptyString(value.itemId),
        ...
        const modifiers = Array.isArray(value.modifiers)
          ? value.modifiers.flatMap(parseModifier)
          : [];
        const modifierSignature =
          typeof value.modifierSignature === "string"
            ? value.modifierSignature
            : "";
        ...
        return [
          {
            lineId:
              asNonEmptyString(value.lineId) ??
              createLineId(itemId, modifierSignature),
            itemId,
            source: value.source === "pos" ? "pos" : "menu",
            name,
            slug,
            ...
            categoryName:
              typeof value.categoryName === "string"
                ? value.categoryName
                : "Other",
            basePrice: sanitizeMoney(value.basePrice ?? value.unitPrice),
            unitPrice: sanitizeMoney(value.unitPrice),
            quantity,
            note:
              typeof value.note === "string" ? value.note.slice(0, 240) : "",
            variantId:
    ...
    const adjustments = isRecord(parsed.adjustments)
      ? normalizeAdjustments(parsed.adjustments)
      : EMPTY_POS_ADJUSTMENTS;
    const rawInternal = isRecord(parsed.internalConsumption)
      ? parsed.internalConsumption
      : {};
    const allowedSaleTypes = new Set([
      "customer",
      "staff_meal",
      "family_meal",
      "complimentary",
    ...
    const saleType =
      typeof rawInternal.saleType === "string" &&
      allowedSaleTypes.has(rawInternal.saleType)
        ? (rawInternal.saleType as PosInternalConsumption["saleType"])
        : "customer";
    const internalConsumption: PosInternalConsumption = {
      saleType,
      referenceId:
        typeof rawInternal.referenceId === "string" && rawInternal.referenceId
          ? rawInternal.referenceId
          : null,
      personName:
        typeof rawInternal.personName === "string"
          ? rawInternal.personName.slice(0, 120)
          : "",
      reason:
        typeof rawInternal.reason === "string"
          ? rawInternal.reason.slice(0, 240)
          : "",
      notes:
        typeof rawInternal.notes === "string"
          ? rawInternal.notes.slice(0, 500)
          : "",
      managerApprovalEmail: "",
      managerApprovalPassword: "",
      managerApprovalReason: "",
    };
    const customer = isRecord(parsed.customer)
      ? normalizeCustomer(parsed.customer as unknown as PosCustomer)
      : WALK_IN_POS_CUSTOMER;
    return {
      version: 4,
      orderType,
      internalConsumption,
      ...
      lines,
      adjustments:
        saleType === "customer" ? adjustments : EMPTY_POS_ADJUSTMENTS,
    };
  } catch {
    return EMPTY_POS_CART;
  }
...
function normalizeCustomer(
  value: PosCustomer | Record<string, unknown>,
): PosCustomer {
  ...
  const name =
    typeof value.name === "string" && value.name.trim()
      ...
      : "Walk-in customer";
  const phone =
    typeof value.phone === "string" ? value.phone.trim().slice(0, 20) : "";
  const email =
    typeof value.email === "string"
      ? value.email.trim().toLowerCase().slice(0, 160)
      : "";
...
function normalizeModifier(modifier: PosSelectedModifier): PosSelectedModifier {
  return {
    ...modifier,
    quantity: Math.max(1, normalizeQuantity(modifier.quantity)),
    unitPrice: sanitizeMoney(modifier.unitPrice),
  };
}
function parseModifier(value: unknown): PosSelectedModifier[] {
  if (!isRecord(value)) return [];
  const groupId = asNonEmptyString(value.groupId),
    groupName = asNonEmptyString(value.groupName),
    optionId = asNonEmptyString(value.optionId),
    optionName = asNonEmptyString(value.optionName);
  ...
  return [
    {
      ...
      optionId,
      optionName,
      quantity: Math.max(1, normalizeQuantity(value.quantity)),
      unitPrice: sanitizeMoney(value.unitPrice),
    },
  ];
}
function normalizeAdjustments(
  value: Partial<PosCartAdjustments> | Record<string, unknown>,
): PosCartAdjustments {
  const discountType: PosDiscountType =
    value.discountType === "fixed" || value.discountType === "percentage"
      ? value.discountType
      : "none";
  const taxMode: PosTaxMode =
    value.taxMode === "inclusive" ? "inclusive" : "exclusive";
  const discountValue =
    discountType === "percentage"
      ? clampMoney(value.discountValue, POS_MAX_DISCOUNT_PERCENT)
      : sanitizeMoney(value.discountValue);
  return {
    discountType,
    discountValue,
    discountReason:
      typeof value.discountReason === "string"
        ? value.discountReason.slice(0, 120)
        : "",
    packingCharge: sanitizeMoney(value.packingCharge),
    serviceCharge: sanitizeMoney(value.serviceCharge),
    additionalCharge: sanitizeMoney(value.additionalCharge),
    additionalChargeLabel:
      typeof value.additionalChargeLabel === "string" &&
      value.additionalChargeLabel.trim()
        ? value.additionalChargeLabel.trim().slice(0, 60)
        : "Additional charge",
    taxRate: clampMoney(value.taxRate, POS_MAX_TAX_PERCENT),
    taxMode,
  };
}
function clampMoney(value: unknown, max: number): number {
  return Math.min(max, sanitizeMoney(value));
}
function normalizeQuantity(value: unknown): number {
  const n = typeof value === "number" ? value : Number(value);
  if (!Number.isFinite(n)) return 0;
  return Math.max(0, Math.min(POS_CART_MAX_QUANTITY, Math.floor(n)));
}
