...
function writeQueue(queue: QueuedPosSale[]) {
  ...
  window.dispatchEvent(
    new CustomEvent("trs-pos-offline-sales-changed", { detail: queue.length }),
  );
}

export function queuedPosSaleCount() {
  return readQueue().length;
}

export function queuePosSale(
  operationId: string,
  payload: Record<string, unknown>,
  ...
  try {
    ...
    const next = [
      ...
      ...(Array.isArray(current) ? current : []).filter(
        (item) => item.operationId !== sale.operationId,
      ),
    ].slice(0, 25);
    window.localStorage.setItem(SYNCED_KEY, JSON.stringify(next));
  } catch {
    window.localStorage.setItem(SYNCED_KEY, JSON.stringify([sale]));
  }
}
    ...
    try {
      ...
      if (!response.ok) {
        ...
        remaining.push({
          ...
          lastError: json.message || "Sale sync failed.",
        });
        continue;
      }
