...
function paymentDisplay(invoice: ReceiptInvoice) {
  ...
  if (parts.length > 0) {
    return [
      ...new Set(parts.map((part) => formatPaymentMethod(part.method))),
    ].join(" + ");
  }
...
function kotActionLabel(action: ReceiptInvoice["items"][number]["kotAction"]) {
  const labels = {
    initial: "PREPARE",
    addition: "ADD",
    modification: "UPDATE",
...
function itemDetails(item: ReceiptInvoice["items"][number]) {
  return [
    ...
    ...(item.modifiers ?? []).map((m) =>
      [m.groupName, m.optionName].filter(Boolean).join(": "),
    ),
  ]
    .filter(Boolean)
    .map(String);
...
function kotTicket(invoice: ReceiptInvoice, options: Required<KotOptions>) {
  const printedAt = new Date();

  const items = invoice.items
    .map((item) => {
      const details = itemDetails(item);
      const action = item.kotAction ?? invoice.kotType ?? "initial";
      ...
      return `<div class="item">
        <div class="label">${escapeInvoiceHtml(kotActionLabel(action))}</div>
        <div class="item-head">
          <div>
            <div class="item-name">${escapeInvoiceHtml(item.name)}</div>
            ${
              details.length
        ...
        ${
          item.previousQuantity !== undefined || item.newQuantity !== undefined
            ? `<div class="details"><strong>Previous:</strong> ${
                item.previousQuantity ?? "—"
              } &nbsp; <strong>New:</strong> ${item.newQuantity ?? "—"}</div>`
            : ""
        }
        ${
        ...
        ${
          options.showPrices
            ? `<div class="details">${escapeInvoiceHtml(
                formatInvoiceMoney(item.lineTotal, invoice.currency ?? "INR"),
              )}</div>`
            : ""
        }
      </div>`;
  ...
  const orderNoteChanged =
    ...
    invoice.kotOrderNote !== invoice.kotPreviousOrderNote;

  return `<section class="ticket">
    <div class="center">
      <div class="brand">THE ROLLING STOVE</div>
      <div class="label">${escapeInvoiceHtml(kotHeading(invoice.kotType))}</div>
      ${
        invoice.kotRevision
    ...
    <div class="token">
      ...
      <strong>${escapeInvoiceHtml(
        token(invoice.orderNumber, invoice.kitchenToken),
      )}</strong>
    </div>
    ...
    ${items || `<div class="instructions">ORDER NOTE UPDATED</div>`}
...
function thermalInvoiceTicket(
  invoice: ReceiptInvoice,
  options: Required<InvoiceOptions>,
) {
  const business = invoice.businessSnapshot ?? {};
  const businessName =
    business.tradeName || business.legalName || "The Rolling Stove";
        ...
        paymentParts(invoice).length > 1
          ? `<div class="thermal-invoice-payment-block">
              ...
              ${paymentParts(invoice)
                .map(
                  (part) =>
                    `<div class="thermal-invoice-row"><span>${escapeInvoiceHtml(
...
export function renderKotHtml(
  ...
  return shell(
    ...
    resolved.paper,
    Array.from({ length: resolved.copies }, () =>
      kotTicket(invoice, resolved),
    ).join(""),
...
export function renderThermalInvoiceHtml(
  ...
  return shell(
    ...
    resolved.paper,
    Array.from({ length: resolved.copies }, () =>
      thermalInvoiceTicket(invoice, resolved),
    ).join(""),
  );
}

export function renderCompactReceiptHtml(
  invoice: ReceiptInvoice,
  format: "thermal" | "kitchen",
) {
  return format === "kitchen"
    ? renderKotHtml(invoice)
    : renderThermalInvoiceHtml(invoice);
}

export function renderKotCopiesHtml(invoice: ReceiptInvoice) {
  return renderKotHtml(invoice);
}
...
export function renderKotAndInvoiceHtml(invoice: ReceiptInvoice) {
  ...
  const main =
    branded.match(/<main class="invoice-shell">[\s\S]*?<\/main>/i)?.[0] ?? "";
