...
export function getAuthCookieOptions(): Partial<ResponseCookie> {
  ...
  return {
    httpOnly: true,
    secure: isProduction && authConfig.AUTH_COOKIE_SECURE === "true",
    sameSite: "lax",
    path: "/",
    ...(isProduction && authConfig.AUTH_COOKIE_DOMAIN
      ? {
...
export async function setAuthCookies(
  ...
  cookieStore.set(ACCESS_COOKIE, accessToken, {
    ...options,
    maxAge: authConfig.ACCESS_TOKEN_TTL_SECONDS,
  });
  ...
  cookieStore.set(REMEMBER_COOKIE, rememberMe ? "true" : "false", {
    ...options,
    ...(rememberMe
      ? {
          maxAge: authConfig.REFRESH_TOKEN_TTL_SECONDS,
...
export async function getRememberMePreference(): Promise<boolean> {
  ...
  return cookieStore.get(REMEMBER_COOKIE)?.value === "true";
}
