...
export async function createReportWorkbook(
  ...
  sheet.addRow([title]);
  sheet.mergeCells(1, 1, 1, Math.max(result.columns.length, 1));
  ...
  sheet.columns.forEach((column) => {
    column.width = Math.min(
      ...
      Math.max(
        12,
        ...(column.values ?? []).map((value) => display(value).length + 2),
      ),
    );
  });
...
export async function createReportPdf(
  ...
  const header = () => {
    ...
    y -= 22;
    page.drawText(title.slice(0, 90), { x: margin, y, size: 12, font: bold });
    y -= 18;
    page.drawText(
      `Generated ${new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}`,
      { x: margin, y, size: 8, font: regular, color: rgb(0.35, 0.35, 0.35) },
    );
    y -= 22;
    columns.forEach((column, index) =>
      page.drawText(column.label.slice(0, 18), {
  ...
  for (const row of result.rows) {
    ...
    columns.forEach((column, index) =>
      page.drawText(display(row[column.key]).slice(0, 22), {
        x: margin + index * columnWidth,
        y,
        size: 6.5,
        font: regular,
