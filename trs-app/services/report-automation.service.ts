...
export async function getReportAutomationMonitor() {
  ...
  const staleBefore = new Date(
    Date.now() - Number(settings?.staleWorkerMinutes ?? 15) * 60_000,
  );
  const [
    statusRows,
    deliveryRows,
    ...
    ScheduledReport.aggregate<{ _id: string; count: number }>([
      { $match: { deletedAt: null } },
      { $group: { _id: "$status", count: { $sum: 1 } } },
    ]),
    ReportJob.aggregate<{ _id: null; bytes: number; files: number }>([
      { $match: { outputKey: { $ne: "" } } },
      {
        $group: {
          _id: null,
          bytes: { $sum: "$outputSize" },
          files: { $sum: 1 },
        },
      },
    ]),
    ReportJob.find({ status: "failed" })
      .select("errorMessage outputFilename failedAt reportId")
  ...
  const deliveries = Object.fromEntries(
    deliveryRows.map((row) => [row._id || "pending", row.count]),
  );
  const scheduleCounts = Object.fromEntries(
    schedules.map((row) => [row._id, row.count]),
  );
  ...
  return {
    ...
    successRate: totalFinished
      ? Math.round(((jobs.completed || 0) / totalFinished) * 1000) / 10
      : 100,
    queueHealthy:
      (jobs.queued || 0) < Number(settings?.queueWarningThreshold ?? 100) &&
      staleProcessing === 0,
    recentFailures,
  };
}
...
export async function cleanupExpiredReportArtifacts() {
  const settings = await getReportAutomationSettings();
  const artifactBefore = new Date(
    Date.now() - Number(settings?.artifactRetentionDays ?? 30) * 86_400_000,
  );
  const failedBefore = new Date(
    Date.now() - Number(settings?.failedJobRetentionDays ?? 90) * 86_400_000,
  );
  const artifacts = await ReportJob.find({
    status: "completed",
    completedAt: { $lt: artifactBefore },
    outputKey: { $ne: "" },
  })
    .select("outputKey")
    .limit(1000);
  ...
  const removed = await ReportJob.deleteMany({
    status: { $in: ["failed", "cancelled"] },
    updatedAt: { $lt: failedBefore },
  });
  return {
    artifactsRemoved: artifacts.length,
    jobsRemoved: removed.deletedCount,
  };
}

export function reportArtifactsBucketName() {
  return "report-job-artifacts";
}
export function reportArtifactsCollectionName() {
  return `${reportArtifactsBucketName()}.files`;
}
