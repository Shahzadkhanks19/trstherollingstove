...
} from "@/services/report-builder-export.service";
import {
  storeReportJobArtifact,
  deleteReportJobArtifact,
} from "@/services/report-job-artifact.service";
...
function failureMessage(error: unknown): string {
  return error instanceof Error
    ...
    : "Unknown scheduled report error.";
}
...
export async function enqueueDueScheduledReports(
  ...
  for (const schedule of due) {
    ...
    const existing = await ReportJob.findOne({
      deduplicationKey,
    ...
    if (!next && normalized.frequency === "one_time")
...
async function executeClaimedJob(
  job: NonNullable<Awaited<ReturnType<typeof claimNextReportJob>>>,
  workerId: string,
) {
  ...
  try {
    ...
    const result = await executeReportPreview(
      {
        ...
        limit: Number(
          (await getReportAutomationSettings())?.maxRowsPerReport ?? 5000,
        ),
      },
    ...
    const bytes =
      format === "csv"
        ? createReportCsv(result)
        : format === "xlsx"
          ? await createReportWorkbook(report.name, result)
          : await createReportPdf(report.name, result);
    ...
    const artifact = await storeReportJobArtifact({
      bytes,
      filename,
      contentType,
    ...
    const completion = await ReportJob.updateOne(
    ...
    if (completion.modifiedCount !== 1) {
      await deleteReportJobArtifact(artifact.key);
      return "cancelled" as const;
    }
    const settings = await getReportAutomationSettings();
    let deliveryStatus: "pending" | "skipped" | "sent" | "failed" = job
      .recipients.length
      ? "pending"
      : "skipped";
    let deliveryError = "";
    ...
    if (job.recipients.length && settings?.emailDeliveryEnabled !== false) {
      try {
        await deliverReportJobArtifact({
          outputKey: artifact.key,
          outputFilename: filename,
        ...
        deliveryError = failureMessage(deliveryFailure);
      }
    } else if (job.recipients.length) deliveryStatus = "skipped";
    await ReportJob.updateOne(
      { _id: job._id },
      {
        $set: { deliveryStatus, deliveredAt, deliveryError },
        $inc: { deliveryAttempts: job.recipients.length ? 1 : 0 },
      },
    );
    if (job.scheduleId) {
      await Promise.all([
        ScheduledReport.updateOne(
          { _id: job.scheduleId },
          {
            $inc: { runCount: 1 },
            $set: { lastJobId: job._id, lastRunAt: completedAt },
          },
        ),
        ReportScheduleAudit.create({
          scheduleId: job.scheduleId,
          jobId: job._id,
          action:
            deliveryStatus === "failed" ? "delivery_failed" : "job_completed",
    ...
    if (settings?.notificationOnSuccess !== false) {
      await sendNotification({
        recipientId: String(job.requestedBy),
        ...
        message:
          deliveryStatus === "failed"
            ? `${report.name} was generated, but email delivery failed: ${deliveryError}`
            : `${report.name} completed with ${result.rowCount} rows.`,
        actionUrl: "/admin/report-jobs",
        metadata: { jobId: String(job._id), deliveryStatus },
        channels: ["in_app"],
      }).catch(() => undefined);
    }
    ...
    if (job.scheduleId) {
      await Promise.all([
        shouldRetry
          ...
          : ScheduledReport.updateOne(
              { _id: job.scheduleId },
              { $inc: { failureCount: 1 }, $set: { lastJobId: job._id } },
            ),
        ReportScheduleAudit.create({
          scheduleId: job.scheduleId,
          jobId: job._id,
          action: shouldRetry ? "job_retried" : "job_failed",
          actorId: job.requestedBy,
          metadata: {
            attempt: job.attempt,
    ...
    if (!shouldRetry) {
      const settings = await getReportAutomationSettings();
      if (settings?.notificationOnFailure !== false)
        await sendNotification({
          recipientId: String(job.requestedBy),
          eventKey: "scheduled_report.failed",
          category: "transactional",
          type: "system",
...
export async function runReportJobWorker(limit = 10) {
  ...
  for (
    let index = 0;
    index < Math.min(Math.max(limit, 1), MAX_BATCH);
    index += 1
  ) {
...
export async function getReportJobQueueSummary() {
  ...
  const [statusRows, averageRows, staleProcessing] = await Promise.all([
    ReportJob.aggregate<{ _id: string; count: number }>([
      { $group: { _id: "$status", count: { $sum: 1 } } },
    ]),
    ReportJob.aggregate<{ _id: null; averageDurationMs: number }>([
...
export async function retryReportJob(id: string, actorId: string) {
  ...
  if (!Types.ObjectId.isValid(actorId))
    throw new AppError("Invalid actor.", 422);
  job.status = "queued";
