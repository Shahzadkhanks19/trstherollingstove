...
import {
  createChangedKotRevision,
  createInitialKotRevision,
...
function isActive(status: string) {
  return ["open", "sent_to_kitchen", "partially_paid"].includes(status);
}
...
function normalizeRunningOrderCartVariants(cart: PosCartState): PosCartState {
  ...
  const lines = cart.lines.map((line) => {
    ...
    if (line.source !== "menu" || line.variantId !== syntheticBaseVariantId)
      return line;
    changed = true;
...
async function syncRunningOrderKitchen(
  order: {
    _id: Types.ObjectId;
    ticketNumber: string;
    tableName: string;
  ...
  const menuItemIds = order.cart.lines
    .filter(
      (line) => line.source === "menu" && Types.ObjectId.isValid(line.itemId),
    )
  ...
  return tables.map((table) => {
    ...
    return {
      ...
      elapsedMinutes: order
        ? Math.max(
            0,
            Math.floor((Date.now() - order.openedAt.getTime()) / 60000),
          )
        : 0,
...
export async function updateTable(
  ...
  if (typeof patch.reservationTime === "string")
    patch.reservationTime = new Date(patch.reservationTime);
...
export async function listRunningOrders() {
  const rows = await POSRunningOrder.find({
    status: { $in: ["open", "sent_to_kitchen", "partially_paid"] },
  })
    .sort({ updatedAt: -1 })
    .lean();
  return rows.map((row) => ({
    ...
    status: row.status,
    cart: row.cart,
    totals: calculatePosCartTotals(row.cart),
    kitchenSentAt: row.kitchenSentAt?.toISOString() ?? null,
...
export async function createRunningOrder(
  input: {
    shiftId: string;
    tableId: string | null;
    tableName: string;
  ...
  const shift = await POSShift.findOne({
    _id: input.shiftId,
    status: "open",
    openedBy: oid(actorId),
  }).lean();
  if (!shift) throw new AppError("Open shift not found for this cashier.", 409);
  let tableName = input.tableName.trim();
  if (input.tableId) {
    const table = await POSTable.findOne({
      _id: input.tableId,
      isActive: true,
    }).lean();
    if (!table) throw new AppError("Selected table is unavailable.", 409);
    if (table.status === "out_of_service")
      throw new AppError("Selected table is out of service.", 409);
    const occupied = await POSRunningOrder.exists({
      tableId: table._id,
      status: { $in: ["open", "sent_to_kitchen", "partially_paid"] },
    });
    if (occupied)
      throw new AppError("Selected table already has a running order.", 409);
  ...
  const order = await POSRunningOrder.create({
    ...
    kitchenToken: identity.kitchenToken,
    shiftId: shift._id,
    registerId: shift.registerId,
    cashierId: oid(actorId),
    tableId: input.tableId ? oid(input.tableId) : null,
    tableName,
    guestCount: input.guestCount,
    cart: normalizedCart,
    status: "open",
    kitchenRevision: kotRevision.revision,
    lastKitchenCart: normalizedCart,
  ...
  await audit(
    actorId,
    "running_order.created",
    "running_order",
    order._id,
    "",
    null,
    { ticketNumber: order.ticketNumber, kitchenRevision: kotRevision.revision },
  );
  emit("running_order_created", String(order._id), actorId, {
...
export async function updateRunningOrder(
  id: string,
  input: { cart: PosCartState; guestCount?: number; sendToKitchen: boolean },
  actorId: string,
) {
  ...
  const normalizedCart = normalizeRunningOrderCartVariants(input.cart);
  const previousKitchenCart = normalizeRunningOrderCartVariants(
    current.lastKitchenCart ?? current.cart,
  );
  const before = {
    cart: current.cart,
    guestCount: current.guestCount,
    status: current.status,
    kitchenRevision: current.kitchenRevision,
  };
  const kotRevision = input.sendToKitchen
    ? createChangedKotRevision(
        previousKitchenCart,
        normalizedCart,
        Number(current.kitchenRevision ?? 0) + 1,
        actorId,
      )
    : null;
  ...
  await audit(
    actorId,
    input.sendToKitchen && kotRevision
      ? "running_order.sent_to_kitchen"
      : "running_order.updated",
    "running_order",
    current._id,
    "",
    before,
    {
      status: current.status,
      kitchenRevision: current.kitchenRevision,
      kitchenChanged: Boolean(kotRevision),
    },
  );
  emit(
    input.sendToKitchen && kotRevision
...
export async function transferRunningOrder(
  id: string,
  tableId: string | null,
  ...
  if (tableId) {
    ...
    if (!table || table.status === "out_of_service")
      throw new AppError("Destination table is unavailable.", 409);
    const occupied = await POSRunningOrder.exists({
      _id: { $ne: order._id },
      tableId: table._id,
      status: { $in: ["open", "sent_to_kitchen", "partially_paid"] },
  ...
  emit("running_order_transferred", id, actorId, {
    tableName: order.tableName,
  });
  return order;
}
...
export async function mergeRunningOrders(
  targetId: string,
  sourceId: string,
  actorId: string,
) {
  if (targetId === sourceId)
    throw new AppError("An order cannot be merged into itself.", 422);
  const [target, source] = await Promise.all([
    POSRunningOrder.findById(targetId),
    POSRunningOrder.findById(sourceId),
  ]);
  if (
    !target ||
    !source ||
    ...
    throw new AppError("One of the running orders is unavailable.", 409);
  target.cart = {
    ...target.cart,
    lines: combineLines(target.cart.lines, source.cart.lines),
    orderNote: [target.cart.orderNote, source.cart.orderNote]
      .filter(Boolean)
      .join(" | "),
  };
  ...
  await audit(
    actorId,
    "running_order.merged",
    "running_order",
    target._id,
    `Merged ${source.ticketNumber}`,
    null,
...
export async function splitRunningOrder(
  id: string,
  ...
  if (!source || !isActive(source.status))
    throw new AppError("Running order not found.", 404);
  ...
  for (const line of source.cart.lines) {
    ...
    if (requested > line.quantity)
      throw new AppError(
        `Cannot move more than the available quantity of ${line.name}.`,
        422,
      );
    if (requested > 0) moved.push({ ...line, quantity: requested });
    if (line.quantity - requested > 0)
      remaining.push({ ...line, quantity: line.quantity - requested });
  }
  if (!moved.length || !remaining.length)
    throw new AppError("A split must leave items on both orders.", 422);
  let tableName = "";
  if (targetTableId) {
    const table = await POSTable.findById(targetTableId).lean();
    if (!table || table.status === "out_of_service")
      throw new AppError("Target table is unavailable.", 409);
    const occupied = await POSRunningOrder.exists({
      tableId: table._id,
      status: { $in: ["open", "sent_to_kitchen", "partially_paid"] },
    });
  ...
  const split = await POSRunningOrder.create({
    ticketNumber: splitIdentity.ticketNumber,
    kitchenToken: splitIdentity.kitchenToken,
    shiftId: source.shiftId,
    registerId: source.registerId,
    cashierId: source.cashierId,
    tableId: targetTableId ? oid(targetTableId) : null,
    tableName,
    guestCount: 1,
    cart: { ...source.cart, lines: moved },
    status: source.status,
    kitchenSentAt: source.kitchenSentAt,
    createdBy: oid(actorId),
    updatedBy: oid(actorId),
  });
  await source.save();
  await audit(
    ...
    "",
    null,
    { splitOrderId: String(split._id) },
  );
  emit("running_order_split", id, actorId, { splitOrderId: String(split._id) });
  return { source, split };
}

export async function voidRunningItem(
  id: string,
  lineId: string,
  quantity: number,
  reason: string,
  ...
  if (!line || quantity > line.quantity)
    throw new AppError("The selected item quantity is unavailable.", 422);
  const before = { lineId, quantity: line.quantity };
  const lines = order.cart.lines.flatMap((entry) =>
    entry.lineId !== lineId
      ? [entry]
...
export async function cancelRunningOrder(
  id: string,
  reason: string,
  actorId: string,
) {
  ...
  if (order.status === "partially_paid") {
    throw new AppError(
      "A partially paid order cannot be cancelled. Settle or refund the payment first.",
      409,
    );
  }
  ...
  emit("running_order_cancelled", id, actorId, {
    ticketNumber: order.ticketNumber,
    reason,
  });
  return order;
...
export async function settleRunningOrder(
  ...
  if (!running || !isActive(running.status))
    throw new AppError("Running order not found.", 404);
  const settlementCart = normalizeRunningOrderCartVariants(running.cart);
  const result = await createPosOrder(
    {
      shiftId: String(running.shiftId),
      orderMode: settlementCart.orderType,
      tableNumber: running.tableName,
      internalConsumption: settlementCart.internalConsumption,
      customerId: settlementCart.customer.id || null,
      customerName: settlementCart.customer.name,
      customerPhone: settlementCart.customer.phone,
      customerEmail: settlementCart.customer.email,
      customerNote: settlementCart.orderNote,
                ...
                amount:
                  payment.paymentMethod === "cash"
                    ...
                    : calculatePosCartTotals(settlementCart).grandTotal +
                      (payment.tipCollection === "restaurant"
                        ? payment.tipAmount
                        : 0),
                reference:
                  payment.paymentMethod === "upi" ? payment.upiReference : "",
              },
            ],
      waivedAmount: 0,
      waivedReason: "",
      ...
      items: settlementCart.lines.map((line) => ({
  ...
  emit("running_order_settled", id, actorId, {
    orderId: String(result.order._id),
  });
  return result;
}
...
export async function refundOrder(
  ...
  if (!order || order.orderSource !== "pos" || order.paymentStatus !== "paid")
    throw new AppError("A paid POS order is required for a refund.", 409);
  if (input.idempotencyKey) {
    const replay = await POSRefund.findOne({
      orderId: order._id,
      idempotencyKey: input.idempotencyKey,
    });
    if (replay) return replay;
  }
  const existing = await POSRefund.aggregate<{ total: number }>([
    { $match: { orderId: order._id, status: "approved" } },
    { $group: { _id: null, total: { $sum: "$amount" } } },
  ]);
  const alreadyRefunded = existing[0]?.total ?? 0;
  if (input.amount > order.grandTotal - alreadyRefunded)
    throw new AppError(
  ...
  for (const line of input.lines) {
    ...
    if (!item || line.quantity > item.quantity)
      throw new AppError("A refund line exceeds the sold quantity.", 422);
  }
  const refund = await POSRefund.create({
    ...
    lines: input.lines.map((line) => {
      const item = order.items.id(line.orderItemId);
      return { ...line, name: item?.name ?? "Item" };
    }),
    approvedBy: oid(actorId),
    createdBy: oid(actorId),
  });
  if (input.method === "cash" && order.posShiftId) {
    await POSCashMovement.create({
      ...
      type: "cash_out",
      amount: input.amount,
      reason: `Refund ${order.orderNumber}: ${input.reason}`,
      referenceType: "refund",
      referenceId: refund._id,
      createdBy: oid(actorId),
    });
    await POSShift.updateOne(
      { _id: order.posShiftId, status: "open" },
      { $inc: { expectedCash: -input.amount } },
    );
  }
  ...
  await audit(
    ...
    input.reason,
    null,
    { refundId: String(refund._id), amount: input.amount, fullyRefunded },
  );
  emit("order_refunded", orderId, actorId, {
    amount: input.amount,
    fullyRefunded,
  });
  publishRealtimeEventSafely({
    event: "pos.refund_created",
    entityId: String(refund._id),
    actorId,
    data: { orderId, amount: input.amount, fullyRefunded },
...
export async function getShiftReport(shiftId: string) {
  ...
  const [orders, movements, refunds] = await Promise.all([
    Order.find({ posShiftId: shift._id })
      .select(
        "grandTotal paymentMethod paymentStatus taxTotal discountTotal itemCount createdAt",
      )
      .lean(),
    POSCashMovement.find({ shiftId: shift._id })
      .populate("createdBy", "name email")
      .sort({ createdAt: 1 })
      .lean(),
    POSRefund.find({
      orderId: {
        $in: await Order.find({ posShiftId: shift._id }).distinct("_id"),
      },
    }).lean(),
  ]);
  ...
  const byMethod = paid.reduce(
    (acc, order) => {
      acc[order.paymentMethod] =
        (acc[order.paymentMethod] ?? 0) + order.grandTotal;
      return acc;
    },
  ...
  return {
    ...
    refundsTotal: refunds.reduce((sum, refund) => sum + refund.amount, 0),
    byMethod,
    movements,
...
export async function cancelPaidPosOrder(
  ...
  const order = await Order.findById(orderId);
  if (!order || order.orderSource !== "pos")
    throw new AppError("POS order not found.", 404);
  if (order.status === "cancelled" || order.paymentStatus === "refunded") {
    throw new AppError(
      "This order has already been cancelled or fully refunded.",
      409,
    );
  }
  if (order.paymentStatus !== "paid")
    throw new AppError(
      "Only a paid POS order can be cancelled from bill history.",
      409,
    );
  ...
  emit("cancelled_after_payment", orderId, actorId, {
