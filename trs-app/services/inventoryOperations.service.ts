...
export async function createPurchaseReturn(input: {
  ...
  supplierCreditExpected: boolean;
  ...
  const items = await InventoryItem.find({
    _id: { $in: input.items.map((item) => oid(item.inventoryItemId)) },
    isActive: true,
  }).lean();
  if (items.length !== input.items.length)
    throw new AppError("One or more inventory items are unavailable.", 409);
  ...
  return PurchaseReturn.create({
    returnNumber: nextNumber("PR"),
    supplierId: supplier._id,
    purchaseOrderId: input.purchaseOrderId ? oid(input.purchaseOrderId) : null,
    items: lines,
    subtotal,
    taxTotal: 0,
    grandTotal: subtotal,
    supplierCreditExpected: input.supplierCreditExpected,
    creditNoteNumber: input.creditNoteNumber,
    notes: input.notes,
    createdBy: oid(input.actorId),
    updatedBy: oid(input.actorId),
  });
}
...
export async function approvePurchaseReturn(id: string, actorId: string) {
  ...
  for (const line of entry.items) {
    await recordInventoryMovement({
      ...
      reason: line.reason || `Purchase return ${entry.returnNumber}`,
...
export async function createStockTransfer(input: {
  ...
  const lines = input.items.map((line) => {
    const item = byId.get(line.inventoryItemId);
    if (!item) throw new AppError("Inventory item not found.", 404);
    return { ...line, itemName: item.name, unitCost: item.averageUnitCost };
  });
  return StockTransfer.create({
    transferNumber: nextNumber("ST"),
    fromWarehouseId: oid(input.fromWarehouseId),
    toWarehouseId: oid(input.toWarehouseId),
    items: lines,
    notes: input.notes,
    createdBy: oid(input.actorId),
    updatedBy: oid(input.actorId),
...
export async function createStockCount(input: {
  ...
  const lines = input.items.map((line) => {
    const item = byId.get(line.inventoryItemId);
    if (!item) throw new AppError("Inventory item not found.", 404);
    const varianceQuantity = line.countedQuantity - item.currentStock;
    return {
      ...line,
      itemName: item.name,
      ...
      varianceQuantity,
      unitCost: item.averageUnitCost,
      varianceValue: varianceQuantity * item.averageUnitCost,
    };
  });
  return StockCount.create({
    countNumber: nextNumber("SC"),
    warehouseId: input.warehouseId ? oid(input.warehouseId) : null,
...
export async function postStockCount(id: string, actorId: string) {
  ...
  for (const line of count.items) {
    ...
    await recordInventoryMovement({
      inventoryItemId: String(line.inventoryItemId),
      type: line.varianceQuantity > 0 ? "adjustment_in" : "adjustment_out",
      quantity: Math.abs(line.varianceQuantity),
      unitCost: line.unitCost,
      referenceType: "stock_count",
      referenceId: String(count._id),
      reason: line.reason || `Stock count ${count.countNumber}`,
      actorId,
    });
  }
  count.status = "posted";
...
export async function createWasteEntry(input: {
  inventoryItemId: string;
  warehouseId: string | null;
  quantity: number;
  wasteType:
    "spoilage" | "expiry" | "damage" | "production_loss" | "theft" | "other";
  batchNumber: string;
  reason: string;
  occurredAt?: Date;
  actorId: string;
  ...
  const waste = await WasteEntry.create({
    wasteNumber: nextNumber("WE"),
    inventoryItemId: item._id,
    warehouseId: input.warehouseId ? oid(input.warehouseId) : null,
    quantity: input.quantity,
    unitCost: item.averageUnitCost,
    totalCost: input.quantity * item.averageUnitCost,
