...
export async function generateProcurementIntelligence(input: {
  ...
  try {
    const forecastRun = await ensureForecast({
      ...input,
    ...
    const [
      ...
      InventoryForecastSnapshot.find({ runId: forecastRun._id })
        .select(
          "inventoryItemId itemName sku category unit currentStock forecastDailyDemand forecastWeeklyDemand forecastMonthlyDemand trendPercent safetyStock reorderPoint recommendedOrderQuantity recommendedOrderValue daysUntilStockout expectedStockoutDate confidenceScore riskLevel velocityClass",
        )
        .lean<SnapshotRow[]>(),
      ...
      InventoryMovement.aggregate<WastageRow>([
        { $match: { type: "wastage", createdAt: { $gte: from30 } } },
        {
          $group: {
            ...
            wastageQuantity: { $sum: "$quantity" },
            wastageValue: { $sum: "$totalCost" },
          },
        },
        {
          $project: {
      ...
      InventoryMovement.aggregate<ExpiryRow>([
        {
          $match: {
            ...
            type: {
              $in: ["opening", "purchase", "adjustment_in", "return_in"],
            },
          },
        },
        {
    ...
    const openMap = new Map(
      openPoRows.map((row) => [row.inventoryItemId, row]),
    );
    const historyMap = new Map(
      supplierHistory.map((row) => [row.inventoryItemId, row]),
    );
    const wasteMap = new Map(
      wastageRows.map((row) => [row.inventoryItemId, row]),
    );
    const expiryMap = new Map(
      expiryRows.map((row) => [row.inventoryItemId, row]),
    );
    ...
    const recommendations = snapshots
      .map<ProcurementRecommendation>((snapshot) => {
        ...
        const unitCost =
          history?.unitCost && history.unitCost > 0
            ? Number(history.unitCost)
            : grossQuantity > 0
              ? Number(snapshot.recommendedOrderValue ?? 0) / grossQuantity
              : 0;
        const flags: string[] = [];
        if (snapshot.velocityClass === "inactive" && snapshot.currentStock > 0)
          flags.push("dead_stock");
        if (
          snapshot.currentStock >
            Math.max(
              snapshot.reorderPoint * 2,
              snapshot.forecastMonthlyDemand * 2,
            ) &&
          snapshot.forecastDailyDemand > 0
        )
          flags.push("overstock");
        if (
          Number(waste?.wastageQuantity ?? 0) >
            snapshot.forecastMonthlyDemand * 0.1 &&
          Number(waste?.wastageQuantity ?? 0) > 0
        )
          flags.push("wastage_risk");
        if (Number(expiry?.trackedQuantity ?? 0) > 0) flags.push("near_expiry");
        if (openQuantity >= grossQuantity && grossQuantity > 0)
          flags.push("covered_by_open_po");
        if (!history?.supplierId && netQuantity > 0)
          flags.push("supplier_not_assigned");
        ...
        return {
          ...
          daysRemaining:
            snapshot.daysUntilStockout === null
              ? null
              : round(snapshot.daysUntilStockout, 1),
          expectedStockoutDate:
            snapshot.expectedStockoutDate?.toISOString() ?? null,
          recommendedPurchaseDate: purchaseDate(
            snapshot.daysUntilStockout,
            input.leadTimeDays,
          ),
          ...
          nearExpiryTrackedQuantity: round(
            Number(expiry?.trackedQuantity ?? 0),
            4,
          ),
      ...
      .sort(
        (left, right) =>
          ...
          left.recommendedPurchaseDate.localeCompare(
            right.recommendedPurchaseDate,
          ) ||
    ...
    const supplierBuckets = new Map<
      string,
      ProcurementIntelligenceResult["suppliers"][number]
    >();
    const categoryBuckets = new Map<
      string,
      ProcurementIntelligenceResult["categories"][number]
    >();
    for (const row of recommendations.filter(
      (item) => item.netRecommendedQuantity > 0,
    )) {
    ...
    const grossRecommendedValue = round(
      recommendations.reduce(
        (sum, row) =>
          sum + row.grossRecommendedQuantity * row.estimatedUnitCost,
        0,
      ),
      2,
    );
    const openPurchaseOrderValue = round(
      openPoRows.reduce((sum, row) => sum + Number(row.openValue ?? 0), 0),
      2,
    );
    const netRecommendedValue = round(
      recommendations.reduce((sum, row) => sum + row.estimatedPurchaseValue, 0),
      2,
    );
    ...
    const deadStockItems = recommendations.filter((row) =>
      row.flags.includes("dead_stock"),
    ).length;
    const overstockItems = recommendations.filter((row) =>
      row.flags.includes("overstock"),
    ).length;
    const stockoutWithin7Days = recommendations.filter(
      (row) => row.daysRemaining !== null && row.daysRemaining <= 7,
    ).length;
    const nearExpiryTrackedValue = round(
      expiryRows.reduce((sum, row) => sum + Number(row.trackedValue ?? 0), 0),
      2,
    );
    const wastage30Value = round(
      wastageRows.reduce((sum, row) => sum + Number(row.wastageValue ?? 0), 0),
      2,
    );
    const inventoryHealthScore = Math.max(
      ...
      Math.round(
        100 -
          (highRiskItems / Math.max(recommendations.length, 1)) * 45 -
          (deadStockItems / Math.max(recommendations.length, 1)) * 20 -
          (overstockItems / Math.max(recommendations.length, 1)) * 15 -
          Math.min(wastage30Value / Math.max(grossRecommendedValue, 1), 1) * 20,
      ),
    );
    ...
    if (criticalItems > 0)
      alerts.push({
        severity: "critical",
        code: "critical_stockout",
        title: `${criticalItems} critical stock items`,
        message:
          "These ingredients may run out before the configured procurement lead time.",
        suggestedAction:
          "Raise or expedite purchase orders today and verify physical stock.",
      });
    const unassigned = recommendations.filter(
      (row) => row.netRecommendedQuantity > 0 && !row.preferredSupplierId,
    ).length;
    if (unassigned > 0)
      alerts.push({
        severity: "high",
        code: "supplier_unassigned",
        title: `${unassigned} purchase recommendations lack a supplier`,
        message:
          "Supplier assignment was not available from historical purchase orders.",
        suggestedAction:
          "Assign a preferred supplier before creating purchase orders.",
      });
    if (nearExpiryTrackedValue > 0)
      alerts.push({
        severity: "medium",
        code: "near_expiry",
        title: "Near-expiry exposure detected",
        message: `Tracked incoming batches worth approximately INR ${nearExpiryTrackedValue.toFixed(0)} expire within 14 days.`,
        suggestedAction:
          "Verify remaining batch quantities and prioritise consumption before replenishing.",
      });
    if (deadStockItems > 0 || overstockItems > 0)
      alerts.push({
        severity: "medium",
        code: "excess_inventory",
        title: "Slow or excess inventory detected",
        message: `${deadStockItems} inactive-demand and ${overstockItems} overstock item(s) require review.`,
        suggestedAction:
          "Pause replenishment, review recipes, and plan controlled utilisation.",
      });
    if (wastage30Value > 0)
      alerts.push({
        severity: wastage30Value > netRecommendedValue * 0.1 ? "high" : "low",
        code: "wastage_cost",
        title: "Recent inventory wastage",
        message: `The last 30 days contain INR ${wastage30Value.toFixed(0)} of recorded wastage.`,
        suggestedAction:
          "Review high-wastage ingredients before confirming reorder quantities.",
      });
    ...
    const result: ProcurementIntelligenceResult = {
      ...
      recommendations,
      suppliers: Array.from(supplierBuckets.values())
        .map((row) => ({
          ...row,
          quantity: round(row.quantity, 4),
          estimatedValue: round(row.estimatedValue, 2),
        }))
        .sort((a, b) => b.estimatedValue - a.estimatedValue),
      categories: Array.from(categoryBuckets.values())
        .map((row) => ({
          ...row,
          netRecommendedValue: round(row.netRecommendedValue, 2),
        }))
        .sort((a, b) => b.netRecommendedValue - a.netRecommendedValue),
      alerts,
    };
    ...
    await ProcurementIntelligenceRun.updateOne(
      { _id: intelligenceRun._id },
      {
        $set: {
          ...
          netPurchaseValue: netRecommendedValue,
          result,
        },
      },
    );
        ...
        $set: {
          ...
          errorMessage:
            error instanceof Error
              ? error.message
              : "Procurement intelligence generation failed.",
        },
      },
    );
    throw error;
