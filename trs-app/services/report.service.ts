...
export async function getExecutiveDashboard(range: ReportRange) {
  const previousRange = getPreviousRange(range);

  const [currentSales, previousSales, lowStock, purchaseOutstanding] =
    await Promise.all([
      getSalesTotals(range),
      getSalesTotals(previousRange),
      getReportCollection(REPORT_COLLECTIONS.inventoryItems).countDocuments({
        isActive: true,
        $expr: {
          $lte: ["$currentStock", "$reorderLevel"],
        },
      }),
      getReportCollection(REPORT_COLLECTIONS.purchaseOrders)
  ...
  return {
    ...
    averageOrderValue: {
      ...
      previousValue: previousSales.averageOrderValue,
      percentageChange: percentageChange(
    ...
    supplierOutstanding: purchaseOutstanding[0]?.outstanding ?? 0,
  };
}
...
export async function getSalesReport(range: ReportRange) {
  ...
  const [summary, daily, bySource, byFulfilment] = await Promise.all([
    getSalesTotals(range),
    orders
      .aggregate([
        {
...
export async function getOrdersReport(range: ReportRange) {
  ...
  const [byStatus, byHour, recent] = await Promise.all([
    orders
      .aggregate([
        {
          $match: {
      ...
      { $unwind: "$items" },
      {
        $group: {
          _id: {
            ...
            name: {
              $ifNull: ["$items.name", "Unknown item"],
            },
          },
          quantitySold: {
            $sum: {
  ...
  return orders
    .aggregate([
      ...
      },
      {
        $group: {
          _id: {
            $ifNull: ["$customerId", "$customerName"],
          },
          customerName: {
            $first: {
              $ifNull: ["$customerName", "Guest customer"],
            },
          },
          orders: { $sum: 1 },
          totalSpent: {
...
export async function getInventoryReport(range: ReportRange) {
  const items = getReportCollection(REPORT_COLLECTIONS.inventoryItems);
  const movements = getReportCollection(REPORT_COLLECTIONS.inventoryMovements);

  const [valuation, lowStock, movementSummary] = await Promise.all([
    items
      .aggregate([
        {
          $match: {
...
export async function getPurchasesReport(range: ReportRange) {
  const purchases = getReportCollection(REPORT_COLLECTIONS.purchaseOrders);

  const [summary, bySupplier, orders] = await Promise.all([
    purchases
      .aggregate([
        {
          $match: {
      ...
      .aggregate([
        ...
        },
        {
          $project: {
            ...
            supplierId: "$_id",
            supplierName: {
              $ifNull: ["$supplier.name", "Unknown supplier"],
            },
            orders: 1,
            purchaseValue: {
              $round: ["$purchaseValue", 2],
...
export async function getPaymentReport(range: ReportRange) {
  const payments = getReportCollection<PaymentDocument>(
...
export async function getExportRows(
  ...
  if (report === "sales") {
    ...
    return result.daily as Array<Record<string, unknown>>;
  }

  if (report === "orders") {
    const result = await getOrdersReport(range);
    return result.recent as Array<Record<string, unknown>>;
  }

  if (report === "menu-performance") {
    return (await getMenuPerformanceReport(range)) as Array<
      Record<string, unknown>
    >;
  }

  if (report === "customers") {
    return (await getCustomerReport(range)) as Array<Record<string, unknown>>;
  }

  if (report === "inventory") {
    const result = await getInventoryReport(range);
    return result.valuation as Array<Record<string, unknown>>;
  }
...
export async function getWaiterTipReport(range: ReportRange) {
    ...
    orders
      .aggregate([
        { $match: match },
        {
          $group: {
            _id: null,
            onlineTipsReceived: { $sum: "$tipAmount" },
            orderCount: { $sum: 1 },
            restaurantQrReceived: {
              $sum: {
                $add: [
                  {
                    $subtract: [
                      "$grandTotal",
                      { $ifNull: ["$waivedAmount", 0] },
                    ],
                  },
                  "$tipAmount",
                ],
        ...
        },
        {
          $project: {
            _id: 0,
            onlineTipsReceived: { $round: ["$onlineTipsReceived", 2] },
            restaurantQrReceived: { $round: ["$restaurantQrReceived", 2] },
            orderCount: 1,
          },
        },
      ])
        ...
        { $match: match },
        {
          $group: {
            _id: {
              $cond: [
                {
                  $gt: [{ $strLenCP: { $ifNull: ["$orderTakerName", ""] } }, 0],
                },
                "$orderTakerName",
                "Unassigned waiter",
              ],
  ...
  return {
    summary: summaryRows[0] ?? {
      onlineTipsReceived: 0,
      restaurantQrReceived: 0,
      orderCount: 0,
    },
    byWaiter,
    recent: recent.map((order) => ({
      ...
      saleAmount: Math.max(
        0,
        Number(order.grandTotal || 0) - Number(order.waivedAmount || 0),
      ),
      restaurantReceived:
        Math.max(
          0,
