...
export async function getSalesAnalytics(range: DashboardDateRange) {
  ...
  const [
    ...
    Order.aggregate([
      ...
      { $unwind: "$parts" },
      { $group: { _id: "$parts.method", amount: { $sum: "$parts.amount" } } },
      {
        $project: {
          _id: 0,
          method: "$_id",
    ...
    Order.aggregate([
      ...
      },
      {
        $project: {
          ...
          tax: "$taxTotal",
          refund: {
            $cond: [{ $eq: ["$paymentStatus", "refunded"] }, "$grandTotal", 0],
          },
          net: {
            $cond: [{ $eq: ["$paymentStatus", "refunded"] }, 0, "$grandTotal"],
          },
          cashier: {
            $ifNull: [
              { $arrayElemAt: ["$cashier.name", 0] },
              "$orderTakerName",
            ],
          },
          status: 1,
  ...
  return {
    range: { from: range.from, to: range.to, granularity: grouping.label },
    summary: {
      ...s,
      averageOrderValue: s.orders
        ? Number((s.netSales / s.orders).toFixed(2))
        : 0,
