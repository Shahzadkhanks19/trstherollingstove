...
import {
  updateRunningOrder,
  transferRunningOrder,
  voidRunningItem,
  updateTable,
} from "@/services/pos-operations.service";
...
async function execute(
  operation: SyncInput["operations"][number],
  actorId: string,
) {
  switch (operation.operationType) {
    ...
    case "running_order.transfer":
      return transferRunningOrder(
        operation.entityId,
        operation.payload.tableId,
        operation.payload.guestCount,
        actorId,
      );
    case "running_order.void_item":
      return voidRunningItem(
        operation.entityId,
...
export async function processOfflineSync(
  ...
  for (const operation of input.operations) {
    const existing = await POSOfflineOperation.findOne({
    ...
    const ledger = await POSOfflineOperation.create({
      ...
      clientCreatedAt: operation.clientCreatedAt
        ? new Date(operation.clientCreatedAt)
        : null,
    });
