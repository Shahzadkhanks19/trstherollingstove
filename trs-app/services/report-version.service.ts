...
export function reportSnapshot(source: Record<string, unknown>) {
  return Object.fromEntries(
    SNAPSHOT_FIELDS.map((field) => [field, source[field]]),
  );
}
...
export async function createReportVersion(
  ...
  await ReportVersion.updateOne(
    ...
    {
      $setOnInsert: {
        snapshot: reportSnapshot(current as unknown as Record<string, unknown>),
        changeSummary,
        createdBy: actorId,
      },
    },
...
export async function restoreReportVersion(
  ...
  const versionRecord = await ReportVersion.findOne({
    reportId,
    version,
  }).lean();
  ...
  const report = await ReportDefinition.findOneAndUpdate(
    ...
    {
      $set: {
        ...(versionRecord.snapshot as Record<string, unknown>),
        updatedBy: actorId,
      },
      $inc: { version: 1 },
    },
  ...
  if (!report)
    throw new AppError("Only the report owner can restore a version.", 403);
  return report;
}
