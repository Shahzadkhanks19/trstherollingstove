...
export async function getPosAnalytics(input: {
  ...
  const [
    ...
    Order.aggregate<HourRow>([
      ...
      { $project: { _id: 0, hour: "$_id", orders: 1, revenue: 1, items: 1 } },
      { $sort: { hour: 1 } },
    ]),
    Order.aggregate<CategoryRow>([
      ...
      },
      {
        $set: {
          categoryId: {
            $ifNull: [
              { $arrayElemAt: ["$menuItem.categoryId", 0] },
              { $arrayElemAt: ["$posItem.categoryId", 0] },
            ],
          },
        },
      },
  ...
  const peak = hourly.reduce(
    ...
    { hour: "—", orders: 0, revenue: 0, items: 0 },
  );
  return {
