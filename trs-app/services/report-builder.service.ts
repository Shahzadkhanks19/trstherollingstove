...
function normalizeValue(
  value: ReportFilterDefinition["value"],
  type: ReportColumnType,
): unknown {
  const convert = (
    ...
    if (type === "number") {
      ...
      if (!Number.isFinite(parsed))
        throw new AppError(
          "A numeric report filter contains an invalid value.",
          422,
        );
      return parsed;
    }
    ...
    if (type === "date") {
      ...
      if (Number.isNaN(parsed.getTime()))
        throw new AppError(
          "A date report filter contains an invalid value.",
          422,
        );
      return parsed;
    }
  ...
  return Array.isArray(value)
    ? value.map((entry) => convert(entry))
    : convert(value);
}

function toFieldMatch(
  ...
  switch (filter.operator) {
    ...
    case "contains": {
      if (column.type !== "string")
        throw new AppError("Contains can only be used with text fields.", 422);
      return {
        [field]: {
    ...
    default:
      throw new AppError("Unsupported filter operator.", 422);
  }
}

function dateGroupExpression(
  path: string,
  interval: ReportGroupDefinition["interval"],
): unknown {
...
function aggregationExpression(
  aggregation: ReportAggregation,
  path: string,
): unknown {
  switch (aggregation) {
    case "sum":
      return { $sum: { $ifNull: [path, 0] } };
    case "avg":
      return { $avg: { $ifNull: [path, 0] } };
    case "min":
      return { $min: path };
    case "max":
      return { $max: path };
    case "count":
      return { $sum: 1 };
    default:
      throw new AppError("Unsupported report aggregation.", 422);
  }
}
...
function assertDefinition(
  input: Pick<
    ReportDefinitionInput,
    "columns" | "filters" | "groups" | "sort" | "chart" | "visualization"
  >,
  columns: RegistryColumn[],
): Map<string, RegistryColumn> {
  const byKey = new Map(columns.map((column) => [column.key, column]));
  const selected = new Set(input.columns.map((column) => column.key));
  for (const column of input.columns) {
    const registryColumn = byKey.get(column.key);
    if (!registryColumn)
      throw new AppError(`Unsupported column: ${column.key}`, 422);
    if (registryColumn.type !== column.type)
      throw new AppError(`Column type mismatch: ${column.key}`, 422);
    if (
      column.aggregation !== "none" &&
      column.aggregation !== "count" &&
      registryColumn.type !== "number"
    ) {
      throw new AppError(
        `${column.label} does not support ${column.aggregation}.`,
        422,
      );
    }
  }
  for (const filter of input.filters)
    if (!byKey.has(filter.field))
      throw new AppError(`Unsupported filter field: ${filter.field}`, 422);
  for (const group of input.groups) {
    const column = byKey.get(group.field);
    if (!column || !selected.has(group.field))
      throw new AppError(
        `Grouping field must be selected: ${group.field}`,
        422,
      );
    if (group.interval !== "none" && column.type !== "date")
      throw new AppError("Date grouping intervals require a date field.", 422);
  }
  for (const sort of input.sort)
    if (!selected.has(sort.field))
      throw new AppError(`Sort field must be selected: ${sort.field}`, 422);
  if (input.visualization !== "table") {
    if (!selected.has(input.chart.categoryField))
      throw new AppError(
        "Chart category must be a selected report column.",
        422,
      );
    for (const series of input.chart.series) {
      ...
      if (!column || column.type !== "number")
        throw new AppError(
          "Chart series must use selected numeric columns.",
          422,
        );
    }
  }
...
function buildPipeline(
  ...
  for (const filter of input.filters)
    matchParts.push(toFieldMatch(byKey.get(filter.field)!, filter));

  const pipeline: PipelineStage[] = [
    { $match: matchParts.length === 1 ? matchParts[0] : { $and: matchParts } },
  ];
  const hasAggregation = input.columns.some(
    (column) => column.aggregation !== "none",
  );
  const isGrouped = hasAggregation || input.groups.length > 0;

  if (isGrouped) {
    const groupsByField = new Map(
      input.groups.map((group) => [group.field, group]),
    );
    ...
    const groupStage: Record<string, unknown> = {
      _id: Object.keys(id).length ? id : null,
    };
    for (const selected of input.columns) {
      if (selected.aggregation !== "none")
        groupStage[selected.key] = aggregationExpression(
          selected.aggregation,
          byKey.get(selected.key)!.path,
        );
      else if (!groupsByField.has(selected.key))
        throw new AppError(
          `Non-aggregated column must be grouped: ${selected.label}`,
          422,
        );
    }
    ...
    const projection: Record<string, unknown> = { _id: 0 };
    for (const selected of input.columns)
      projection[selected.key] = byKey.get(selected.key)!.path;
    pipeline.push({ $project: projection });
  }

  const sort: Record<string, 1 | -1> = Object.fromEntries(
    input.sort.map((item) => [item.field, item.direction === "asc" ? 1 : -1]),
  );
  if (Object.keys(sort).length) pipeline.push({ $sort: sort });
  else if (input.columns.some((column) => column.key === "createdAt"))
...
export async function executeReportPreview(
  ...
  try {
    const rows =
      await Order.aggregate<Record<string, unknown>>(pipeline).allowDiskUse(
        false,
      );
    ...
    return {
      ...
      grouped:
        ...
        input.columns.some((column) => column.aggregation !== "none"),
    };
  } catch (error) {
