...
export function calculateNextRun(
  ...
  if (schedule.frequency === "one_time")
    return schedule.runAt && schedule.runAt > after ? schedule.runAt : null;
  ...
  let month =
    schedule.frequency === "yearly" ? schedule.monthOfYear : current.month;
...
export async function assertReportExists(reportId: string) {
  const report = await ReportDefinition.findOne({
    ...
    .lean();
  if (!report)
    throw new AppError("Selected report does not exist or is archived.", 404);
  return report;
}
...
export async function createScheduledReport(
  ...
  if (!nextRunAt)
    throw new AppError(
      "The schedule does not have a future execution time.",
      400,
    );
  ...
  await ReportScheduleAudit.create({
    scheduleId: schedule._id,
    action: "created",
    actorId,
    metadata: { nextRunAt, frequency: input.schedule.frequency },
  });
  return schedule;
}
...
export async function updateScheduledReport(
  ...
  if (input.schedule || schedule.status === "active") {
    const normalized =
      input.schedule ?? normalizeScheduleConfig(schedule.schedule);
  ...
  await ReportScheduleAudit.create({
    ...
    action: "updated",
    actorId,
    metadata: { fields: Object.keys(input), nextRunAt: schedule.nextRunAt },
  });
  return schedule;
}

export async function performScheduleAction(
  id: string,
  action: "pause" | "resume" | "run_now" | "archive" | "restore",
  actorId: string,
) {
  const schedule = await ScheduledReport.findOne({
    _id: id,
  ...
  if (action === "run_now") {
    if (schedule.deletedAt)
      throw new AppError("Restore this schedule before running it.", 400);
  ...
  if (action === "resume") {
    if (schedule.deletedAt)
      throw new AppError("Restore this schedule before resuming it.", 400);
    const normalized = normalizeScheduleConfig(schedule.schedule);
    const next = calculateNextRun(normalized);
    if (!next)
      throw new AppError("This schedule has no future execution time.", 400);
