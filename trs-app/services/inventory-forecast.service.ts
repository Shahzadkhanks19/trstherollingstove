...
function velocityClass(forecastDailyDemand: number, activeDemandDays: number) {
...
export async function generateInventoryForecast(options: ForecastOptions) {
  ...
  try {
    ...
    for (const row of movementRows) {
      const itemDemand =
        demandByItem.get(row.inventoryItemId) ?? new Map<string, number>();
      itemDemand.set(row.date, row.quantity);
      demandByItem.set(row.inventoryItemId, itemDemand);
    }

    const dates = Array.from({ length: options.lookbackDays }, (_, index) => {
    ...
    const snapshots = items.map((item) => {
      ...
      const demandMap = demandByItem.get(itemId) ?? new Map<string, number>();
      const daily = dates.map((date) => demandMap.get(date) ?? 0);
      ...
      const weightedDemand =
        recentAverage * 0.5 + thirtyDayAverage * 0.3 + averageDailyDemand * 0.2;
      ...
      const trendMultiplier = clamp(1 + trendPercent / 200, 0.6, 1.6);
      const forecastDailyDemand = Math.max(0, weightedDemand * trendMultiplier);
      ...
      const reorderPoint =
        forecastDailyDemand * options.leadTimeDays + safetyStock;

      const horizonDemand = forecastDailyDemand * options.horizonDays;
      ...
      const daysUntilStockout =
        forecastDailyDemand > 0
          ? Number(item.currentStock ?? 0) / forecastDailyDemand
          : null;
      const expectedStockoutDate =
        daysUntilStockout !== null && Number.isFinite(daysUntilStockout)
          ? new Date(generatedAt.getTime() + daysUntilStockout * 86_400_000)
          : null;

      const activeDemandDays = daily.filter((value) => value > 0).length;
      const coverageRatio =
        options.lookbackDays > 0 ? activeDemandDays / options.lookbackDays : 0;
      const coefficientOfVariation =
        averageDailyDemand > 0 ? demandStdDev / averageDailyDemand : 1;
      ...
      return {
        ...
        forecastWeeklyDemand: round(forecastDailyDemand * 7, 4),
        forecastMonthlyDemand: round(forecastDailyDemand * 30, 4),
        ...
        recommendedOrderQuantity: round(recommendedOrderQuantity, 4),
        recommendedOrderValue: round(
          recommendedOrderQuantity * Number(item.averageUnitCost ?? 0),
          2,
        ),
        daysUntilStockout:
          daysUntilStockout === null ? null : round(daysUntilStockout, 1),
        ...
        velocityClass: velocityClass(forecastDailyDemand, activeDemandDays),
    ...
    if (snapshots.length) {
      await InventoryForecastSnapshot.insertMany(snapshots, { ordered: false });
    }
    ...
    const recommendedOrderValue = snapshots.reduce(
      (sum, snapshot) => sum + snapshot.recommendedOrderValue,
      0,
    );
    ...
    run.recommendedOrderValue = round(recommendedOrderValue, 2);
    ...
    return {
      ...
      itemCount: snapshots.length,
      highRiskCount,
      recommendedOrderValue: round(recommendedOrderValue, 2),
...
export async function getForecastSummary(runId?: string) {
  ...
  const [summaryRows, categories, riskDistribution] = await Promise.all([
    InventoryForecastSnapshot.aggregate([
      { $match: { runId: run._id } },
      {
        $group: {
          ...
          fastMoving: {
            $sum: {
              $cond: [{ $eq: ["$velocityClass", "fast"] }, 1, 0],
            },
          },
        },
      },
  ...
  return {
    run: {
      ...
      recommendedOrderValue: run.recommendedOrderValue,
      durationMs: run.durationMs,
