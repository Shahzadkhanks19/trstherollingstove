...
export async function calculateForecastAccuracy(runId: string) {
  const run = await BusinessForecastRun.findById(runId)
  ...
  if (!eligible.length) {
    return ForecastAccuracySnapshot.create({
      runId,
  ...
  const to = new Date(
    `${eligible.at(-1)?.date ?? eligible[0].date}T23:59:59.999Z`,
  );
  ...
  const actualMap = new Map(
    actualRows.map((row) => [row.date, Number(row.revenue ?? 0)]),
  );
  const errors = eligible.map((row) => ({
    forecast: row.revenue,
    actual: actualMap.get(row.date) ?? 0,
  }));
  ...
  const rmse = Math.sqrt(
    errors.reduce((sum, row) => sum + (row.forecast - row.actual) ** 2, 0) /
      errors.length,
  );
  const percentageErrors = errors
    .filter((row) => row.actual > 0)
    .map((row) => (Math.abs(row.forecast - row.actual) / row.actual) * 100);
  const mape = percentageErrors.length
    ? percentageErrors.reduce((sum, value) => sum + value, 0) /
      percentageErrors.length
    : 100;
  const bias =
    errors.reduce((sum, row) => sum + (row.forecast - row.actual), 0) /
    errors.length;
  ...
  const driftDetected =
    ...
    Math.abs(bias) >
      Math.max(
        500,
        (errors.reduce((sum, row) => sum + row.actual, 0) / errors.length) *
          0.25,
      );
  return ForecastAccuracySnapshot.create({
    runId,
...
export async function getForecastGovernanceDashboard(limit = 30) {
  const [runs, governance, accuracy, audits, failedRuns] = await Promise.all([
    BusinessForecastRun.find({ status: "completed" })
      .sort({ createdAt: -1 })
      .limit(limit)
      .select(
        "source lookbackDays dataDays durationMs result completedAt createdAt requestedBy",
      )
      .lean(),
    ...
    ForecastAudit.find({})
      ...
      .lean(),
    BusinessForecastRun.countDocuments({
      status: "failed",
      createdAt: { $gte: new Date(Date.now() - 7 * DAY_MS) },
    }),
  ]);
  const governanceMap = new Map(
    governance.map((row) => [String(row.runId), row]),
  );
  const accuracyMap = new Map<string, (typeof accuracy)[number]>();
  for (const row of accuracy)
    if (!accuracyMap.has(String(row.runId)))
      accuracyMap.set(String(row.runId), row);
  const items = runs.map((run) => {
    const id = String(run._id);
    const result = run.result as BusinessForecastResult | null;
    const control = governanceMap.get(id);
    const metric = accuracyMap.get(id);
    return {
      id,
      source: run.source,
      lookbackDays: run.lookbackDays,
      dataDays: run.dataDays,
      durationMs: run.durationMs,
      completedAt: run.completedAt,
      createdAt: run.createdAt,
      qualityScore: result?.quality.score ?? 0,
      forecast30Revenue: result?.summary.forecast30Revenue ?? 0,
      status: control?.status ?? "draft",
      versionNumber: control?.versionNumber ?? 1,
      modelVersion: control?.modelVersion ?? "statistical-v1",
      notes: control?.notes ?? "",
      accuracy: metric
        ? {
            sampleDays: metric.sampleDays,
            mae: metric.mae,
            rmse: metric.rmse,
            mape: metric.mape,
            bias: metric.bias,
            ...
            evaluatedThrough: metric.evaluatedThrough,
          }
        : null,
    };
  });
  const latest = items[0];
  const completedLast7Days = await BusinessForecastRun.countDocuments({
    status: "completed",
    createdAt: { $gte: new Date(Date.now() - 7 * DAY_MS) },
  });
  const published = items.filter((row) => row.status === "published").length;
  const measured = items.filter((row) => row.accuracy);
  const averageAccuracy = measured.length
    ? round(
        measured.reduce(
          (sum, row) => sum + (row.accuracy?.accuracyScore ?? 0),
          0,
        ) / measured.length,
        1,
      )
  ...
  const reliabilityScore = Math.max(
    ...
    Math.round(
      100 -
        failedRuns * 12 -
        (stale ? 20 : 0) -
        items.filter((row) => row.accuracy?.driftDetected).length * 5,
    ),
  );
  return {
...
export async function performForecastGovernanceAction(input: {
  runId: string;
  ...
  if (!Types.ObjectId.isValid(input.runId))
    throw new Error("Invalid forecast run ID.");
  const run = await BusinessForecastRun.findOne({
    _id: input.runId,
    status: "completed",
  })
    .select("_id")
  ...
  if (input.action === "recalculate_accuracy") {
    ...
    await ForecastAudit.create({
      runId: input.runId,
      action: "accuracy_recalculated",
      actorId: input.actorId,
      notes: input.notes,
      metadata: {
        accuracyScore: accuracy.accuracyScore,
        sampleDays: accuracy.sampleDays,
      },
    });
    return { message: "Forecast accuracy recalculated." };
  }
  const governance = await ForecastGovernance.findOneAndUpdate(
    { runId: input.runId },
    {
      $setOnInsert: {
        runId: input.runId,
  ...
  if (input.action === "approve") {
    ...
    governance.approvedBy = new Types.ObjectId(input.actorId);
    governance.approvedAt = now;
  }
  if (input.action === "publish") {
    if (!governance.approvedAt)
      throw new Error("Forecast must be approved before publishing.");
    governance.status = "published";
    governance.publishedBy = new Types.ObjectId(input.actorId);
    governance.publishedAt = now;
  }
  if (input.action === "unpublish") {
    governance.status = "approved";
    governance.publishedBy = null;
    governance.publishedAt = null;
  }
  if (input.action === "archive") {
    governance.status = "archived";
    governance.archivedBy = new Types.ObjectId(input.actorId);
    governance.archivedAt = now;
  }
  if (input.action === "restore") {
    governance.status = governance.approvedAt ? "approved" : "draft";
    governance.archivedBy = null;
    governance.archivedAt = null;
  }
  if (input.action === "update_notes") governance.notes = input.notes;
  if (input.notes && input.action !== "update_notes")
    governance.notes = input.notes;
  ...
  await ForecastAudit.create({
    ...
    metadata: {
      ...
      versionNumber: governance.versionNumber,
    },
  });
