...
function orderCostPipeline(
  orderIdsMatch: Record<string, unknown>,
): PipelineStage[] {
  return [
...
export async function getInternalConsumptionFinancialReport(input: {
  ...
  const [customer, internal, expenses, customerOrderIds, internalOrderIds] =
    await Promise.all([
            ...
            itemMarkdowns: {
              $sum: {
                $map: {
                  ...
                  in: {
                    $multiply: [
                      { $ifNull: ["$$item.itemDiscountSavings", 0] },
                      "$$item.quantity",
                    ],
                  },
                },
            ...
            summary: [
              {
                $group: {
                  ...
                  netRevenue: {
                    $sum: { $subtract: ["$grandTotal", "$taxTotal"] },
                  },
                  tax: { $sum: "$taxTotal" },
                  manualDiscount: {
                    $sum: {
                      $max: [
                        0,
                        {
                          $subtract: [
                            "$discountTotal",
                            { $add: ["$couponDiscount", "$coinDiscount"] },
                          ],
                        },
                      ],
                    },
      ...
      Order.find(customerMatch as never)
        .select({ _id: 1 })
        .lean(),
      Order.find(internalMatch as never)
        .select({ _id: 1, saleType: 1 })
        .lean(),
    ]);
  ...
  const [customerCosts, internalCosts] = await Promise.all([
    customerIds.length
      ? InventoryMovement.aggregate<{ _id: unknown; cost: number }>(
          orderCostPipeline({ referenceId: { $in: customerIds } }),
        )
      : [],
    internalIds.length
      ? InventoryMovement.aggregate<{ _id: unknown; cost: number }>(
          orderCostPipeline({ referenceId: { $in: internalIds } }),
        )
      : [],
  ]);

  const customerCostMap = new Map(
    customerCosts.map((row) => [String(row._id), Number(row.cost)]),
  );
  const internalCostMap = new Map(
    internalCosts.map((row) => [String(row._id), Number(row.cost)]),
  );
  const customerCogs = round(
    customerCosts.reduce((sum, row) => sum + Number(row.cost), 0),
  );
  const internalConsumptionCost = round(
    internalCosts.reduce((sum, row) => sum + Number(row.cost), 0),
  );
  const operatingExpenses = round(
    expenses.reduce((sum, row) => sum + Number(row.amount), 0),
  );
  const inputTax = round(
    expenses.reduce((sum, row) => sum + Number(row.tax), 0),
  );
  ...
  const adjustedOperatingProfit = round(
    grossProfit - operatingExpenses - internalConsumptionCost,
  );
  ...
  const byType = (internal[0]?.byType ?? []).map((row) => {
    const ids = internalOrderIds
      .filter((order) => order.saleType === row._id)
    ...
    return {
      ...
      inventoryCost: round(
        ids.reduce((sum, id) => sum + (internalCostMap.get(id) ?? 0), 0),
      ),
    };
  });

  const dailyCostRows = await (customerIds.length
    ? InventoryMovement.aggregate<{ _id: string; cogs: number }>([
        {
          $match: {
            referenceType: "order",
            type: "sale",
  ...
  const dailyCostMap = new Map<string, number>(
    dailyCostRows.map((row) => [row._id, Number(row.cogs)]),
  );
  const revenueTrend = (customer[0]?.daily ?? []).map((row) => {
    ...
    return {
      ...
      tax: round(row.tax),
      cogs,
      grossProfit: round(dailyRevenue - cogs),
    };
  });
  ...
  discounts.total = round(
    discounts.manual +
      discounts.coupons +
      discounts.coins +
      discounts.itemMarkdowns +
      discounts.comboSavings,
  );

  const profitAndLoss: InternalConsumptionFinancialReport["profitAndLoss"] = [
    {
      key: "net_revenue",
      label: "Net revenue (excluding GST)",
      amount: netRevenue,
      kind: "income",
    },
    {
      key: "customer_cogs",
      label: "Customer order food cost",
      amount: customerCogs,
      kind: "expense",
    },
    {
    ...
    {
      key: "internal_consumption",
      label: "Internal consumption inventory cost",
      amount: internalConsumptionCost,
      kind: "expense",
    },
    {
      key: "adjusted_operating_profit",
      label: "Adjusted operating profit",
      amount: adjustedOperatingProfit,
      kind: "subtotal",
    },
  ];
  ...
  return {
    ...
    kpis: {
      ...
      grossMarginPercent:
        netRevenue > 0 ? round((grossProfit / netRevenue) * 100) : 0,
      foodCostPercent:
        netRevenue > 0 ? round((customerCogs / netRevenue) * 100) : 0,
      internalConsumptionPercent:
        netRevenue > 0
          ? round((internalConsumptionCost / netRevenue) * 100)
          : 0,
      orderCount,
    ...
    internalConsumption: {
      ...
      costCoveragePercent:
        internalMenuValue > 0
          ? round((internalConsumptionCost / internalMenuValue) * 100)
          : 0,
    ...
    gst: {
      ...
      effectiveTaxRatePercent:
        netRevenue > 0 ? round((taxCollected / netRevenue) * 100) : 0,
    },
    revenueTrend,
    ...
    profitAndLoss,
    dataQuality: {
      customerOrdersWithoutCost: Math.max(
        0,
        customerOrderIds.length - customerCostMap.size,
      ),
      internalOrdersWithoutCost: Math.max(
        0,
        internalOrderIds.length - internalCostMap.size,
      ),
      costCoveragePercent:
        totalTrackedOrders > 0
          ? round((costedOrders / totalTrackedOrders) * 100)
          : 100,
    },
  };
}
