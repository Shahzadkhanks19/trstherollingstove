...
const row = (
  ...
  label,
  amount: round(amount),
  debit: side === "debit" ? round(Math.max(0, amount)) : 0,
  credit: side === "credit" ? round(Math.max(0, amount)) : 0,
});

export async function buildFinancialReportSnapshot(input: {
  days: number;
  ...
  const [revenue, expenses, receivables, payables, documents, payments] =
    await Promise.all([
      buildRevenueSnapshot(input),
      buildExpenseSnapshot(input),
      buildAccountsReceivableSnapshot(input),
      buildAccountsPayableSnapshot(input),
      buildInvoiceReceiptSnapshot(input),
      buildPaymentManagementSnapshot(input),
    ]);
  if (
    !revenue?.metrics ||
    !expenses?.metrics ||
    ...
    throw new Error(
      "One or more finance modules did not return report metrics.",
    );
  }
  ...
  const outputTax = Number(
    revenue.metrics.taxCollected || documents.metrics.taxAmount,
  );
  ...
  const metrics = {
    ...
    grossProfit,
    netProfit,
    profitMargin: netRevenue ? round((netProfit / netRevenue) * 100) : 0,
    cashInflows: round(cashInflows),
    cashOutflows: round(cashOutflows),
    netCashFlow,
  ...
  const profitAndLoss = [
    ...
    row(
      ...
      "Net profit / (loss)",
      Math.abs(netProfit),
      netProfit >= 0 ? "credit" : "debit",
    ),
  ];
  const cashFlow = [
    row(
      "collections",
      "Customer and gateway collections",
      cashInflows,
      "debit",
    ),
    row(
  ...
  const balanceSheet = [
    ...
    row(
      ...
      Math.abs(workingCapital),
      workingCapital >= 0 ? "credit" : "debit",
    ),
  ];
  const trialBalance = [
    row("cash", "Cash / settlements", cashBalance, "debit"),
    row(
      "accounts_receivable",
      "Accounts receivable",
      accountsReceivable,
    ...
    row("operating_expense", "Operating expenses", operatingExpenses, "debit"),
    row("accounts_payable", "Accounts payable", accountsPayable, "credit"),
    row("sales_revenue", "Sales revenue", netRevenue, "credit"),
    row("output_tax", "Output tax", outputTax, "credit"),
  ...
  const gstSummary = [
    row(
      "taxable_sales",
      "Taxable sales",
      Number(revenue.metrics.taxableRevenue),
      "credit",
    ),
  ...
  for (const item of revenue.byDay ?? [])
    trendMap.set(item.key, {
      revenue: Number(item.netRevenue),
      expenses: trendMap.get(item.key)?.expenses ?? 0,
    });
  ...
  const trend = [...trendMap.entries()]
    .map(([date, values]) => ({
      ...
      profit: round(values.revenue - values.expenses),
    }))
    .sort((a, b) => a.date.localeCompare(b.date));
  const periodKey = `${dayKey(range.start)}_${dayKey(range.end)}`;
  return FinancialReportSnapshot.findOneAndUpdate(
    { periodKey },
    {
      $set: {
...
export async function getFinancialReportSummary(days: number) {
  ...
  const periodKey = `${dayKey(range.start)}_${dayKey(range.end)}`;
  return (
    (await FinancialReportSnapshot.findOne({ periodKey }).lean()) ??
    buildFinancialReportSnapshot({ days, source: "system" })
  );
}
