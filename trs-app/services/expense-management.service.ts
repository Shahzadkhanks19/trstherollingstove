...
import type {
  expenseCreateSchema,
  expenseUpdateSchema,
} from "@/validators/expense-management";
...
function totals(input: {
  ...
  const totalAmount = round(
    Math.max(0, input.subtotal + input.taxAmount - input.discountAmount),
  );
...
export async function buildExpenseSnapshot(input: {
  days: number;
  ...
  for (const expense of expenses) {
    const values = {
      ...
      paid: Number(expense.paidAmount),
      outstanding: Number(expense.outstandingAmount),
      tax: Number(expense.taxAmount),
    };
    totalExpenses += values.total;
    paidExpenses += values.paid;
    outstandingExpenses += values.outstanding;
    taxPaid += values.tax;
    if (expense.approvalStatus === "approved") {
      approvedCount += 1;
      approvedExpenses += values.total;
    }
    if (expense.approvalStatus === "pending") pendingCount += 1;
    if (expense.recurring?.enabled)
      recurringCommitments += values.outstanding || values.total;
  ...
  const metrics = {
    ...
    approvedExpenses: round(approvedExpenses),
    paidExpenses: round(paidExpenses),
    outstandingExpenses: round(outstandingExpenses),
    taxPaid: round(taxPaid),
    recurringCommitments: round(recurringCommitments),
    averageExpense: expenses.length
      ? round(totalExpenses / expenses.length)
      : 0,
  };
  ...
  return ExpenseSnapshot.findOneAndUpdate(
    { periodKey },
    {
      $set: {
        ...
        byCategory: finalize(byCategory).sort((a, b) => b.total - a.total),
        byDepartment: finalize(byDepartment).sort((a, b) => b.total - a.total),
...
export async function getExpenseSummary(days: number) {
  ...
  return {
    snapshot:
      snapshot ?? (await buildExpenseSnapshot({ days, source: "system" })),
    recentExpenses,
  };
}
