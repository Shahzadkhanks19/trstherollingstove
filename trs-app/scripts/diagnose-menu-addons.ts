...
async function main(): Promise<void> {
  ...
  if (!search) {
    throw new Error(
      'Provide the item slug or name, for example: npm run diagnose:menu-addons -- "cheese-burst-pizza"',
    );
  }
  ...
  if (rawModifierIds.length === 0) {
    ...
    const validIds = rawModifierIds
      .filter((id) => Types.ObjectId.isValid(id))
      .map((id) => new Types.ObjectId(id));
    const groups = await modifierGroups
    ...
    for (const id of rawModifierIds) {
      ...
      if (!group) {
        console.log({
          ...
          reason: "No modifiergroups document exists for this ID.",
        });
        continue;
      }

      const options = Array.isArray(group.options)
        ? group.options.map(asRecord)
        : [];
      ...
      if (activeOptions.length === 0)
        reasons.push("group has no active/available options");
  ...
  if (rawRecommendationIds.length > 0) {
    const validIds = rawRecommendationIds
      .filter((id) => Types.ObjectId.isValid(id))
      .map((id) => new Types.ObjectId(id));
    const recommendations = await menuItems
      .find({ _id: { $in: validIds } })
      .toArray();
    const byId = new Map(
      recommendations.map((entry) => [asId(entry._id), entry]),
    );
