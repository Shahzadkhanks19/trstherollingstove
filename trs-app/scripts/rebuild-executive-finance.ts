...
async function main() {
  ...
  const scenario = (process.argv[4] ?? "base") as
  ...
  const snapshot = await buildExecutiveFinanceSnapshot({
    days,
    fiscalYear,
    scenario,
    source: "scheduled",
  ...
  console.log(
    `Executive finance dashboard rebuilt: ${snapshot?.periodKey ?? "unknown period"}`,
  );
}
main().catch((error) => {
  console.error(error);
