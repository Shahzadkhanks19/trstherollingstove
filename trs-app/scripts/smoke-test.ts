async function main() {
  ...
  const baseUrl = process.env.SMOKE_TEST_BASE_URL ?? process.env.APP_URL;

  if (!baseUrl) {
    console.error("SMOKE_TEST_BASE_URL or APP_URL is required.");
    process.exit(1);
  }
    ...
    try {
      const response = await fetch(url, {
        ...
        headers: {
          ...
          "user-agent": "trs-production-smoke-test",
        },
